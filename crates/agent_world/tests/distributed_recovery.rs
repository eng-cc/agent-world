...
#[test]
fn recovery_after_storage_restart() {
    ...
    // Simulate storage restart by re-instantiating CAS.
    let store_restarted = LocalCasStore::new(&dir);
    assert!(store_restarted
        .has(&write.snapshot_manifest_ref.content_hash)
        .expect("manifest exists"));

    // Provide RR handlers based on restarted store.
    ...
    let validation = replay_validate_head("w1", &client, &store_restarted).expect("replay");
