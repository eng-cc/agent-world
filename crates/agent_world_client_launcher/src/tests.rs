...
fn stop_child_process_handles_already_exited_child() {
    let mut child = spawn_immediate_exit_child();
    let _ = child.wait().expect("wait");
    stop_child_process(&mut child).expect("stop should be no-op for exited child");
}

#[cfg(unix)]
#[test]
fn stop_child_process_sends_interrupt_for_graceful_exit() {
    let start = std::time::Instant::now();
    let mut child = Command::new("sh")
        .arg("-c")
        .arg("trap 'exit 0' INT; while :; do sleep 1; done")
        .spawn()
        .expect("spawn long running process");
    stop_child_process(&mut child).expect("stop child");
    assert!(
        start.elapsed() < Duration::from_millis(GRACEFUL_STOP_TIMEOUT_MS),
        "graceful stop should complete before fallback timeout"
    );
    assert!(child.try_wait().expect("try_wait").is_some());
}

fn spawn_immediate_exit_child() -> std::process::Child {
    #[cfg(unix)]
    {
    ...
    {
        panic!("unsupported host for child-process tests");
    }
}
