...
# 1) Build native binaries for launcher/live.
...
# 3) Generate one-command launch wrapper and readme for players.
...
Quick start:
1) Run: ./run-game.sh
2) Open URL printed by launcher (launcher also auto-opens browser by default).
