...
## 你可以期待的世界（畅想）
在这个世界里：
- Agent 会为了电力预算、硬件维护与升级、争夺数据源而行动；会选择工作、交易、学习技能，也会在冲突中妥协或对抗。
- 地点会有资源与规则（营业时间、禁区、治安）；资源会短缺、流通、被垄断或被发现替代品。
...
- M4：最小社会与经济（生产/消耗/交易/关系/声誉；核心为 WASM 动态调用系统，Agent 创造的新事物以 Rust 编写并编译为 WASM，通过事件/接口与世界交互）
