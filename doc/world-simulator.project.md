...
- [ ] C2 辐射源标度修订（发射强度与尺度关系可配置）
- [ ] C3 采集场模型对齐（近邻源 + 背景场，含距离项）
- [ ] C4 背景辐射守恒说明（`radiation_floor` 来源与边界）
- [ ] C5 运动学约束补齐（最大位移/速度上限）
...
- 下一步：M4（最小社会与经济）并行推进“背景故事物理一致性修订（C2 优先）”
- 最近更新：完成 C1 尺寸口径统一（默认半径与文档一致，2026-02-07）
