...
- 最右侧 Chat 面板：独立承载 Agent Chat，不与综合面板混排；顶部区域展开时显示，可通过模块可见性中的 `Chat` 开关隐藏。面板内提供可展开的“预设 Prompt”小区域：支持聊天预设编辑并一键填充输入框，同时可编辑 `system prompt`、`短期目标`、`长期目标`，并直接应用到当前目标 Agent。三个字段会显示系统默认值提示（未设置 override 时的生效值）。
