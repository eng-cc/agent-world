...
- [x] T0：输出项目管理文档（本文件）
- [ ] T1：扩展 LLM 决策主链路（市场/制度动作）+ `test_tier_required` 用例
- [ ] T2：DistFS 默认持久化主路径（写入+优先恢复+JSON 兜底）+ persistence 用例
- [ ] T3：live 生产默认拓扑（triad）+ CLI/拓扑测试
...
- 当前阶段：T1（进行中）
- 阻塞项：无
- 下一步：实现 LLM 新动作解析与 schema 扩展，补齐 required tests。
