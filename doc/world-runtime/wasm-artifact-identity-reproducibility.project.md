...
## 任务拆解
- [ ] WIR-1 设计文档落地（`doc/world-runtime/wasm-artifact-identity-reproducibility.md`）
- [ ] WIR-1 项目管理文档落地（本文件）
- [ ] WIR-2 固定 toolchain（`rust-toolchain.toml`）并对齐 CI workflow
- [ ] WIR-3 新增模块身份结构并接入 `ModuleManifest`
- [ ] WIR-3 runtime 增加身份完整性校验并补齐测试
- [ ] WIR-4 更新任务日志并完成回归
