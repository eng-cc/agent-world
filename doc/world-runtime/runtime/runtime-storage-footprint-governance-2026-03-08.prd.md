...
| Replication hot/cold retention 对齐 | `max_hot_commit_messages`, `cold_index`, `archive_bytes` | 超出热窗口后迁移到冷索引/对象存储，并更新 metrics | `hot -> cold-indexed` | 热窗口按高度连续保留；冷索引按 height -> content_hash | 自动执行；策略由 profile 决定 |
- Acceptance Criteria:
