# Agent World Runtime：Membership Dead-Letter Replay 重试计数与比率阈值数值语义硬化（15 点清单第八阶段）设计
...
## 1. 设计定位
定义数值正确性硬化第 8 阶段设计，聚焦 `Agent World Runtime：Membership Dead-Letter Replay 重试计数与比率阈值数值语义硬化（15 点清单第八阶段）` 涉及的计数、时间、比率或状态转移边界，消除溢出、截断与顺序漂移风险。
