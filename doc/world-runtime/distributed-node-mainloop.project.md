...
- [ ] DNM-2：新增 `crates/node`，实现 `NodeRole/NodeConfig/NodeRuntime/NodeSnapshot` 与单元测试。
- [ ] DNM-3：在 `world_viewer_live` 启动流程中接入节点自动启动，并补充 CLI 参数解析测试。
...
## 状态
- 当前阶段：DNM-1 完成，进入 DNM-2。
- 下一步：实现 `node` crate 主循环并补齐单元测试。
- 最近更新：2026-02-16。
