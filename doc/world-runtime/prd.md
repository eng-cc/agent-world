...
扩展事件与 native compatibility event 必须进入同一 canonical journal；任何 module effect 都必须经过 Kernel apply，失败不得留下半个 debit、credit、event 或 schedule。当前 public direct module call/command 与 trusted capability command 已使用 borrowed-base typed stage，在 module state/resource fee/effect/emit、event/intent sequence、queue/backpressure、journal 与 consensus 全部 prepare 后才不可失败安装；authority-drift 与 post-prepare 回归证明失败不发布中间态，`ModuleCallFailed` 会在丢弃 staged 业务变化后以现有格式单独原子记录一次。公开 `step()`、`step_with_modules()`、committed-context wrapper 与 governed module lifecycle proposal apply 仍使用 cloned `World` 粗粒度边界；其他 publication/infrastructure failure 不追加第二事件。内部阶段、其他 lifecycle 入口、instance-state 对齐、恢复/迁移写入、durable external effect 与 receipt/outbox 尚未统一，因此不能据此宣称已经提供完整 ExecutionReceipt 级别的原子提交。
