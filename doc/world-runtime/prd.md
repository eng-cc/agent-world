...
首个 CapabilityAuthorization 有状态切片覆盖 public invocation-context installation：System subject 所需的可选 `SystemIdentityInstalled` 与必需 `InvocationContextInstalled` 必须作为一个 typed prepared batch 预演授权 map、event id/era、journal/backpressure、最终 authorization root 与单个 tick-consensus candidate，再以不可失败步骤一起安装。post-prepare failure 不得留下 identity/context 半写、event-id 空洞或 root/consensus/metric 污染；相同 context 的重复安装继续保持 no-op，已提交事件继续使用既有 replay reducer 与顺序。本切片不覆盖 agent identity、budget account、grant registration、command/receipt 或 authorization admin 路径，因此 CapabilityAuthorization 及整体 ExecutionTransaction 能力仍为 `partial`。
