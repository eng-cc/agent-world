...
首批 CapabilityAuthorization 有状态切片覆盖 public grant registration、invocation-context 与 budget-account installation：System subject 所需的可选 `SystemIdentityInstalled` 与必需 `InvocationContextInstalled` 作为一个 typed prepared batch；`BudgetAccountInstalled` 复用该 batch 并增加 typed budget-map projection；`GrantRegistered` 同步预演 canonical grant JSON map。三条路径都必须预演授权 map、event id/era、journal/backpressure、最终 authorization root 与单个 tick-consensus candidate，再以不可失败步骤安装。post-prepare failure 不得留下 grant/identity/context/budget 半写、event-id 空洞或 root/consensus/metric 污染；相同对象的重复安装继续保持既有语义，已提交事件继续使用既有 replay reducer 与顺序。这些切片不覆盖 agent identity、command/receipt 或 authorization admin 路径，因此 CapabilityAuthorization 及整体 ExecutionTransaction 能力仍为 `partial`。
