# oasis7 Runtime：WASM 模块设计对齐增强（项目管理文档）

- 对应设计文档: `doc/world-runtime/wasm/wasm-agent-os-alignment-hardening.design.md`
- 对应需求文档: `doc/world-runtime/wasm/wasm-agent-os-alignment-hardening.prd.md`

审计轮次: 4

## 任务拆解（含 PRD-ID 映射）
- [x] T-MIG-20260303 (PRD-ENGINEERING-006): 逐篇阅读旧文档并完成人工重写迁移到 `.prd` 命名。
- [x] AOSA-1 ABI/Schema 合约约束增强（manifest 字段与 shadow 校验）。
- [x] AOSA-2 cap slot 绑定（manifest `cap_slots` + output `cap_slot` 解析）。
- [x] AOSA-3 pure 模块策略插件链路（effect 前置判定）。
- [x] AOSA-5 `ModuleContext` 元信息增强与贯通。
- [x] AOSA-6 `WasmExecutor` 磁盘编译缓存（wasmtime serialized module）。

## 依赖
- doc/world-runtime/wasm/wasm-agent-os-alignment-hardening.prd.md
- 现有 wasm ABI：`crates/agent_world_wasm_abi/src/lib.rs`
- 现有 runtime 模块执行链路：`crates/agent_world/src/runtime/world/module_runtime.rs`
- 执行器实现：`crates/agent_world_wasm_executor/src/lib.rs`
- 参考实现（只读）：`third_party/agent-os`

## 状态
- 当前阶段：五项对齐增强任务（AOSA-1/2/3/5/6）已全部完成。
- 最近更新：AOSA-6 已完成（WasmExecutor 落盘编译缓存 + 缓存损坏恢复测试）。
