...
- [ ] TASK-WORLD_RUNTIME-017 (PRD-WORLD_RUNTIME-016) [test_tier_required]: 引入线上发布清单主真源与节点加载策略切换（生产禁用本地未授权 fallback）。
- [ ] TASK-WORLD_RUNTIME-018 (PRD-WORLD_RUNTIME-016) [test_tier_required]: 将 `m1/m4/m5` builtin artifact 加载从仓库内置清单迁移到可治理清单入口，保留受控 bootstrap 兜底。
- [ ] TASK-WORLD_RUNTIME-019 (PRD-WORLD_RUNTIME-016) [test_tier_full]: 补齐线上 manifest 不可达/回滚/版本漂移场景回归与故障签名。
- [ ] TASK-WORLD_RUNTIME-020 (PRD-WORLD_RUNTIME-017) [test_tier_required]: 生产策略下禁用 `identity_hash_v1` 回退，强制 artifact ed25519 签名校验。
...
- [ ] TASK-WORLD_RUNTIME-027 (PRD-WORLD_RUNTIME-016) [test_tier_required]: 为现有 `ModuleRelease*` 状态机补齐 `release manifest` 映射关系与回放一致性测试，保障迁移期兼容。
