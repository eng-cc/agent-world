...
- Success Criteria:
  ...
  - SC-4: 违规发布（缺签名、缺角色审批、profile 冲突）必定被拒绝并记录 `ActionRejected` 原因。
...
| Shadow 校验 | `ModuleReleaseShadow { request_id }` | 校验 wasm_hash/ABI/limits/profile 冲突 | `Requested -> Shadowed` | `shadow_manifest_hash` 必须可复现 | 需已绑定角色的 operator 执行 |
| 角色审批 | `ModuleReleaseApproveRole { request_id, role }` | 记录角色审批 | `Shadowed -> PartiallyApproved/Approved` | 角色集合归一化，去重 | 审批人必须绑定该 role |
| 应用发布 | `ModuleReleaseApply { request_id }` | 应用 module_changes + profile_changes | `Approved -> Applied` | module_changes 先于 profile_changes，按 module_id/product_id/recipe_id 排序 | 需满足全部 required_roles |
| 产品 profile 治理 | `GovernProductProfile { proposal_id, profile }` | 校验字段白名单与冲突 | `pending -> governed` | `product_id` 作为唯一键 | 必须引用 Applied 的 proposal_id |
| 配方 profile 治理 | `GovernRecipeProfile { proposal_id, profile }` | 校验字段白名单与依赖产品 | `pending -> governed` | `recipe_id` 作为唯一键 | 必须引用 Applied 的 proposal_id |
| 工厂 profile 治理 | `GovernFactoryProfile { proposal_id, profile }` | 校验字段白名单与 tier/slots | `pending -> governed` | `factory_id` 作为唯一键 | 必须引用 Applied 的 proposal_id |
...
- Acceptance Criteria:
  ...
  - AC-4 (PRD-WORLD_RUNTIME-012): 缺少 artifact identity、角色审批不足、profile 冲突均触发 `ActionRejected`，并记录审计事件。
...
- Architecture Overview:
  - 玩家提交 wasm 工件或源码编译 -> module artifact 写入 store -> ModuleReleaseSubmit(包含 profile_changes) -> Shadow 校验(artifact/ABI/limits/profile冲突) -> 多角色审批 -> Apply：写入 ModuleEvent + ProfileGoverned 事件 -> world state 更新 module registry + profile maps -> 经济系统路由新制成品。
- Integration Points:
...
- Edge Cases & Error Handling:
  ...
  - profile 冲突（同 `product_id/recipe_id/factory_id` 不一致）-> Shadow 阶段拒绝。
...
| PRD-WORLD_RUNTIME-011 | TASK-WORLD_RUNTIME-012 | `test_tier_full` | 三节点发布压测场景，统计 `submit->apply` 延迟 | 发布链路时延 |
