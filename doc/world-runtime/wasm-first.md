...
### W6 测试补充（确定性）
- 规则模块调用顺序固定为 `module_id` 字典序，测试中显式验证事件记录顺序。
- 冲突：多个规则模块给出不同 `override_action` 时应拒绝，并给出可重复的拒绝原因。
- 拒绝：任一规则模块 `deny` 应稳定拒绝动作（即使前序模块 `modify`）。
- 覆盖：多个模块给出一致 `override_action` 时应稳定覆盖并应用。
