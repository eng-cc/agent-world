...
- 以 **预置 WASM 工件模块** 形式提供两项基础能力：
  - 低效率辐射发电（只能提供缓慢补能）
  - 小容量储能（只能支持短时活动）
- 保持内核最小可信边界不变：内核仍只保证位置/资源/审计，供能语义由模块决定。

> 现状对齐（2026-02-13）：runtime 已切换为 wasm-only 执行链路，本文“出厂模块”均指 wasm 工件模块。

## 范围

### In Scope
- 定义 2 个默认电力模块与对应模块清单（通过 wasm 工件安装）：
...
- 若后续切到多工件装载策略，需保证当前 ABI 与事件语义保持兼容。
