...
| P2 | 输出整改方案与验收命令 | A4-* 整改项 + 验收命令 | done |
| P3 | 执行整改并回写索引/引用 | 修订文档 + 索引/引用更新 | done |
| P4 | 回写 `审计轮次: 4` 与复审结论 | `S_round004` + ROUND-004 复审结果 | done |
...
| 编号 | 动作 | 产物 | 状态 |
| --- | --- | --- | --- |
| F4-001 | 统一 ROUND-004 统计分母口径（默认排除 `doc/devlog/**`） | 主台账口径说明 + 统计命令 | done |
| F4-002 | 补审 `doc/world-simulator/**` 未覆盖文档并即时回写 | 分区补审记录 + 进度日志增量 | done |
| F4-003 | 重生成 `S_round004` 并做日志集合对账 | 更新后的 `round-004-reviewed-files.md` + 对账结果 | done |
| F4-004 | 纠偏后再关闭 `A4-002/A4-006` 与 `P1/B4-002` | 状态回写 + 复审说明 | done |
