...
- 执行规则（即时回写）：
  - 当单篇文档完成“内容对齐修订”时，必须在同一提交中回写该文档 `审计轮次: 1`。
  - 若仅登记问题但未修订文档正文，则保持 `审计轮次: 0`，并在本记录“一致性问题/整改项”登记原因与计划。
...
| A-013 | 对已完成本轮抽样并完成修订的文档回写 `审计轮次: 1`（缺省=0 保留），形成可统计清单；执行要求为“单文档审计完成即同提交回写” | 各模块维护者 | 2026-03-12 | in_progress |
