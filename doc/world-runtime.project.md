...
## 状态
- 当前阶段：R7（分布式能力彻底拆分）进行中，R7-3 完成。
- 下一步：执行 R7-4（Viewer 协议并入 `agent_world_proto` 并完成双端适配）。
- 最近更新：完成 R7-3（移除 `agent_world` 根级 runtime/simulator 大 facade re-export，调用方切为显式模块路径，2026-02-14）
