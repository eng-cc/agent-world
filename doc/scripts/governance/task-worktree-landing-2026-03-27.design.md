...
- 当前缺口：任务完成后回流 `main` 的 landing 仍靠人工 git 序列，缺少统一失败语义与 JSON 契约。

## 2. Architecture
- branch 解析层：识别 source branch（默认当前 branch）和 target branch（默认 `main`）。
...
## 6. Rationale
- 选择在 source worktree rebase，而不是在 target worktree merge 后再修线性历史，是因为 task worktree 模型天然适合“每个需求自己整理历史，再进入 `main`”。
