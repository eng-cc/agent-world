...
- Proposed Solution: 新增 `scripts/new-task-worktree.sh` 作为标准入口，接收 `<module> <task>` 并派生稳定 `task/<module>-<task>` 分支名与默认 worktree 路径；默认阻断脏源 worktree，检测 branch/path 冲突，并提供可选 JSON 摘要供 agent 或上层脚本消费。
- Success Criteria:
...
| 命名派生 | `module_slug`、`task_slug`、`branch_name`、`worktrees_root` | 默认生成 branch/path | `validated -> named` | 默认 `task/<module>-<task>` 与 `../worktrees/<repo>-<module>-<task>` | scripts owner 可维护 |
...
- Acceptance Criteria:
  - AC-1: `scripts/new-task-worktree.sh --help` 明确列出 `<module> <task>`、`--base`、`--branch`、`--path`、`--json`、`--allow-dirty-source`。
