...
- [x] T0：输出项目管理文档（本文件）
- [ ] T1：默认构建启用 `wasmtime`（缺口 1）并补齐测试/校验
- [ ] T2：实现 node libp2p replication request/handler 通道（缺口 2）并补齐测试
- [ ] T3：实现跨主机 `triad_distributed` 拓扑（缺口 4）并补齐 CLI/启动测试
...
## 状态
- 当前阶段：进行中（T0 已完成，T1~T5 待执行）。
- 阻塞项：无。
- 下一步：执行 T1（默认 `wasmtime`）。
