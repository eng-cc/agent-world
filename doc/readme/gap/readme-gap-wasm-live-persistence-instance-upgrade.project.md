...
## 任务拆解
- [x] T0：输出设计文档（`doc/readme/gap/readme-gap-wasm-live-persistence-instance-upgrade.md`）与项目管理文档（本文件）
...
- [x] T4：新增对外 `upgrade_module` 动作，要求仅接口兼容可升级，并补齐治理/审计/测试
...
## 状态
- 当前阶段：已完成（T0~T4 全部完成）
- 最近更新：完成 T4，新增 `upgrade_module_from_artifact` 动作（`instance_id + from->to`），落地接口兼容门禁、治理闭环（propose/shadow/approve/apply）与审计事件 `DomainEvent::ModuleUpgraded`，并补齐 required-tier 升级成功/拒绝用例。
- 阻塞项：无。
