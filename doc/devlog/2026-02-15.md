...
- 完成内容：完成 WLB2（后端 WebSocket/HTTP bridge）。
  ...
  - 更新 `crates/oasis7/src/bin/world_viewer_live.rs`：新增 `--web-bind <addr>`，可同进程启动 bridge。
...
- 完成内容：完成 LBA3（Web 闭环验证）。
  - 启动 live server：
    - `env -u RUSTC_WRAPPER cargo run -p oasis7 --bin world_viewer_live -- llm_bootstrap --bind 127.0.0.1:5123 --web-bind 127.0.0.1:5111 --tick-ms 300`
  - 启动 web viewer：
