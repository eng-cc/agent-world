...
- 完成内容：完成 V2D2（2D 标签可读性增强）。
  - 在 `crates/oasis7_viewer/src/label_lod.rs` 增加按 `ViewerCameraMode` 分支的 LOD 参数策略，2D 模式放宽可见距离、标签容量和遮挡配额，3D 模式保持既有策略。
...
- 完成内容：执行并通过回归验证（test_tier_required 本任务最小闭环）。
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_viewer label_lod -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_viewer label_lod_params_two_d_are_more_permissive -- --nocapture`
...
- 完成内容：完成 V2D3（2D 流向层工业化表达）代码落地。
  - 修改 `crates/oasis7_viewer/src/world_overlay.rs`：
...
- 完成内容：完成 Viewer 缩放上限放开，解决大场景下“无法拉远看全貌”问题。
  - 修改 `crates/oasis7_viewer/src/main.rs`：将 `ORBIT_MAX_RADIUS` 从 `300.0` 提升到 `5_000.0`。
  - 修改 `crates/oasis7_viewer/src/camera_controls.rs`：将 2D 正交缩放上限 `ORTHO_MAX_SCALE` 从 `8.0` 提升到 `128.0`。
  - 修改 `crates/oasis7_viewer/src/viewer_automation.rs`：同步自动化缩放上限 `AUTOMATION_ORTHO_MAX_SCALE` 到 `128.0`，保证截图闭环路径一致。
  - 新增测试：
...
- 完成内容：执行并通过回归验证（test_tier_required 本任务最小闭环）。
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_viewer camera_controls::tests:: -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo check -p oasis7_viewer`
...
- 完成内容：完成缩放常量去重，改为单一来源。
  - 在 `crates/oasis7_viewer/src/main.rs` 新增共享常量：`ORTHO_MIN_SCALE`、`ORTHO_MAX_SCALE`。
  - 在 `crates/oasis7_viewer/src/camera_controls.rs` 移除本地 `ORTHO_*` 定义，改为引用父模块共享常量。
  - 在 `crates/oasis7_viewer/src/viewer_automation.rs` 移除 `AUTOMATION_ORTHO_*` 定义，自动化缩放改为与手动缩放共用 `ORTHO_*` 边界。
  - 更新项目文档状态：
    - `doc/world-simulator/viewer-open-world-sandbox-readiness.project.md`（新增并勾选 OWR3.9）
- 完成内容：执行并通过回归验证（test_tier_required 本任务最小闭环）。
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_viewer camera_controls::tests:: -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_viewer viewer_automation::tests:: -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo check -p oasis7_viewer`
- 遗留事项：`oasis7_viewer` 仍存在既有未使用 import 警告（`selection_emphasis.rs`），与本任务无关，后续可单独清理。

- 日期：2026-02-15
- 完成内容：完成 QAG2/QAG3（Viewer 事件联动“快速定位 Agent”按钮）代码落地。
  - 修改 `crates/oasis7_viewer/src/selection_linking.rs`：新增 `quick_locate_agent_action`（优先定位当前选中 Agent，否则定位字典序首个 Agent），并补充按钮组件与标签刷新逻辑。
  - 修改 `crates/oasis7_viewer/src/egui_right_panel.rs`：在 Event Link 区新增“定位 Agent / Locate Agent”按钮并接入快速定位动作。
  - 修改 `crates/oasis7_viewer/src/ui_locale_text.rs`：新增 `quick_locate_agent_label`，并补充联动状态文案的中文映射。
  - 修改 `crates/oasis7_viewer/src/selection_linking/tests.rs`：新增快速定位行为测试，并更新事件联动按钮布局/标签测试覆盖。
...
- 完成内容：完成 QAG4/QAG5 回归与文档收口。
  ...
  - 执行并通过回归验证（test_tier_required 本任务最小闭环）：
    ...
    - `env -u RUSTC_WRAPPER cargo test -p oasis7_viewer ui_locale_text -- --nocapture`
    - `env -u RUSTC_WRAPPER cargo check -p oasis7_viewer`
  - 提交钩子附带执行并通过：`env -u RUSTC_WRAPPER cargo test -p oasis7 --tests --features test_tier_required`。
- 遗留事项：`oasis7_viewer` 仍有既有未使用 import 警告（`selection_emphasis.rs`），与本任务无关，后续可单独清理。

- 日期：2026-02-15
- 完成内容：补齐快速定位 Agent 按钮的旧 UI 系统调度接线。
  - 修改 `crates/oasis7_viewer/src/app_bootstrap.rs`：在 Update 链中接入 `selection_linking::handle_quick_locate_agent_button`。
  - 修改 `crates/oasis7_viewer/src/selection_linking.rs`：移除临时 `dead_code` 抑制属性。
  - 执行并通过回归：
    - `env -u RUSTC_WRAPPER cargo test -p oasis7_viewer selection_linking::tests:: -- --nocapture`
    - `env -u RUSTC_WRAPPER cargo check -p oasis7_viewer`
- 遗留事项：`oasis7_viewer` 仍有既有未使用 import 警告（`selection_emphasis.rs`），与本任务无关。
...
- 完成内容：完成 CFX2（Location 渲染尺度映射修复）。
  - 修改 `crates/oasis7_viewer/src/scene_helpers.rs`：Location 实体缩放从“米口径”改为按 `radius_cm * cm_to_unit` 的 world unit 映射，并增加渲染半径上下限钳制，避免 `llm_bootstrap` 场景中大体量碎片遮屏。
...
- 完成内容：执行并通过回归验证（test_tier_required 本任务最小闭环）。
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_viewer location_render_radius_units_scales_by_world_units_and_clamps -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo check -p oasis7_viewer`
...
- 完成内容：完成 CFX3（2D 自动聚焦缩放同步）。
  - 修改 `crates/oasis7_viewer/src/camera_controls.rs`：将 2D 正交缩放同步函数提升为模块内可复用接口。
  - 修改 `crates/oasis7_viewer/src/auto_focus.rs`：2D 模式自动聚焦写入 `orbit.radius` 后，同步正交投影 `scale`；若投影类型异常则回填 2D 正交投影。
...
- 完成内容：执行并通过回归验证（test_tier_required 本任务最小闭环）。
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_viewer apply_focus_to_camera_syncs_two_d_projection_scale_with_radius -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo check -p oasis7_viewer`
...
- 完成内容：完成 CFX4（默认信息密度收敛与回归收口）。
  - 修改 `crates/oasis7_viewer/src/right_panel_module_visibility.rs`：调整右侧模块默认可见性，保留核心区块（controls/overview/event_link），默认收起 overlay/diagnosis/timeline/details。
...
- 完成内容：执行并通过回归验证（test_tier_required 本任务最小闭环）。
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_viewer right_panel_module_visibility -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo check -p oasis7_viewer`
  - `./scripts/capture-viewer-frame.sh --scenario llm_bootstrap --addr 127.0.0.1:5023 --tick-ms 300 --viewer-wait 12 --capture-max-wait 120 --no-prewarm --keep-tmp`
...
- 完成内容：执行并通过回归验证（test_tier_required）。
  ...
  - `env -u RUSTC_WRAPPER cargo check -p oasis7_viewer`
- 遗留事项：`oasis7_viewer` 存在既有未使用代码/导入告警（location detail 旧渲染辅助与材质句柄），与本任务目标无冲突，后续可单独清理。

- 日期：2026-02-15
- 完成内容：完成 Viewer 编译告警清理（`oasis7_viewer` warning=0）。
...
- 完成内容：执行并通过回归验证（test_tier_required 本任务最小闭环）。
  - `env -u RUSTC_WRAPPER cargo check -p oasis7_viewer`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_viewer`
- 遗留事项：当前 `oasis7_viewer` 编译告警已清零；后续若重新引入 location 细节渲染，需以新任务重新设计并接入最小可验证测试。
...
- 完成内容：完成 FSF2（frag 实际比例与选中显示修复）代码落地。
  - 修改 `crates/oasis7_viewer/src/scene_helpers.rs`：`location_render_radius_units` 改为 `radius_cm * cm_to_unit` 线性映射，移除渲染钳制，确保 frag 尺寸比例与数据口径一致。
  - 修改 `crates/oasis7_viewer/src/location_fragment_render.rs`：frag 分块实体新增 `BaseScale`（取初始 `Transform.scale`），修复选中高亮后比例被覆盖、取消选中无法恢复的问题。
  - 修改测试：
    - `crates/oasis7_viewer/src/tests_scene_entities.rs`：`spawn_location_entity_uses_linear_anchor_radius_scale`。
    - `crates/oasis7_viewer/src/tests_scene_grid.rs`：新增 `rebuild_scene_fragment_entities_preserve_base_scale_for_selection_reset`。
    - `crates/oasis7_viewer/src/scene_helpers.rs`：更新尺度映射单测为无钳制口径。
  - 更新项目文档状态：
...
- 完成内容：执行并通过回归验证（test_tier_required）。
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_viewer location_render_radius_units_scales_by_world_units_without_clamp -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_viewer rebuild_scene_fragment_entities_preserve_base_scale_for_selection_reset -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_viewer spawn_location_entity_uses_linear_anchor_radius_scale -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_viewer`
  - `env -u RUSTC_WRAPPER cargo check -p oasis7_viewer`
...
- 完成内容：完成 FSF5（二次修复：误选与选中缩放）代码落地。
  - 修改 `crates/oasis7_viewer/src/selection_linking.rs`：`pick_3d_selection` 的 location 拾取改为 `With<Mesh3d>` 过滤，避免点击时误选不可见 location 锚点。
  - 修改 `crates/oasis7_viewer/src/scene_helpers.rs`：新增 `should_apply_scale_highlight`，对 `SelectionKind::Fragment` 返回 false。
  - 修改 `crates/oasis7_viewer/src/main.rs`、`crates/oasis7_viewer/src/selection_linking.rs`、`crates/oasis7_viewer/src/event_click_list.rs`：高亮逻辑按 `should_apply_scale_highlight` 分支，frag 选中不再缩放，保持真实比例。
  - 新增/更新测试：
...
- 完成内容：执行并通过回归验证（test_tier_required）。
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_viewer apply_selection_does_not_scale_fragment_entity -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_viewer should_apply_scale_highlight_skips_fragment -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_viewer`
  - `env -u RUSTC_WRAPPER cargo check -p oasis7_viewer`
...
- 完成内容：完成 FSF6（frag 三次修复：去掉黄色 halo + Agent 量纲修正）代码落地。
  - 修改 `crates/oasis7_viewer/src/selection_emphasis.rs`：`SelectionKind::Fragment` 选中时不再显示 selection halo，并补充 `selection_emphasis_hides_halo_for_fragment_selection`。
  - 修改 `crates/oasis7_viewer/src/scene_helpers.rs`：Agent 几何尺寸（主体、模块环、标签偏移、附着位置）统一按 `effective_cm_to_unit` 映射至世界单位，修复与 frag/位置半径量纲不一致问题。
  - 修改 `crates/oasis7_viewer/src/scene_helpers_entities.rs`：2D Agent 地图标记的半径/厚度/偏移统一按世界单位缩放。
  - 修改 `crates/oasis7_viewer/src/tests_scene_entities.rs`：更新 Agent 尺寸与位置断言，按 `effective_cm_to_unit` 推导期望值，避免旧米制常量导致误判。
  - 更新项目文档状态：
...
- 完成内容：完成 FSF7（相机缩放/裁剪量纲修复，恢复 Agent 放大可见性）代码落地。
  - 修改 `/Users/scc/ccwork/oasis7/crates/oasis7_viewer/src/camera_controls.rs`：
  ...
  - 修改 `/Users/scc/ccwork/oasis7/crates/oasis7_viewer/src/auto_focus.rs`：
    - 自动聚焦/快捷聚焦半径下限改为米制语义再映射至世界单位，避免聚焦逻辑把相机强制拉回“公里级半径”。
  - 修改 `/Users/scc/ccwork/oasis7/crates/oasis7_viewer/src/viewer_automation.rs`：
    - automation `ZoomFactor` 使用动态最小半径口径，和手动缩放行为对齐。
  - 更新项目文档状态：
    - `/Users/scc/ccwork/oasis7/doc/world-simulator/viewer-frag-scale-selection-fix.project.md`（FSF7.1~FSF7.3 勾选完成）
    - `/Users/scc/ccwork/oasis7/doc/world-simulator.project.md`（勾选 Viewer Frag 四次修复）
- 完成内容：执行并通过回归验证（test_tier_required）。
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_viewer`
  - `env -u RUSTC_WRAPPER cargo check -p oasis7_viewer`
...
- 完成内容：完成 OVZ1（Viewer 全览图缩放切换）文档任务初始化。
  - 新增设计文档：`/Users/scc/ccwork/oasis7/doc/world-simulator/viewer-overview-map-zoom.md`。
  - 新增项目管理文档：`/Users/scc/ccwork/oasis7/doc/world-simulator/viewer-overview-map-zoom.project.md`（OVZ1.1/OVZ1.2 勾选）。
  - 更新总项目文档：`/Users/scc/ccwork/oasis7/doc/world-simulator.project.md`，挂载全览图任务入口。
- 完成内容：执行文档任务最小回归。
  - `env -u RUSTC_WRAPPER cargo check -p oasis7_viewer`
...
- 完成内容：完成 OVZ2/OVZ3（Viewer 全览图缩放切换）代码与回归。
  - 缩放层级机制：
    - 在 `/Users/scc/ccwork/oasis7/crates/oasis7_viewer/src/camera_controls.rs` 新增 `TwoDZoomTier`（`Detail/Overview`）与迟滞阈值判定（`sync_two_d_zoom_tier`）。
    - 2D 默认 orbit 半径改为细节态默认倍率（不再默认世界全宽视角）。
  - 全览图可见性联动：
    - `TwoDMapMarker` 改为仅在 `TwoD + Overview` 显示。
    - 新增 `DetailZoomEntity`（`/Users/scc/ccwork/oasis7/crates/oasis7_viewer/src/scene_helpers.rs`），在 `TwoD + Overview` 隐藏细节几何（Agent 细节与 frag 分块）。
    - `DetailZoomEntity` 接入 `spawn_agent_entity` 与 `/Users/scc/ccwork/oasis7/crates/oasis7_viewer/src/location_fragment_render.rs`。
  - 调度接入：
    - `/Users/scc/ccwork/oasis7/crates/oasis7_viewer/src/app_bootstrap.rs` 新增 `TwoDZoomTier` 资源并接入 `sync_two_d_zoom_tier/sync_two_d_map_marker_visibility/sync_detail_zoom_visibility` 系统顺序。
    - `/Users/scc/ccwork/oasis7/crates/oasis7_viewer/src/main.rs` 补充 `TwoDZoomTier` 导入供启动装配使用。
  - 测试更新：
    ...
    - 全量 `oasis7_viewer` 测试从 211 增至 213，全部通过。
  - 项目文档更新：`/Users/scc/ccwork/oasis7/doc/world-simulator/viewer-overview-map-zoom.project.md` 勾选 OVZ2.1~OVZ3.2。
- 完成内容：执行并通过回归验证（test_tier_required）。
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_viewer`
  - `env -u RUSTC_WRAPPER cargo check -p oasis7_viewer`
...
- 完成内容：完成 OVZ3.3~OVZ3.5（全览图缩放切换后续修复）并收口“先放大后立刻变小”问题。
  - 修改 `crates/oasis7_viewer/src/auto_focus.rs`：启动自动聚焦完成后统一设置 `skip_next_mode_sync`，避免首帧 `sync_camera_mode` 把已聚焦半径回写为默认值。
  - 修改 `crates/oasis7_viewer/src/camera_controls.rs`：
  ...
  - 修改 `crates/oasis7_viewer/src/scene_helpers_entities.rs`：回退 2D Agent 标记到物理口径（移除调试期超大半径），并为 map marker 挂载 `BaseScale`。
  - 修改 `crates/oasis7_viewer/src/main.rs`、`crates/oasis7_viewer/src/scene_helpers.rs`：为 scene root 与 location anchor 补齐 `Visibility`，修复层级可见性传播告警（B0004）。
  - 修改 `crates/oasis7_viewer/src/app_bootstrap.rs`：接入 `sync_two_d_map_marker_scale` 调度顺序。
  - 更新文档状态：
...
- 完成内容：完成 Agent frag 初始站位优化（放在数据初始化阶段）。
  ...
  - 修改 `crates/oasis7/src/simulator/init.rs`：Agent 出生候选改为“有 frag 时优先 frag”；frag 出生位通过初始化阶段直接写入“上方 + 约 50m”坐标。
  - 新增 `crates/oasis7/src/simulator/frag_spawn.rs`：封装 frag 出生偏移计算与边界降级（优先 50m，越界时降级到 20m，仍越界则回退中心）。
  - 新增测试 `crates/oasis7/src/simulator/tests/init_agent_frag_spawn.rs`，并在 `tests/mod.rs` 挂载。
  - 更新总项目文档：`doc/world-simulator.project.md` 挂载并勾选对应任务项。
- 完成内容：执行并通过回归验证（test_tier_required 本任务最小闭环）。
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 init_agent_frag_spawn -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 asteroid_fragment_bootstrap_seeds_fragments_and_resources -- --nocapture`
...
- 完成内容：完成 Agent frag 初始站位 FSP4（二次修复：2D 视角遮挡）。
  - 修改 `crates/oasis7/src/simulator/frag_spawn.rs`：frag 出生方向改为严格正上方（`x/y` 与 frag 中心一致，仅抬升 `z`），优先 50m 表面间距，越界降级 20m，仍越界回退中心。
  - 修改 `crates/oasis7_viewer/src/scene_helpers.rs`：`agent_translation_for_render` 改为保留预计算径向距离 standoff，不再将 Agent 强制贴回 frag 表面。
  - 更新初始化测试 `crates/oasis7/src/simulator/tests/init_agent_frag_spawn.rs`：新增 `x/y` 对齐断言，确保“正上方”语义。
  - 新增 Viewer 测试：
    - `crates/oasis7_viewer/src/tests_scene_entities.rs`：`spawn_agent_entity_preserves_location_standoff_distance`
    - `crates/oasis7_viewer/src/tests.rs`：`spawn_agent_surface_standoff_test_system`
  - 更新文档：
...
- 完成内容：执行并通过回归验证（test_tier_required 本任务最小闭环）。
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 init_agent_frag_spawn -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_viewer spawn_agent_entity_attaches_to_location_surface -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_viewer spawn_agent_entity_preserves_location_standoff_distance -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo check -p oasis7`
  - `env -u RUSTC_WRAPPER cargo check -p oasis7_viewer`
...
- 完成内容：执行并通过最小闭环验证（test_tier_required）。
  - `env -u RUSTC_WRAPPER cargo check -p oasis7_builtin_wasm --target wasm32-unknown-unknown`
...
- 完成内容：完成 Wasm 构建一致性与模块身份升级（WIR-1~WIR-4）。
  ...
  - 模块身份升级（兼容模式）：
    - `crates/oasis7_wasm_abi/src/lib.rs` 新增 `ModuleArtifactIdentity`（`source_hash/build_manifest_hash/artifact_signature`）。
  ...
  - 测试补充：
    - `oasis7_wasm_abi` 新增 `module_artifact_identity_unsigned_signature_roundtrip`。
    - `oasis7` 新增：
      - `shadow_rejects_incomplete_module_artifact_identity`
      - `shadow_rejects_module_artifact_identity_signature_mismatch`
- 完成内容：执行并通过回归验证。
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_wasm_abi module_artifact_identity_unsigned_signature_roundtrip -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 shadow_rejects_incomplete_module_artifact_identity -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 shadow_rejects_module_artifact_identity_signature_mismatch -- --nocapture`
...
- 完成内容：完成 BWD-2~BWD-5（Builtin Wasm DistFS 存储与提交前校验）代码落地。
  ...
  - git 追踪策略改造：
    - 移除 `crates/oasis7/src/runtime/world/artifacts/m1_builtin_modules/*.wasm` 与 `m4_builtin_modules/*.wasm` 的 git 追踪。
...
- 完成内容：完成 DAC-2~DAC-4（Builtin Wasm DistFS API 闭环）代码落地。
  - 扩展 `oasis7_distfs`：
    - `crates/oasis7_distfs/src/lib.rs` 新增 `HashAlgorithm`（`Blake3`/`Sha256`）与 `LocalCasStore::new_with_hash_algorithm`。
  ...
  - 新增 DistFS hydrate 工具：
    - `crates/oasis7_distfs/src/bin/hydrate_builtin_wasm.rs`。
  ...
  - runtime 读取接线：
    ...
    - `crates/oasis7/src/runtime/blob_store.rs`、`crates/oasis7/src/runtime/mod.rs` 同步导出 `HashAlgorithm`。
- 完成内容：更新项目管理文档状态。
...
- 完成内容：完成 WBR2~WBR4（Viewer Bevy 浏览器运行路径）代码与入口接入。
  ...
  - 新增 Web 启动入口：
    - `crates/oasis7_viewer/index.html`（trunk rust pipeline，`data-bin=oasis7_viewer`）。
...
- 完成内容：执行并通过回归验证。
  - `env -u RUSTC_WRAPPER cargo check -p oasis7 --target wasm32-unknown-unknown`
  - `env -u RUSTC_WRAPPER cargo check -p oasis7_viewer --target wasm32-unknown-unknown`
  - `env -u RUSTC_WRAPPER cargo check -p oasis7_viewer`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_viewer`
- 遗留事项：浏览器在线连接 `oasis7_viewer_live` 仍未接入；后续可新增 WebSocket/HTTP bridge 任务打通在线模式。
...
- 遗留事项：当前 Web 运行仍为离线模式；在线连接 `oasis7_viewer_live` 需后续引入 WebSocket/HTTP bridge。
...
- 完成内容：完成 WCT3（Viewer Web 端闭环策略回归与收口）。
  - 执行并通过 wasm 最小回归：`env -u RUSTC_WRAPPER cargo check -p oasis7_viewer --target wasm32-unknown-unknown`。
...
- 完成内容：完成 WLB2（后端 WebSocket/HTTP bridge）。
  - 新增 `crates/oasis7/src/viewer/web_bridge.rs`：实现 `WebSocket <-> TCP line protocol` 双向桥接。
  - 更新 `crates/oasis7/src/viewer/mod.rs`：导出 `ViewerWebBridge*`。
  - 更新 `crates/oasis7/src/bin/oasis7_viewer_live.rs`：新增 `--web-bind <addr>`，可同进程启动 bridge。
  - 更新 `crates/oasis7/Cargo.toml`：增加 `tungstenite` 依赖。
...
- 完成内容：完成 WLB3（Web Viewer wasm WebSocket 接入）。
  - 更新 `crates/oasis7_viewer/src/main.rs`：
  ...
  - 更新 `crates/oasis7_viewer/Cargo.toml`：新增 wasm 端依赖（`web-sys`/`wasm-bindgen`/`gloo-timers`/`getrandom(js)`）。
...
- 完成 Web 端在线闭环验证（Playwright）：
  - live server: `oasis7_viewer_live llm_bootstrap --bind 127.0.0.1:5023 --web-bind 127.0.0.1:5011 --tick-ms 300`
...
- 完成内容：完成 BFC（Builtin Wasm 先拉取后编译回退）实现与 full 闭环测试。
  ...
  - runtime 实现：
    - 新增 `crates/oasis7/src/runtime/builtin_wasm_materializer.rs`，实现 builtin wasm 装载顺序：
      1) 本地 distfs 命中；
      2) 网络拉取（`OASIS7_BUILTIN_WASM_FETCHER` / `OASIS7_BUILTIN_WASM_FETCH_URLS`）；
      3) 拉取失败回退本地编译（`OASIS7_BUILTIN_WASM_COMPILER` 或默认 `scripts/build-wasm-module.sh`）。
    - `m1_builtin_wasm_artifact.rs`、`m4_builtin_wasm_artifact.rs` 接入统一 materializer。
  - 测试：
    - 新增 `crates/oasis7/src/runtime/tests/builtin_wasm_materializer.rs`
    - 闭环覆盖 `fetch miss -> fallback compile -> 写回 distfs -> 校验可读`（`test_tier_full`）。
- 完成内容：执行并通过回归验证。
  - `env -u RUSTC_WRAPPER cargo check -p oasis7 --tests --features test_tier_required`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 runtime::tests::power_bootstrap::install_power_bootstrap_modules_registers_and_activates --features "test_tier_full,wasmtime" -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 runtime::tests::builtin_wasm_materializer::materializer_fetch_miss_falls_back_to_compile_and_caches_blob --features "test_tier_full,wasmtime" -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 --tests --features "test_tier_full,wasmtime,viewer_live_integration" runtime::tests::builtin_wasm_materializer::materializer_fetch_miss_falls_back_to_compile_and_caches_blob -- --nocapture`
- 遗留事项：
...
- 完成内容：修复 Web Viewer 构建时 `reqwest::blocking` 在 `wasm32` 目标不可用导致的编译失败。
  ...
  - 报错位置：`crates/oasis7/src/runtime/builtin_wasm_materializer.rs`
  - 修复内容：
...
- 完成内容：执行并通过回归验证。
  ...
  - 关键结果：
    ...
    - `cargo test -p oasis7 --tests --features test_tier_required` 通过（354 passed）；
    - 新增的 `cargo check -p oasis7_viewer --target wasm32-unknown-unknown` 通过。
- 遗留事项：
  - 当前为“每次 pre-commit 均执行 viewer wasm32 check”，后续如需优化耗时，可按变更路径做条件触发（例如仅在 `oasis7` runtime/viewer 相关文件改动时执行）。
...
- 完成内容：执行并通过 `test_tier_required` 快速回归。
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 asteroid_fragment_default_mix_is_conservative_for_high_radiation_materials --features test_tier_required -- --nocapture`
- 遗留事项：
...
- 完成内容：完成 FRB2（frag 资源平衡与新手友好生成）实现。
  - 配置扩展：`crates/oasis7/src/simulator/world_model.rs`
  ...
  - 生成器改造：`crates/oasis7/src/simulator/asteroid_fragment.rs`
  ...
  - 测试补充：`crates/oasis7/src/simulator/tests/asteroid_fragment.rs`
    - 新增保底数量测试。
    - 新增中心区分布偏置测试。
    - 新增配置 sanitize 边界测试。
- 完成内容：执行并通过定向回归（`test_tier_required`）。
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 simulator::tests::asteroid_fragment:: --features test_tier_required -- --nocapture`
...
- 完成内容：执行并通过收口回归。
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 simulator::tests::asteroid_fragment::asteroid_fragment_generator_biases_distribution_toward_core_zone --features test_tier_required -- --nocapture`
- 遗留事项：
...
- 完成内容：完成 FRB4.2~FRB4.5 代码实现与测试。
  - 配置扩展：`crates/oasis7/src/simulator/world_model.rs`
    ...
    - `sanitized` 接入边界约束。
  - 运行期补种：
    - 新增 `crates/oasis7/src/simulator/kernel/fragment_replenish.rs`。
  ...
  - 资源类型分布策略：
    - 修改 `crates/oasis7/src/simulator/asteroid_fragment.rs`：材质采样支持按分区策略动态权重。
...
- 完成内容：执行并通过定向回归（`test_tier_required`）。
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 simulator::tests::asteroid_fragment:: --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 step_replenishes_fragments_every_hundred_ticks_at_one_percent --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 replay_from_snapshot_applies_fragment_replenished_event --features test_tier_required -- --nocapture`
- 遗留事项：
...
- 完成内容：完成 LBA1（LLM 建造链路动作解析与接线）。
  - `crates/oasis7/src/simulator/llm_agent/decision_flow.rs`：新增 `transfer_resource` 与 `refine_compound` 决策解析，补充 owner/resource kind 解析与错误兜底。
  - `crates/oasis7/src/simulator/llm_agent/decision_flow.rs`：扩展 `serialize_decision_for_prompt`，支持新增动作回写。
  - `crates/oasis7/src/simulator/llm_agent/tests.rs`：新增合法/非法路径测试（transfer/refine + invalid kind）。
  - `doc/world-simulator/llm-build-chain-actions.project.md`：LBA1 状态回写。
- 完成内容：执行并通过任务测试。
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 llm_agent -- --nocapture`
- 遗留事项：
...
- 完成内容：完成 LBA2（Prompt 约束与回归）。
  - `crates/oasis7/src/simulator/llm_agent/prompt_assembly.rs`：在 Decision JSON Schema 中新增 `transfer_resource`/`refine_compound` 动作定义、推荐模板与字段约束文案。
  - `crates/oasis7/src/simulator/llm_agent/tests_part2.rs`：补充 Prompt 内容断言，验证新动作提示词已进入用户提示。
...
- 完成内容：完成 LBA3（Web 闭环验证）。
  - 启动 live server：
    - `env -u RUSTC_WRAPPER cargo run -p oasis7 --bin oasis7_viewer_live -- llm_bootstrap --bind 127.0.0.1:5123 --web-bind 127.0.0.1:5111 --tick-ms 300`
  - 启动 web viewer：
...
- 完成内容：验收结果。
  - 页面加载成功（Title: `oasis7 Viewer (Web)`）。
