...
- 完成内容：继续以 `runtime_engineer` 视角完成 `PRD-WORLD_SIMULATOR-040 / TASK-WORLD_RUNTIME-040 / TASK-WORLD_SIMULATOR-150`。在 `crates/oasis7/src/simulator/decision_provider.rs` 为 `ObservationEnvelope` / `DecisionRequest` 补齐 `mode`、`observation_schema_version`、`action_schema_version`、`environment_class`、`fallback_reason`、`fixture_id`、`replay_id`，并为 `ProviderBackedAgentBehavior` 增加对应 builder；同时将元数据接入 `crates/oasis7/src/viewer/runtime_live/llm_sidecar.rs`、`crates/oasis7/src/bin/oasis7_openclaw_parity_bench.rs`、`crates/oasis7/src/bin/oasis7_openclaw_local_bridge.rs` 与 `scripts/openclaw-parity-p0.sh`，确保 runtime live 与 headless parity 产物共享统一 replay/summary traceability。
- 完成内容：补齐 `decision_provider` / `openclaw_adapter` / `openclaw_local_http` / `oasis7_openclaw_local_bridge` / `oasis7_openclaw_parity_bench` 的定向断言与测试覆盖，验证 schema version、mode、environment class、fixture/replay 字段均可稳定透传。
- 验证：`env -u RUSTC_WRAPPER cargo test -p oasis7 decision_provider -- --nocapture`、`env -u RUSTC_WRAPPER cargo test -p oasis7 openclaw_local_http -- --nocapture`、`env -u RUSTC_WRAPPER cargo test -p oasis7 openclaw_adapter -- --nocapture`、`env -u RUSTC_WRAPPER cargo test -p oasis7 --bin oasis7_openclaw_local_bridge -- --nocapture`、`env -u RUSTC_WRAPPER cargo test -p oasis7 --bin oasis7_openclaw_parity_bench -- --nocapture`、`env -u RUSTC_WRAPPER cargo fmt --all`、`env -u RUSTC_WRAPPER cargo check -p oasis7` 已通过。
