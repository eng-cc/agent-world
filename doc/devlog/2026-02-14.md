...
- 遗留事项：继续推进 R4-1（迁移模块清单与变更计划类型到 `oasis7_wasm_abi`）。
- 完成内容：完成 T96（继续激进下沉 runtime 逻辑，去掉 `distributed_head_follow` 路径包装）：
  - `crates/oasis7_net/src/head_tracking.rs`：
  ...
  - `crates/oasis7/src/runtime/distributed_head_follow.rs`：
    - 从 `#[path]` 同源包装改为 runtime 本地薄适配实现，复用 `oasis7_net::HeadTracker`，保留 `World` 引导语义和原对外 API（`HeadFollower`/`HeadUpdateDecision`）。
...
- 完成内容：执行并通过验证：
  ...
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 --test distributed_head_follow`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 --test distributed_observer_sync`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_net`
  - `./scripts/check-include-warning-baseline.sh`
- 遗留事项：继续下一阶段，优先把 `distributed_observer` / `distributed_observer_replay` 中可独立的状态编排逻辑下沉到 `oasis7_net`，进一步压缩 runtime 路径包装与别名层。

- 日期：2026-02-14
- 完成内容：完成 T97（继续激进下沉 observer 编排逻辑并去掉 runtime 路径包装）：
  - `crates/oasis7_net/src/head_sync.rs`：
  ...
  - `crates/oasis7/src/runtime/distributed_observer.rs`：
    - 从 `#[path]` 同源包装切换为 runtime 本地薄适配实现，复用 `oasis7_net::observer_flow`，保留原有 `ObserverClient` / `HeadSyncReport` / `HeadFollowReport` API。
  - `doc/world-runtime/distributed-crate-split-net-consensus.project.md`：
...
- 完成内容：执行并通过验证：
  ...
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_net`
  - `./scripts/check-include-warning-baseline.sh`
- 遗留事项：继续下一阶段，优先收敛 `distributed_observer_replay` 路径包装，并将其可独立校验编排抽象继续下沉到 `oasis7_net`。
...
- 完成内容：完成 T98（继续激进下沉 observer replay 编排逻辑并去掉 runtime 路径包装）：
  ...
  - `crates/oasis7/src/runtime/distributed_observer_replay.rs`：
    - 从 `#[path]` 同源包装切换为 runtime 本地薄适配实现，复用 `oasis7_net::observer_replay_flow`，保留原有 `replay_validate_*` API。
  - `doc/world-runtime/distributed-crate-split-net-consensus.project.md`：
...
- 完成内容：执行并通过验证：
  ...
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 --test distributed_observer_sync`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 --test distributed_head_follow`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 --test distributed_bootstrap`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_net`
  - `./scripts/check-include-warning-baseline.sh`
- 遗留事项：继续下一阶段，优先收敛剩余路径包装模块，并继续推进 `oasis7_net --features runtime_bridge` 的可编译闭环抽象。

- 日期：2026-02-14
- 完成内容：完成 T99（继续激进去除 runtime `distributed_bootstrap` 路径包装）：
  - `crates/oasis7/src/runtime/distributed_bootstrap.rs`：
...
- 完成内容：执行并通过验证：
  - `env -u RUSTC_WRAPPER cargo check -p oasis7`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 --test distributed_consistency`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 --test distributed_bootstrap`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 --test distributed_observer_sync`
  - `./scripts/check-include-warning-baseline.sh`
- 遗留事项：继续下一阶段，优先推进 `oasis7_net --features runtime_bridge` 在 storage/validation 方向的抽象闭环，减少 runtime 与 net 的重复实现面。


- 日期：2026-02-14
- 完成内容：完成 R4-1，将 `ModuleRole/ModuleManifest/ModuleChangeSet/ModuleActivation/ModuleDeactivation/ModuleUpgrade` 从 `crates/oasis7/src/runtime/modules.rs` 下沉到 `crates/oasis7_wasm_abi/src/lib.rs`，`oasis7` 改为通过 re-export 复用 ABI 定义。
- 完成内容：在 ABI crate 增补 `ModuleChangeSet::is_empty` 行为单元测试，验证迁移后接口可用性。
- 完成内容：执行回归测试 `env -u RUSTC_WRAPPER cargo test -p oasis7_wasm_abi -- --nocapture`、`env -u RUSTC_WRAPPER cargo test -p oasis7 runtime::tests::modules:: -- --nocapture`、`env -u RUSTC_WRAPPER cargo test -p oasis7 --test module_lifecycle -- --nocapture`，结果通过。
...
# 遗留事项
- 执行 R5-1：将 `ModuleRegistry/ModuleRecord/ModuleEvent/ModuleEventKind` 迁移至 `oasis7_wasm_abi` 并完成回归。
...
# 完成内容
- 完成 R5-1：将 `ModuleRegistry/ModuleRecord/ModuleEvent/ModuleEventKind` 迁移到 `oasis7_wasm_abi`。
- `oasis7` 的 `runtime::modules` 改为 re-export 上述类型，移除本地重复定义，保持外部导入兼容。
...
- 执行回归：
  ...
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 runtime::tests::modules:: -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 --test module_lifecycle -- --nocapture`
...
# 完成内容
- 完成 R6-1：新增 `oasis7_wasm_store` crate，承载 `ModuleStore` 文件持久化实现。
- 将 `oasis7` 的 `runtime::module_store` 改为轻量包装层，转调新 crate 并保持 `WorldError` 语义兼容。
- 更新 workspace 与 `oasis7` 依赖，接入新 crate。
- 补充 `oasis7_wasm_store` 单元测试（roundtrip + version mismatch）。
- 更新 R6 项目管理文档与总项目管理文档为已完成状态。
- 执行回归：
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_wasm_store -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 --test module_store -- --nocapture`
...
- 完成 R7-1：新增 `oasis7_distfs` crate（`BlobStore`/`LocalCasStore`、snapshot/journal 分片、组装校验）。
- `oasis7::runtime::blob_store` 与 `segmenter` 切换为复用 `oasis7_distfs`；补齐 proto/runtime 错误映射。

# 测试
- `env -u RUSTC_WRAPPER cargo check -p oasis7_distfs`
- `env -u RUSTC_WRAPPER cargo test -p oasis7_distfs`
- `env -u RUSTC_WRAPPER cargo check -p oasis7`
- `env -u RUSTC_WRAPPER cargo test -p oasis7 runtime::segmenter::tests::`

# 遗留事项
- 执行 R7-2：删除 `oasis7` 内分布式实现文件并切换调用方到 `oasis7_net`/`oasis7_consensus`/`oasis7_distfs`/`oasis7_proto`。


# 2026-02-14

## 完成内容
- 完成 R7-2：从 `crates/oasis7` 彻底移除分布式实现路径。
  - 删除 `crates/oasis7/src/runtime/distributed*.rs`、`crates/oasis7/src/runtime/distributed_membership_sync/` 与 `crates/oasis7/src/runtime/libp2p_net.rs`。
  - 删除 `crates/oasis7/tests/distributed_*.rs` 分布式集成测试文件。
  - 收敛 `crates/oasis7/src/runtime/mod.rs`：移除分布式模块声明与分布式 re-export，仅保留 world runtime 核心能力。
  - 收敛 `crates/oasis7/src/lib.rs`：移除分布式 API 导出（network/dht/consensus/membership/libp2p 等）。
  - 移除 world 核心中的分布式 fetch 入口：
    - `crates/oasis7/src/runtime/world/governance.rs` 删除 `shadow_proposal_with_fetch` / `apply_proposal_with_fetch`。
    - `crates/oasis7/src/runtime/world/module_runtime.rs` 删除 `load_module_with_fetch` / `prefetch_active_modules_with_fetch` / `ensure_module_changes_with_fetch`。
  - 修正分布式协议类型引用：
    - `crates/oasis7/src/runtime/segmenter.rs` 改为直接使用 `oasis7_proto::distributed::SnapshotManifest`。
    - `crates/oasis7/src/runtime/error.rs` 改为直接使用 `oasis7_proto::distributed::DistributedErrorCode`。
  - 精简 `crates/oasis7/Cargo.toml`：移除 `oasis7_net`、`oasis7_consensus`、`libp2p`、`futures` 依赖与对应 feature。
- 文档更新：
...
## 遗留事项
- 继续执行 R7-3：拆分 `oasis7` facade，进一步按领域收敛对外导出边界。


# 2026-02-14

## 完成内容
- 完成 R7-3：拆分 `oasis7` 大 facade，收敛导出边界并清理跨层 re-export。
  - `crates/oasis7/src/lib.rs`：移除根级对 `runtime`/`simulator` 的大规模 `pub use`，仅保留模块入口与几何/模型基础导出。
  - 调用方改造为显式模块路径：
    - `crates/oasis7/src/bin/world_viewer_demo.rs`
    - `crates/oasis7/src/bin/world_viewer_live.rs`
    - `crates/oasis7/src/bin/world_init_demo.rs`
    - `crates/oasis7/tests/{module_lifecycle,module_store,module_input_cbor,module_state,module_subscription_filters,wasm_executor,viewer_live_integration,viewer_offline_integration}.rs`
    - `crates/oasis7_net/src/{bootstrap,observer_replay}.rs`
  - 集成测试中的 runtime 类型引用统一改为 `oasis7::runtime::*`（避免继续依赖根级跨层导出）。
- 文档更新：
...
- 回归验证：
  - `env -u RUSTC_WRAPPER cargo check -p oasis7`
  - `env -u RUSTC_WRAPPER cargo check -p oasis7 --all-targets`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 --test module_lifecycle`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 --test viewer_offline_integration`
  - `env -u RUSTC_WRAPPER cargo check -p oasis7_net`

## 遗留事项
- 继续执行 R7-4：viewer 协议并入 `oasis7_proto` 并完成 server/viewer 双端适配。


# 2026-02-14

## 完成内容
- 完成 R7-4：将 viewer 协议并入 `oasis7_proto`，并完成 server/viewer 双端适配。
  - 新增 `crates/oasis7_proto/src/viewer.rs`：
    - 承载 `VIEWER_PROTOCOL_VERSION`、`ViewerRequest`、`ViewerControl`、`ViewerStream`、`ViewerEventKind`、`ViewerResponse`、`PromptControl*` 等协议定义。
    - `ViewerResponse` / `PromptControlAck` 使用泛型承载 snapshot/event/trace/metrics/time，避免 `proto` 直接依赖 `oasis7::simulator`。
  - `crates/oasis7_proto/src/lib.rs` 新增 `pub mod viewer;`。
  - `crates/oasis7/src/viewer/protocol.rs` 改为协议桥接层：
    - 将 runtime 具体类型通过 type alias 绑定到 `oasis7_proto::viewer` 泛型协议。
    - 保留 `viewer_event_kind_matches` 作为 `ViewerEventKind -> WorldEventKind` 过滤匹配函数。
  - `crates/oasis7/src/viewer/live.rs` 与 `crates/oasis7/src/viewer/server.rs` 切换到桥接匹配函数（替代原 `ViewerEventKind::matches` 内置实现）。
- 文档更新：
...
- 回归验证：
  ...
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_proto viewer::`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 viewer::protocol::tests::`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_viewer tests_prompt_control::`

## 遗留事项
- 继续执行 R7-5：收敛 WASM ABI 边界，去除 `oasis7_net` 中重复 `ModuleManifest` 定义并统一到 ABI/proto。


# 2026-02-14

## 完成内容
- 完成 R7-5：收敛 WASM ABI 边界，移除 `oasis7_net` 内重复 `ModuleManifest` 定义。
  - `crates/oasis7_net/src/lib.rs`：删除本地 `ModuleManifest` 结构体，`modules` 模块改为直接复用 `oasis7_wasm_abi::{ModuleArtifact, ModuleManifest}`。
  - `crates/oasis7_net/src/tests.rs`：按 ABI 统一后的 `ModuleManifest` 字段补齐测试构造（`kind/role/interface_version/exports/subscriptions/required_caps/limits`），确保 DHT 拉取模块清单路径仍可正确反序列化。
- 文档更新：
...
- 完成 R7-6：拆分全部超 1200 行 Rust 文件，并保持行为不变。
  - `crates/oasis7/src/simulator/llm_agent.rs`：抽离 OpenAI payload/响应解析相关函数到 `crates/oasis7/src/simulator/llm_agent/openai_payload.rs`。
  - `crates/oasis7/src/simulator/llm_agent/tests.rs`：拆分为主文件 + `tests_part2.rs`，保留辅助夹具在主文件，测试用例分段组织。
  - `crates/oasis7_viewer/src/tests.rs`：拆分为主文件 + `tests_scene_entities.rs` + `tests_selection_panels.rs`。
  - `crates/oasis7_viewer/src/scene_helpers.rs`：抽离实体生成与位置细节构造函数到 `scene_helpers_entities.rs`，主文件保留调用入口与公共 API。
  - `crates/oasis7_viewer/src/ui_text.rs`：抽离 owner/power activity 辅助函数到 `ui_text_activity.rs`。
- 行数约束复检：
...
- 回归验证：
  ...
  - `env -u RUSTC_WRAPPER cargo check -p oasis7_viewer`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 --features wasmtime simulator::llm_agent::tests::`
...
## 完成内容
- 删除 `crates/oasis7/src/runtime/sandbox.rs`，并在 `crates/oasis7/src/runtime/mod.rs` 移除 `mod sandbox` 与相关 re-export。
- 将 runtime/simulator/tests 中对 `ModuleCall* / ModuleOutput / ModuleSandbox` 的引用改为直接使用 `oasis7_wasm_abi`。
- 将测试中执行器类型引用显式收敛为 `oasis7_wasm_executor`（并修复 `ModuleEffectIntent` 导入与条件导入告警）。
- 新增 R8 设计与项目管理文档：
...
- 回归验证：
  - `env -u RUSTC_WRAPPER cargo check -p oasis7 --features wasmtime`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 --features wasmtime`
...
- 在 ABI 层新增 Recipe/Product/Factory 经济模块接口基础：
  - 新增 `crates/oasis7_wasm_abi/src/economy.rs`
  - 在 `crates/oasis7_wasm_abi/src/lib.rs` 引入并导出 economy 接口
  - 在 `crates/oasis7/src/runtime/modules.rs` 与 `crates/oasis7/src/runtime/mod.rs` 暴露新 ABI 类型
...
- 新增定向测试文件：`crates/oasis7/src/runtime/tests/economy.rs`，覆盖：
  - 工厂建造延迟完成与建造成本扣减
  - 配方排产的输入与电力扣减、到期产出入账
  - 产线槽位占满时拒绝新排产
  - 材料不足时拒绝建造
...
## 完成内容
- 在 `oasis7_builtin_wasm` 新增 M4 经济模块实现，覆盖三类模块：
...
- 验证命令：
  ...
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 --features wasmtime runtime::tests::economy_bootstrap -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 --features wasmtime runtime::tests::power_bootstrap::m1_builtin_module_ids_manifest_matches_runtime_constants -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo check -p oasis7 --features wasmtime`
...
1. 完成 M4-E7 Product 模块在线校验接线。
   - 在 `oasis7_wasm_abi` 新增 `ProductValidationRequest` 与 `ProductValidationDecision`。
...
4. 测试与检查通过。
   ...
   - `env -u RUSTC_WRAPPER cargo test -p oasis7_wasm_abi`
   - `env -u RUSTC_WRAPPER cargo test -p oasis7 runtime::tests::economy -- --nocapture`
   - `env -u RUSTC_WRAPPER cargo test -p oasis7 --features wasmtime runtime::tests::economy -- --nocapture`
   - `env -u RUSTC_WRAPPER cargo check -p oasis7 --features wasmtime`
...
- 运行验证：`env -u RUSTC_WRAPPER cargo check -p oasis7 --quiet`。
...
- 调整 `scripts/ci-tests.sh`：
  - `required` 改为 `cargo test -p oasis7 --tests --features test_tier_required`。
  - `full` 改为 `cargo test -p oasis7 --tests --features test_tier_full,wasmtime,viewer_live_integration`。
...
## 遗留事项
- 目前 feature 标签分级主要覆盖 `oasis7` crate；后续可按同样模式扩展到其他 crate 的 tests。
...
2. `env -u RUSTC_WRAPPER cargo test -p oasis7_wasm_abi economy::tests:: -- --nocapture`
3. `env -u RUSTC_WRAPPER cargo test -p oasis7 runtime::tests::economy_module_requests:: -- --nocapture`
4. `env -u RUSTC_WRAPPER cargo test -p oasis7 runtime::tests::economy:: -- --nocapture`
...
1. 按测试分级流程巡检现有测试 case 与 feature 覆盖。
   - 执行 `env -u RUSTC_WRAPPER cargo test -p oasis7 --tests --features test_tier_required`。
   - 执行 `CI_VERBOSE=1 ./scripts/ci-tests.sh full`（覆盖 `test_tier_full`、`wasmtime`、`viewer_live_integration`、`oasis7_net/libp2p`）。
...
- 完成内容：完成 IVA（任务 A：产线可视化 + 物流网络可视化）首版落地。
  - 新增工业摘要模块：`crates/oasis7_viewer/src/ui_text_industrial.rs`。
  ...
  - 新增工业区块中英文本地化映射与单测：`crates/oasis7_viewer/src/ui_locale_text.rs`。
- 完成内容：更新项目文档状态：
...
- 完成内容：执行并通过验证：
  ...
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_viewer localize_industrial_ops_block_supports_zh -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo check -p oasis7_viewer`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_viewer`
...
- 完成内容：完成 IVB（任务 B：经营面板）首版落地。
  - 新增经营看板聚合模块：`crates/oasis7_viewer/src/ui_text_economy.rs`。
  ...
  - 右侧详情区按需新增 `Economy Dashboard` 区块（仅存在经济信号时显示）：
    - `crates/oasis7_viewer/src/egui_right_panel.rs`
    - `crates/oasis7_viewer/src/ui_text.rs`
  - 新增经营看板本地化映射：`crates/oasis7_viewer/src/ui_locale_text.rs`。
- 完成内容：更新项目文档状态：
...
- 完成内容：执行并通过验证：
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_viewer ui_text_economy -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_viewer localize_economy_dashboard_block_supports_zh -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_viewer`
  - `env -u RUSTC_WRAPPER cargo check -p oasis7_viewer`
...
- 完成内容：完成 IVC（任务 C：多尺度导航 + 告警根因）首版落地。
  - 新增运营导航聚合模块：`crates/oasis7_viewer/src/ui_text_ops_navigation.rs`。
  - 多尺度摘要：
    - 世界层：近期活动/告警事件计数。
    - 区域层：按 chunk 聚合热点（events/alerts）。
    - 节点层：按 agent/location 聚合热点分数。
  - 告警根因：聚合 `ActionRejected` 的根因类型 Top 列表。
  - 右侧详情区新增可开关导航区块（默认关闭，启用环境变量 `OASIS7_VIEWER_SHOW_OPS_NAV=1`）：
    - `crates/oasis7_viewer/src/egui_right_panel.rs`
    - `crates/oasis7_viewer/src/ui_text.rs`
  - 新增导航区块本地化映射：`crates/oasis7_viewer/src/ui_locale_text.rs`。
- 完成内容：更新项目文档状态：
...
- 完成内容：执行并通过验证：
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_viewer ui_text_ops_navigation -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_viewer localize_ops_navigation_block_supports_zh -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_viewer`
- 遗留事项：继续推进 IVD（任务 D：产品化表现层）。

- 日期：2026-02-14
- 完成内容：完成 IVD（任务 D：产品化表现层）收口。
  - 右侧详情区新增产品化表现层开关：`OASIS7_VIEWER_PRODUCT_STYLE`（默认关闭，不影响既有快照与交互）。
  - 产品化卡片抽离为独立模块 `crates/oasis7_viewer/src/egui_observe_section_card.rs`，并在 `egui_right_panel.rs` 接入，确保单文件不超过 1200 行。
  - 语义色板：按世界/活动/工业/经营/运营/详情/事件分配色调与强调色。
  - 动效开关：新增 `OASIS7_VIEWER_PRODUCT_STYLE_MOTION`，对工业/经营/运营/事件卡片提供轻量边框脉冲过渡。
...
- 完成内容：执行并通过验证：
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_viewer`
  - `env -u RUSTC_WRAPPER cargo check -p oasis7_viewer`
  - `OASIS7_VIEWER_PRODUCT_STYLE=1 OASIS7_VIEWER_PRODUCT_STYLE_MOTION=1 ./scripts/capture-viewer-frame.sh --scenario llm_bootstrap --addr 127.0.0.1:5023 --tick-ms 300 --viewer-wait 12 --capture-max-wait 90 --no-prewarm`
...
- 完成内容：执行并通过验证：
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_viewer two_d_map_marker_visibility_follows_camera_mode -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_viewer spawn_location_entity_renders_fine_grained_ring_details -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_viewer spawn_agent_entity_renders_module_markers_up_to_cap -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo check -p oasis7_viewer`
