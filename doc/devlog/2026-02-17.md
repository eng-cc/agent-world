...
- 完成内容：修复 required-tier 回归测试前置条件，消除提交钩子失败。
  - 代码：`crates/oasis7/src/simulator/llm_agent/tests.rs`
...
- 测试：
  ...
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 llm_agent_parse_schedule_recipe_action_with_default_batches --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo run -p oasis7 --bin oasis7_llm_agent_demo -- llm_bootstrap --ticks 30 --print-llm-io --llm-io-max-chars 1800 --report-json output/llm_bootstrap/user_factory_closedloop_lfo6_rerun_2026-02-16_235611/report.json`
- 遗留事项：
...
- 完成内容：回写项目管理文档，勾选 LFO7.2-LFO7.4 并补充 LFO7 实施摘要。
  - 文档：`doc/world-simulator/llm-factory-strategy-optimization.project.md`
- 测试：
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 responses_tools_register_expected_function_names --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 response_function_call_maps_decision_tool_to_decision_json --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 build_responses_request_payload_includes_tools_and_required_choice --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 completion_result_from_raw_response_json_rejects_output_text_without_tool_call --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 prompt_assembly_includes_tool_call_constraints --features test_tier_required -- --nocapture`
- 遗留事项：
...
- 完成内容：修复 required-tier 回归断言并恢复提交链路。
  - 代码：`crates/oasis7/src/simulator/llm_agent/tests_part2.rs`
  - 变更：`llm_agent_system_prompt_contains_configured_goals` 断言从旧关键词 `module_call` 更新为 `agent_submit_decision`，匹配 tool-only 协议。
- 测试：
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 llm_agent_system_prompt_contains_configured_goals --features test_tier_required -- --nocapture`
- 遗留事项：
  - 无新增遗留；继续执行提交全链路验证。
...
- 完成内容：执行 tool-only 协议在线闭环抽样（用户目标：建工厂 + 制成品），并输出 JSON 报告。
  - 命令：`env -u RUSTC_WRAPPER OASIS7_LLM_SHORT_TERM_GOAL='你必须先建造可用工厂，再安排可执行配方，确保在本次运行内至少产出1批制成品。' OASIS7_LLM_LONG_TERM_GOAL='形成稳定生产链并持续产出制成品，避免无效等待。' cargo run -p oasis7 --bin oasis7_llm_agent_demo -- llm_bootstrap --ticks 30 --report-json output/llm_closed_loop/tool_only_llm_bootstrap_report.json`
...
- 完成内容：执行短程 `--print-llm-io` 抽样定位 parse_error 根因。
  - 命令：`env -u RUSTC_WRAPPER OASIS7_LLM_SHORT_TERM_GOAL='你必须先建造可用工厂，再安排可执行配方，确保在本次运行内至少产出1批制成品。' OASIS7_LLM_LONG_TERM_GOAL='形成稳定生产链并持续产出制成品，避免无效等待。' cargo run -p oasis7 --bin oasis7_llm_agent_demo -- llm_bootstrap --ticks 10 --print-llm-io --llm-io-max-chars 500`
...
- 完成内容：按用户指令执行一次在线闭环测试（`llm_bootstrap --ticks 30`），使用强化目标 prompt 驱动 Agent 完成“建工厂 + 制成品 data”。
  - 命令：`OASIS7_LLM_SYSTEM_PROMPT='你是工业生产调度Agent。主目标：在30 tick内形成闭环，必须先建工厂，再排产制成品data。禁止空转和重复无效动作；遇到reject_reason要立即切换恢复动作。严格输出一个tool call，不要输出文本JSON。' OASIS7_LLM_SHORT_TERM_GOAL='30 tick 内必须完成：1) build_factory(factory.assembler.mk1) 至少一次成功；2) schedule_recipe(recipe.assembler.logistics_drone) 至少两次成功，推动 data 增长。恢复策略：insufficient hardware->refine_compound(compound_mass_g>=7000)；insufficient electricity->harvest_radiation(max_amount>=120)；factory_not_found->build_factory；agent_already_at_location->不要move，改为refine或schedule。' OASIS7_LLM_LONG_TERM_GOAL='保持可持续工业闭环：harvest_radiation -> refine_compound -> build_factory -> schedule_recipe，避免连续3轮以上同类无效采集。' env -u RUSTC_WRAPPER cargo run -p oasis7 --bin oasis7_llm_agent_demo -- llm_bootstrap --ticks 30 --print-llm-io --llm-io-max-chars 1800 --report-json output/llm_bootstrap/user_factory_closedloop_codex_2026-02-17_115631/report.json | tee output/llm_bootstrap/user_factory_closedloop_codex_2026-02-17_115631/run.log`
...
- 完成内容：回写项目管理文档，新增 LFO10 任务与在线复跑摘要，并记录新观察 TODO。
  - 文档：`doc/world-simulator/llm-factory-strategy-optimization.project.md`
- 测试：
  - `OASIS7_LLM_SYSTEM_PROMPT='你是工业生产调度Agent。主目标：在30 tick内形成闭环，必须先建工厂，再排产制成品data。禁止空转和重复无效动作；遇到reject_reason要立即切换恢复动作。严格输出一个tool call，不要输出文本JSON。' OASIS7_LLM_SHORT_TERM_GOAL='30 tick 内必须完成：1) build_factory(factory.assembler.mk1) 至少一次成功；2) schedule_recipe(recipe.assembler.logistics_drone) 至少两次成功，推动 data 增长。恢复策略：insufficient hardware->refine_compound(compound_mass_g>=7000)；insufficient electricity->harvest_radiation(max_amount>=120)；factory_not_found->build_factory；agent_already_at_location->不要move，改为refine或schedule。' OASIS7_LLM_LONG_TERM_GOAL='保持可持续工业闭环：harvest_radiation -> refine_compound -> build_factory -> schedule_recipe，避免连续3轮以上同类无效采集。' env -u RUSTC_WRAPPER cargo run -p oasis7 --bin oasis7_llm_agent_demo -- llm_bootstrap --ticks 30 --print-llm-io --llm-io-max-chars 1800 --report-json output/llm_bootstrap/user_factory_closedloop_codex_2026-02-17_115631/report.json`
- 遗留事项：
...
- 完成内容：执行在线闭环复跑（LFO8 对比口径）验证 parse_error 收敛。
  - 命令：`env -u RUSTC_WRAPPER OASIS7_LLM_SHORT_TERM_GOAL='你必须先建造可用工厂，再安排可执行配方，确保在本次运行内至少产出1批制成品。' OASIS7_LLM_LONG_TERM_GOAL='形成稳定生产链并持续产出制成品，避免无效等待。' cargo run -p oasis7 --bin oasis7_llm_agent_demo -- llm_bootstrap --ticks 30 --report-json output/llm_closed_loop/todo11_lfo8_compare_2026-02-17_121121/report.json | tee output/llm_closed_loop/todo11_lfo8_compare_2026-02-17_121121/run.log`
...
- 测试：
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 rewrites_wait_action --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 llm_agent_rewrites_execute_until_wait_action_to_actionable_harvest --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 llm_agent_reroutes_schedule_recipe_when_hardware_cannot_cover_one_batch --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 llm_agent --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER OASIS7_LLM_SHORT_TERM_GOAL='你必须先建造可用工厂，再安排可执行配方，确保在本次运行内至少产出1批制成品。' OASIS7_LLM_LONG_TERM_GOAL='形成稳定生产链并持续产出制成品，避免无效等待。' cargo run -p oasis7 --bin oasis7_llm_agent_demo -- llm_bootstrap --ticks 10 --print-llm-io --llm-io-max-chars 700`
- 遗留事项：
...
- 完成内容：更新项目管理文档，新增 LFO12 任务与结果摘要，并记录本轮新增 TODO。
  - 文档：`doc/world-simulator/llm-factory-strategy-optimization.project.md`
- 测试：
  - `env -u RUSTC_WRAPPER cargo run -p oasis7 --bin oasis7_llm_agent_demo -- llm_bootstrap --ticks 60 --print-llm-io --llm-io-max-chars 1800 --report-json output/llm_bootstrap/all_factory_all_products_codex_2026-02-17_123501/report.json`
- 遗留事项：
...
- 测试：
  ...
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 refine_compound --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 simulator::tests::kernel:: --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 simulator::tests::persist:: --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 simulator::tests::conservation:: --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 simulator::tests::consistency:: --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 simulator::tests::kernel_rule_invariants:: --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 llm_agent_parse_mine_compound_action_defaults_to_self_owner --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 responses_tools_register_expected_function_names --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 build_responses_request_payload_includes_tools_and_required_choice --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 simulator::tests::init::scenarios_are_stable --features test_tier_required -- --nocapture`
- 遗留事项：
...
- 测试：
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 llm_agent_mock_sequence_recovers_and_completes_factory_recipe_chain --features test_tier_required -- --nocapture`
...
- 完成内容：完成 MMD2（debug 模式 LLM 补给工具）实现与回归收敛。
  - 配置与守卫：
    - 新增 `OASIS7_LLM_DEBUG_MODE`（默认 `false`），并增加布尔值解析与错误类型 `InvalidDebugMode`。
...
- 测试：
  - `env -u RUSTC_WRAPPER cargo check -p oasis7 --tests --features test_tier_required`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 llm_agent_parse_debug_grant_resource_action_when_debug_mode_enabled --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 llm_agent_rejects_debug_grant_resource_action_when_debug_mode_disabled --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 responses_tools_register_debug_grant_tool_in_debug_mode_only --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 response_function_call_maps_debug_grant_tool_to_typed_decision_turn --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 build_responses_request_payload_includes_debug_tool_when_enabled --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 llm_config_reads_debug_mode_from_env --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 llm_config_rejects_invalid_debug_mode --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 debug_grant_resource_adds_requested_amount_to_owner_stock --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 replay_from_snapshot_applies_debug_resource_granted_event --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo check -p oasis7_viewer --target wasm32-unknown-unknown`
- 遗留事项：
...
- 完成内容：完成 MMD3 闭环验证与收口。
  ...
  - 在线闭环抽样 #1（机制链路验证：先采矿再生产）：
    - 命令：`env -u RUSTC_WRAPPER cargo run -p oasis7 --bin oasis7_llm_agent_demo -- llm_bootstrap --ticks 60 --print-llm-io --llm-io-max-chars 1800 --report-json output/llm_bootstrap/mmd3_industrial_closure_2026-02-17_132336/report.json`
    - 产物：
  ...
  - 在线闭环抽样 #2（全配方覆盖尝试）：
    - 命令：`env -u RUSTC_WRAPPER cargo run -p oasis7 --bin oasis7_llm_agent_demo -- llm_bootstrap --ticks 80 --print-llm-io --llm-io-max-chars 1200 --report-json output/llm_bootstrap/mmd3_all_recipes_2026-02-17_132728/report.json`
    - 产物：
...
- 测试：
  - `env -u RUSTC_WRAPPER cargo check -p oasis7 --tests --features test_tier_required`
  - `env -u RUSTC_WRAPPER cargo check -p oasis7_viewer --target wasm32-unknown-unknown`
  - `env -u RUSTC_WRAPPER cargo run -p oasis7 --bin oasis7_llm_agent_demo -- llm_bootstrap --ticks 60 --print-llm-io --llm-io-max-chars 1800 --report-json output/llm_bootstrap/mmd3_industrial_closure_2026-02-17_132336/report.json`
  - `env -u RUSTC_WRAPPER cargo run -p oasis7 --bin oasis7_llm_agent_demo -- llm_bootstrap --ticks 80 --print-llm-io --llm-io-max-chars 1200 --report-json output/llm_bootstrap/mmd3_all_recipes_2026-02-17_132728/report.json`
- 遗留事项：
...
- 完成内容：按用户指令执行“所有工厂 + 所有制成品”在线闭环复跑（两轮），并提取 recipe 级成功计数。
  - 运行 #1（基线强化 prompt，100 tick）：
    - 命令：`OASIS7_LLM_SYSTEM_PROMPT='你是工业闭环调度Agent。必须通过单一tool call输出决策，禁止输出文本JSON。你的任务不是“多建厂”，而是“先达成全制成品覆盖”。除非 factory_not_found，否则禁止再次 build_factory。所有 mine_compound 单次必须 <=5000。' OASIS7_LLM_SHORT_TERM_GOAL='100 tick内完成：1) 至少一次 build_factory(factory.assembler.mk1) 成功；2) schedule_recipe 成功覆盖 recipe.assembler.control_chip、recipe.assembler.motor_mk1、recipe.assembler.logistics_drone 各>=1。' OASIS7_LLM_LONG_TERM_GOAL='形成可持续工业链并优先完成全配方覆盖。' env -u RUSTC_WRAPPER cargo run -p oasis7 --bin oasis7_llm_agent_demo -- llm_bootstrap --ticks 100 --print-llm-io --llm-io-max-chars 2000 --report-json output/llm_bootstrap/user_all_factories_all_products_2026-02-17_142739/report.json`
    - 产物：
  ...
  - 运行 #2（顺序约束 prompt，优先 `logistics_drone`，120 tick）：
    - 命令：`OASIS7_LLM_SYSTEM_PROMPT='你是工业闭环调度Agent。必须通过单一tool call输出决策，禁止输出文本JSON。你的任务不是“多建厂”，而是“先达成全制成品覆盖”。除非 factory_not_found，否则禁止再次 build_factory。所有 mine_compound 单次必须 <=5000。' OASIS7_LLM_SHORT_TERM_GOAL='120 tick 内必须完成：A) 至少一次 build_factory(factory.assembler.mk1) 成功；B) schedule_recipe 成功覆盖 recipe.assembler.logistics_drone、recipe.assembler.motor_mk1、recipe.assembler.control_chip 各>=1。硬约束执行顺序：先 logistics_drone，再 motor_mk1，再 control_chip。' OASIS7_LLM_LONG_TERM_GOAL='形成稳定链路并先完成三配方覆盖。' env -u RUSTC_WRAPPER cargo run -p oasis7 --bin oasis7_llm_agent_demo -- llm_bootstrap --ticks 120 --print-llm-io --llm-io-max-chars 1600 --report-json output/llm_bootstrap/user_all_factories_all_products_retry_2026-02-17_143342/report.json`
    - 产物：
...
- 测试：
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 simulator::tests::power --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 simulator::tests::kernel --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 simulator::tests::persist --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 simulator::llm_agent::prompt_assembly --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 simulator::tests::init --features test_tier_required -- --nocapture`
- 遗留事项：
  - 无；本轮范围内任务已完成，进入提交阶段。
...
- 测试：
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 llm_agent_normalizes_schedule_factory_id_from_kind_alias --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 llm_agent_reroutes_duplicate_build_factory_to_schedule_on_known_factory --features test_tier_required -- --nocapture`
- 遗留事项：
...
- 测试：
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 llm_agent_uses_relay_fallback_after_move_distance_exceeded_history --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 llm_agent_segments_move_agent_when_target_distance_exceeds_limit --features test_tier_required -- --nocapture`
- 遗留事项：
...
- 测试：
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 llm_agent_clamps_mine_compound_mass_by_known_location_availability --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 llm_agent_reroutes_mine_compound_from_depleted_location_to_alternative_location --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 llm_agent_reroutes_schedule_recipe_to_mine_when_compound_missing_and_caps_mass --features test_tier_required -- --nocapture`
- 遗留事项：
...
- 完成内容：完成 MMD6.5 回归验证与文档收口。
  ...
  - 在线闭环抽样（MMD6 收口口径）：
    - 命令：`OASIS7_LLM_SYSTEM_PROMPT='你是工业闭环调度Agent。必须通过单一tool call输出决策，禁止输出文本JSON。主目标：在本次窗口内完成全部制成品配方覆盖并保持可执行动作。' OASIS7_LLM_SHORT_TERM_GOAL='120 tick 内完成：1) 至少一次 build_factory(factory.assembler.mk1) 成功；2) schedule_recipe 成功覆盖 recipe.assembler.control_chip、recipe.assembler.motor_mk1、recipe.assembler.logistics_drone 各>=1。约束：observation.recipe_coverage.missing 非空时优先 missing[0]；禁止重复 build 已存在工厂；mine_compound 若提示矿点可用量不足必须立刻下调质量或迁移矿点；move_agent 若上轮超距被拒绝，下一轮必须先走可见中继。' OASIS7_LLM_LONG_TERM_GOAL='形成稳定工业链：harvest_radiation -> mine_compound -> refine_compound -> schedule_recipe，并持续降低 facility_not_found / move_distance_exceeded / 重复耗尽采矿失败。' env -u RUSTC_WRAPPER cargo run -p oasis7 --bin oasis7_llm_agent_demo -- llm_bootstrap --ticks 120 --print-llm-io --llm-io-max-chars 1800 --report-json output/llm_bootstrap/mmd6_opt_all_recipes_2026-02-17_162515/report.json`
    - 产物：
...
- 日期：2026-02-17（MMD8.2）；时刻：2026-02-17 17:32:11 CST；完成内容：完成协议收敛实现（单轮多 tool call 硬拒绝、`schedule_recipe.batches<=0` 自动归一、Responses 请求设置 `parallel_tool_calls=false`）；代码：`crates/oasis7/src/simulator/llm_agent/behavior_loop.rs`、`crates/oasis7/src/simulator/llm_agent/decision_flow.rs`、`crates/oasis7/src/simulator/llm_agent/openai_payload.rs`、`crates/oasis7/src/simulator/llm_agent/tests.rs`、`crates/oasis7/src/simulator/llm_agent/tests_part2.rs`；测试：`env -u RUSTC_WRAPPER cargo test -p oasis7 llm_agent_rejects_multiple_tool_calls_in_single_turn --features test_tier_required -- --nocapture`、`env -u RUSTC_WRAPPER cargo test -p oasis7 llm_parse_schedule_recipe_non_positive_batches_normalizes_to_one --features test_tier_required -- --nocapture`、`env -u RUSTC_WRAPPER cargo test -p oasis7 build_responses_request_payload_includes_tools_and_required_choice --features test_tier_required -- --nocapture`；遗留事项：进入 MMD8.3（电力预检 guardrail）实施。
- 日期：2026-02-17（MMD8.3）；时刻：2026-02-17 17:41:05 CST；完成内容：完成电力前置预算 guardrail（`schedule_recipe` 加 recipe 电力预算与 `batches` 资源双约束，`move_agent` 加移动电力预检并在不足时回切 `harvest_radiation`，`mine/schedule` 触发的 move 回路复用同一电力守卫）；代码：`crates/oasis7/src/simulator/llm_agent.rs`、`crates/oasis7/src/simulator/llm_agent/tests_part2.rs`、`doc/world-simulator/llm-industrial-mining-debug-tools.project.md`；测试：`env -u RUSTC_WRAPPER cargo test -p oasis7 llm_agent_reroutes_schedule_recipe_to_harvest_when_electricity_is_insufficient --features test_tier_required -- --nocapture`、`env -u RUSTC_WRAPPER cargo test -p oasis7 llm_agent_reroutes_move_agent_to_harvest_when_electricity_is_insufficient --features test_tier_required -- --nocapture`、`env -u RUSTC_WRAPPER cargo test -p oasis7 llm_agent_clamps_schedule_recipe_batches_by_available_hardware --features test_tier_required -- --nocapture`；遗留事项：进入 MMD8.4（持续产出默认化 + 采矿冷却）实施。
- 日期：2026-02-17（MMD8.4）；时刻：2026-02-17 17:50:33 CST；完成内容：完成策略收敛实现（TODO-17/20）：`wait/wait_ticks` 在 recipe 全覆盖后自动回切持续产出（优先 `schedule_recipe`，不可执行时沿 guardrail 回切恢复动作），并为耗尽矿点增加 location 级冷却窗口（冷却期跳过并优先替代矿点，过期允许重试）；代码：`crates/oasis7/src/simulator/llm_agent.rs`、`crates/oasis7/src/simulator/llm_agent/behavior_loop.rs`、`crates/oasis7/src/simulator/llm_agent/tests_part2.rs`、`doc/world-simulator/llm-industrial-mining-debug-tools.project.md`；测试：`env -u RUSTC_WRAPPER cargo test -p oasis7 llm_agent_rewrites_wait --features test_tier_required -- --nocapture`、`env -u RUSTC_WRAPPER cargo test -p oasis7 llm_agent_skips_depleted_location_during_cooldown_window --features test_tier_required -- --nocapture`、`env -u RUSTC_WRAPPER cargo test -p oasis7 llm_agent_allows_retry_depleted_location_after_cooldown_expires --features test_tier_required -- --nocapture`、`env -u RUSTC_WRAPPER cargo test -p oasis7 llm_agent --features test_tier_required -- --nocapture`；遗留事项：进入 MMD8.5，执行在线 `llm_bootstrap` 抽样并完成 TODO-17~20 收口文档。
- 日期：2026-02-17（MMD8.5）；时刻：2026-02-17 18:01:27 CST；完成内容：完成 MMD8.5 回归验证与文档收口；required-tier：`env -u RUSTC_WRAPPER cargo test -p oasis7 llm_agent --features test_tier_required -- --nocapture`（120 用例通过）；在线抽样：`env -u RUSTC_WRAPPER cargo run -p oasis7 --bin oasis7_llm_agent_demo -- llm_bootstrap --ticks 120 --print-llm-io --llm-io-max-chars 1800 --report-json output/llm_bootstrap/mmd8_opt_all_factory_all_products_2026-02-17_175323/report.json`，产物：`output/llm_bootstrap/mmd8_opt_all_factory_all_products_2026-02-17_175323/run.log`、`output/llm_bootstrap/mmd8_opt_all_factory_all_products_2026-02-17_175323/report.json`；关键指标：`active_ticks=120`、`action_success=97`、`action_failure=23`、`parse_errors=0`、`llm_errors=0`、`repair_rounds_total=0`；工厂成功：`factory.power.radiation.mk1=1`、`factory.assembler.mk1=1`；recipe 成功：`control_chip=2`、`motor_mk1=1`、`logistics_drone=1`；观测：在线日志出现 `mine_compound cooldown guardrail` 命中，验证冷却窗口生效；`decision_wait=0`/`decision_wait_ticks=0`，未出现提前空转。遗留事项：新增 TODO-21（`insufficient_resource` 仍占 18/23，后续可强化矿点探索优先队列与低收益矿点黑名单策略）。
...
- 按单文件约束拆分 `oasis7_viewer_live` 测试模块：
  - `crates/oasis7/src/bin/oasis7_viewer_live.rs` 保持在 1200 行以内。
  - 测试迁移到 `crates/oasis7/src/bin/oasis7_viewer_live/world_viewer_live_tests.rs`。
...
- `env -u RUSTC_WRAPPER cargo check -p oasis7`
- `env -u RUSTC_WRAPPER cargo test -p oasis7 reward_asset_ --features test_tier_required -- --nocapture`
- `env -u RUSTC_WRAPPER cargo test -p oasis7 --bin oasis7_viewer_live --features test_tier_required -- --nocapture`
...
## 测试
- `env -u RUSTC_WRAPPER cargo check -p oasis7 --bin oasis7_viewer_live`
...
- 在 `oasis7_viewer_live` reward runtime 增加状态文件：`<report_dir>/reward-runtime-state.json`。
- reward runtime 启动时优先恢复 collector 状态，运行中每次采样后原子持久化，避免重启丢失采样窗口与累计状态。

## 测试
- `env -u RUSTC_WRAPPER cargo check -p oasis7 --bin oasis7_viewer_live`
- `env -u RUSTC_WRAPPER cargo test -p oasis7 ledger_snapshot_roundtrip_restores_epoch_and_cumulative_points --features test_tier_required`
- `env -u RUSTC_WRAPPER cargo test -p oasis7 collector_snapshot_roundtrip_restores_state --features test_tier_required`
...
- 在 `oasis7_viewer_live` 的 reward runtime 生产配置中启用该策略，默认要求兑换签名人与兑换节点一致。
- 在 `runtime/tests/reward_asset.rs` 增加拒绝不匹配签名人的单元测试，并同步更新现有治理策略字面量。

## 测试
- `env -u RUSTC_WRAPPER cargo check -p oasis7 --bin oasis7_viewer_live`
- `env -u RUSTC_WRAPPER cargo test -p oasis7 reward_asset_redeem_power_signed --features test_tier_required`
...
## 完成内容
- 在 `oasis7_viewer_live` 的 reward runtime 主循环中移除 settlement 节点的占位身份绑定逻辑（`runtime-node-*`）。
- 结算 mint 路径不再隐式注入节点身份，统一依赖显式身份绑定；未绑定节点将按既有错误路径在 mint 阶段被拒绝并记录错误。

## 测试
- `env -u RUSTC_WRAPPER cargo check -p oasis7 --bin oasis7_viewer_live`
- `env -u RUSTC_WRAPPER cargo test -p oasis7 --bin oasis7_viewer_live --features test_tier_required`
...
## 完成内容
- 完成 `oasis7` 的 `test_tier_required` 回归，覆盖本期 reward runtime 与既有核心路径。
- 回写项目管理文档，标记 PRH1-1 ~ PRH1-5 全部完成，阶段状态收口。

## 测试
- `env -u RUSTC_WRAPPER cargo check -p oasis7 --bin oasis7_viewer_live`
- `env -u RUSTC_WRAPPER cargo test -p oasis7 --features test_tier_required`
...
## 完成内容
- 在 `oasis7_consensus` 增加 `Ed25519SignatureSigner`，支持 `ActionEnvelope/WorldHeadAnnounce` 签名与验签。
...
## 测试
- `env -u RUSTC_WRAPPER cargo check -p oasis7_consensus`
- `env -u RUSTC_WRAPPER cargo test -p oasis7_consensus signature::tests::`
...
## 测试
- `env -u RUSTC_WRAPPER cargo check -p oasis7_consensus`
- `env -u RUSTC_WRAPPER cargo test -p oasis7_consensus sequencer_mainloop::tests::`
...
## 完成内容
- 完成 `oasis7_consensus` 全量单测回归，覆盖签名治理、PoS、membership 等核心路径。
- 完成 `oasis7` `test_tier_required` 全量回归，确认本轮改动未影响主 crate 行为。
...
## 测试
- `env -u RUSTC_WRAPPER cargo check -p oasis7_consensus`
- `env -u RUSTC_WRAPPER cargo test -p oasis7_consensus membership_tests::`
...
## 完成内容
- 完成 `oasis7_consensus` 全量回归，确认 membership ed25519 双栈改造未破坏现有 consensus/membership 能力。
- 完成 `oasis7` `test_tier_required` 全量回归，确认主 crate 链路保持稳定。
...
## 完成内容
- 完成 `oasis7_consensus` 全量回归，确认 membership signer 公钥白名单治理未破坏既有 consensus/membership 路径。
- 完成 `oasis7` `oasis7_viewer_live` 编译检查与 `test_tier_required` 回归，确认主 crate 链路稳定。
...
## 完成内容
- 新增共享模块：`crates/oasis7_consensus/src/ed25519_signer_policy.rs`：
...
## 测试
- `env -u RUSTC_WRAPPER cargo test -p oasis7_consensus ed25519_signer_policy::tests::`
- `env -u RUSTC_WRAPPER cargo test -p oasis7_consensus signature::tests::`
- `env -u RUSTC_WRAPPER cargo check -p oasis7_consensus`
- `env -u RUSTC_WRAPPER cargo test -p oasis7_consensus`
- `env -u RUSTC_WRAPPER cargo check -p oasis7 --bin oasis7_viewer_live`
...
## 完成内容
- 在 `oasis7_distfs` 新增 `manifest.rs` 模块，补齐文件索引 manifest 能力：
...
## 完成内容
- 执行 `oasis7_distfs` 全量回归测试，覆盖 DistFS 当前 19 条单元测试（含 CAS 条件写删、索引审计/孤儿回收、manifest 导入导出与复制路径）。
...
## 完成内容
- 新增 `crates/oasis7_distfs/src/challenge.rs`，落地存储挑战基础模型：
...
## 完成内容
- 完成 `oasis7_distfs` 全量回归测试，覆盖 DistFS 当前 28 条单元测试（含挑战下发应答、回执校验、语义投影、节点统计聚合、索引审计与 manifest 等能力）。
...
## 完成内容
- 在 `oasis7_distfs/challenge.rs` 增加挑战探测配置与报告：
...
## 遗留事项
- 继续执行 DPH3-3：接线 `oasis7_viewer_live` reward runtime 消费 probe 结果。
...
## 完成内容
- 在 `oasis7_viewer_live` reward runtime 主循环中接入 DistFS 挑战探测：
...
## 遗留事项
- 继续执行 DPH3-4：执行 DistFS + oasis7_viewer_live 回归并收口文档。
...
## 完成内容
- 新增 `crates/oasis7_distfs/src/challenge_scheduler.rs`：
...
## 完成内容
- 在 `oasis7_viewer_live` reward runtime 配置新增 DistFS probe 状态文件路径：
...
## 完成内容
- 新增 `oasis7_viewer_live` 子模块：`crates/oasis7/src/bin/oasis7_viewer_live/distfs_probe_runtime.rs`。
  - 引入 `DistfsProbeRuntimeConfig`（默认值+序列化）。
  - 下沉 DistFS probe 相关函数：状态加载/持久化、挑战报告采集。
- `oasis7_viewer_live` 增加 DistFS probe CLI 参数：
...
- 补充并更新单测：
  ...
  - 既有 probe 相关测试保持通过。
- 控制单文件行数：`oasis7_viewer_live.rs` 保持在 1200 行上限内。
...
## 完成内容
- 执行 DistFS 与 oasis7_viewer_live 相关回归，确认自适应策略、状态扩展与兼容行为稳定。
- 同步修复 `oasis7_viewer_live` 测试构造体字面量以匹配新增状态字段。
...
## 遗留事项
- 继续执行 DPH7-3：将 adaptive policy 参数接入 `oasis7_viewer_live` CLI 与 probe runtime。
...
- 将 DistFS probe/adaptive 相关 CLI 解析从 `oasis7_viewer_live.rs` 抽离到 `distfs_probe_runtime.rs`，降低主文件复杂度并保持主文件行数约束。
- 新增 adaptive 参数：
...
- 更新 `oasis7_viewer_live` help 文本，公开新参数默认值和失败原因语义。
...
## 完成内容
- 执行 DistFS 与 oasis7_viewer_live 回归，确认 reason-aware multiplier CLI 治理化与测试增强稳定。
...
## 完成内容
- 更新 `oasis7_viewer_live` DistFS probe state roundtrip 测试，覆盖新增退避观测字段。
...
## 完成内容
- 执行 DistFS 与 oasis7_viewer_live 回归，确认 backoff 可观测状态扩展与 runtime 兼容测试在当前实现下稳定。
...
## 测试
- `env -u RUSTC_WRAPPER cargo check -p oasis7`
- `env -u RUSTC_WRAPPER cargo test -p oasis7 reward_asset_settlement_action_ --features test_tier_required -- --nocapture`
...
## 遗留事项
- 继续执行 NSTX-4：切换 `oasis7_viewer_live` reward runtime 发起主路径原生结算交易。
...
## 完成内容
- `oasis7_viewer_live` 奖励循环从“直接调用状态写入 API”切换为“构造签名 mint 记录 + 提交 `ApplyNodePointsSettlementSigned` action + `step()` 执行共识主路径”。
- 新增 `build_reward_settlement_mint_records` 预览构建函数并补测试，验证构建记录阶段不会污染主 world 状态。
- 为满足单文件行数约束，将 reward settlement 运行时辅助逻辑下沉到 `oasis7_viewer_live/reward_runtime_settlement.rs` 子模块；将新增 settlement action 测试拆分到 `reward_asset_settlement_action.rs`。

## 测试
- `env -u RUSTC_WRAPPER cargo test -p oasis7 --bin oasis7_viewer_live build_reward_settlement_mint_records_uses_preview_world_without_mutation --features test_tier_required -- --nocapture`
- `env -u RUSTC_WRAPPER cargo test -p oasis7 --bin oasis7_viewer_live --features test_tier_required -- --nocapture`
...
- 校验本次涉及 Rust 文件行数，`oasis7_viewer_live.rs` 与 `reward_asset.rs` 均已控制在 1200 行以内。
...
- 完成内容：清理 `doc/world-simulator` 过时设计文档，并归档到 `doc/world-simulator/archive/`。
  ...
  - 对照依据：
    - `crates/oasis7_viewer/src/egui_right_panel.rs`（当前无左侧 Chat History Panel，右侧双面板为主信息 + 最右 Chat）
    - `crates/oasis7_viewer/src/egui_right_panel_chat.rs`（Prompt 默认值已走预填充 + hint + patch 语义，不再有独立默认值展示行）
  - 回写项目管理文档：`doc/world-simulator.project.md`（维护章节新增“过时文档归档”任务，状态最近更新回填）
- 测试：
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_viewer show_chat_panel_requires_expanded_top_and_visibility_enabled -- --nocapture`
  - 结果：通过（1 passed）。
  - 备注：尝试 `--features test_tier_required` 发现 `oasis7_viewer` 无该 feature，已按实际包能力改为无 feature 运行。
- 遗留事项：
...
- 完成内容：修复仓库 Rust 格式校验失败（fmt），统一格式化受影响代码文件。
  ...
  - 变更文件：
    ...
    - `crates/oasis7/src/bin/oasis7_viewer_live/distfs_probe_runtime.rs`
    - `crates/oasis7/src/bin/oasis7_viewer_live/world_viewer_live_tests.rs`
...
- 遗留事项：
  - 无新增功能遗留；`oasis7_viewer_live` 仍有既有 `dead_code` warning（未在本任务处理范围）。
...
- 完成内容：完成 NBS-3，在 wasm 构建入口固化 nightly build-std 环境准备。
  ...
  - 新增默认参数：`OASIS7_WASM_TOOLCHAIN=nightly-2025-12-11`、`OASIS7_WASM_BUILD_STD=1`。
  - 新增逻辑：构建前执行 rustup toolchain/target/rust-src 准备，并导出 `OASIS7_WASM_BUILD_STD*` 给 wasm build suite。
- 测试：
  - `bash -n scripts/build-wasm-module.sh`
  - `./scripts/build-wasm-module.sh --module-id m1.rule.move --manifest-path crates/oasis7_builtin_wasm/Cargo.toml --out-dir .tmp/wasm-build-suite --profile release --dry-run`
- 遗留事项：
...
- 完成内容：完成 NBS-5，更新 CI workflow 固化 nightly build-std 构建环境。
  ...
  - 新增全局环境：`OASIS7_WASM_TOOLCHAIN=nightly-2025-12-11`、`OASIS7_WASM_BUILD_STD=1`、`OASIS7_WASM_BUILD_STD_COMPONENTS`、`OASIS7_WASM_BUILD_STD_FEATURES`。
...
- 完成内容：完成 NBS-6/NBS-7 并收敛 nightly build-std 参数兼容性。
  - 兼容性修正：移除默认 `OASIS7_WASM_BUILD_STD_FEATURES=panic_immediate_abort`（nightly 已将其升级为 panic strategy，继续使用会构建失败）。
  - 变更文件：
    - `scripts/build-wasm-module.sh`（默认 `OASIS7_WASM_BUILD_STD_FEATURES` 为空）
...
## 完成内容
- 新增 `oasis7_viewer_live` 子模块：`crates/oasis7/src/bin/oasis7_viewer_live/execution_bridge.rs`。
...
## 测试
- `env -u RUSTC_WRAPPER cargo test -p oasis7 --bin oasis7_viewer_live --features test_tier_required execution_bridge -- --nocapture`
- `env -u RUSTC_WRAPPER cargo check -p oasis7 --bin oasis7_viewer_live`
...
- 完成内容：完成 ERCB-2（奖励结算网络触发）实现收口并接入 `oasis7_viewer_live` 主循环。
  - 代码：
    - `crates/oasis7/src/bin/oasis7_viewer_live.rs`
...
- 完成内容：按仓库约束拆分 `oasis7_viewer_live`，确保单文件不超过 1200 行。
  - 新增模块：
    - `crates/oasis7/src/bin/oasis7_viewer_live/cli.rs`
    - `crates/oasis7/src/bin/oasis7_viewer_live/node_keypair_config.rs`
...
- 完成内容：完成 ERCB-5 收口。
  ...
  - 复核本次涉及 Rust 文件行数：`oasis7_viewer_live.rs=1044`、`reward_runtime_network.rs=465`、`cli.rs=461`、`node_keypair_config.rs=142`，均满足单文件 <1200 约束。
- 测试：
  - 沿用 ERCB-4 回归结果（`oasis7_viewer_live` required-tier 46/46 通过）。
- 遗留事项：
  - 无。
...
- 日期：2026-02-17
- 时刻：2026-02-17 19:06:17 CST
- 完成内容：完成 PRG-2（`oasis7_node` 链式 `block_hash` 改造）。
  - 代码：
    - `crates/oasis7_node/src/lib.rs`
    - `crates/oasis7_node/src/pos_state_store.rs`
    - `crates/oasis7_node/src/tests.rs`
  - 变更：
    - 提案 `block_hash` 从字符串占位升级为 `blake3(cbor(payload))`，并纳入 `parent_block_hash` 形成链式哈希。
    - `PosNodeEngine` 新增 `last_committed_block_hash`，在提交时更新。
    - `PosNodeStateSnapshot` 新增 `last_committed_block_hash`（`serde(default)` 兼容旧快照），恢复路径补 legacy 回退值。
    - 新增单测 `pos_engine_generates_chain_hashed_block_ids`，锁定 64 位十六进制哈希语义。
- 测试：
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_node pos_engine_generates_chain_hashed_block_ids -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_node runtime_pos_state_persists_across_restart -- --nocapture`
- 遗留事项：
  - 继续执行 PRG-3（结算 envelope 签名）与 PRG-4（回归与收口）。

- 日期：2026-02-17
- 时刻：2026-02-17 19:06:55 CST
- 完成内容：完成 PRG-3（奖励结算 `RewardSettlementEnvelope` 传输签名与验签）。
  - 代码：
    - `crates/oasis7/src/bin/oasis7_viewer_live/reward_runtime_network.rs`
    - `crates/oasis7/src/bin/oasis7_viewer_live.rs`
  - 变更：
    - `RewardSettlementEnvelope` 新增 `signer_public_key_hex` 与 `signature` 字段。
    - 新增 `sign_reward_settlement_envelope` / `verify_reward_settlement_envelope`，签名前缀 `rewardsett:v1:`。
    - 结算包发布路径改为先签名后发布；消费路径在提交 `ApplyNodePointsSettlementSigned` 前强制验签。
    - 新增 signer 身份绑定校验：`signer_node_id` 的绑定公钥必须与 envelope 公钥一致。
    - 补充结算 envelope 签名正向与篡改拒绝单测。
- 测试：
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 settlement_envelope_signature_verifies --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 settlement_envelope_signature_rejects_tampered_payload --features test_tier_required -- --nocapture`
- 遗留事项：
  - 继续执行 PRG-4：汇总回归结果并完成文档收口。

- 日期：2026-02-17
- 时刻：2026-02-17 19:07:38 CST
- 完成内容：完成 PRG-4（回归与文档收口）。
  - 文档：`doc/p2p/production-grade-blockchain-p2pfs-roadmap.project.md` 状态回写为 PRG-1 ~ PRG-4 全完成。
  - 回归结果：
    - `oasis7_node` 全量单测：21/21 通过。
    - `oasis7` 的 `reward_runtime_network` 模块测试：13/13 通过。
- 测试：
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 reward_runtime_network --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_node -- --nocapture`
- 遗留事项：
  - 路线图后续里程碑（PRG-M4 ~ PRG-M6）待进入下一轮实现：共识内生执行、跨节点挑战证明网络化、交易化支付市场。

- 日期：2026-02-17
- 时刻：2026-02-17 19:11:27 CST
- 完成内容：完成 PRG-B1（Phase B 共识内生执行）文档任务。
  - 新增设计文档：`doc/p2p/production-grade-blockchain-p2pfs-phaseb-consensus-execution.md`
  - 新增项目管理文档：`doc/p2p/production-grade-blockchain-p2pfs-phaseb-consensus-execution.project.md`
  - 明确本轮范围：NodeRuntime 执行 hook、快照/持久化扩展、oasis7_viewer_live 执行驱动接线与 fallback 策略。
- 测试：
  - 文档任务，无代码测试。
- 遗留事项：
  - 继续执行 PRG-B2 ~ PRG-B4 代码实现与回归。

- 日期：2026-02-17
- 时刻：2026-02-17 19:20:14 CST
- 完成内容：完成 PRG-B2（`oasis7_node` 共识内生执行 hook + 快照持久化扩展）。
  - 代码：
    - `crates/oasis7_node/src/lib.rs`
    - `crates/oasis7_node/src/pos_state_store.rs`
    - `crates/oasis7_node/src/tests.rs`
    - `crates/oasis7_node/src/error.rs`
    - `crates/oasis7_node/src/execution_hook.rs`
    - `crates/oasis7_node/src/types.rs`
  - 变更：
    - 新增 `NodeExecutionHook`、`NodeExecutionCommitContext`、`NodeExecutionCommitResult`，在提交决议后触发内生执行。
    - `NodeConsensusSnapshot` 增加执行高度/执行块哈希/执行状态根字段。
    - `PosNodeStateSnapshot` 增加 execution 字段并使用 `serde(default)` 兼容旧快照。
    - 节点运行时支持 `with_execution_hook(...)` 注入执行驱动。
    - 拆分 `lib.rs` 到 `types.rs`/`error.rs`，将单文件降至 1200 行以内。
    - 新增回归：执行 hook 触发与重启后 execution 高度延续。
- 测试：
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_node -- --nocapture`
- 遗留事项：
  - 继续执行 PRG-B3（oasis7_viewer_live 接入节点内生执行驱动）。

- 日期：2026-02-17
- 时刻：2026-02-17 19:20:42 CST
- 完成内容：完成 PRG-B3（`oasis7_viewer_live` 接入节点内生执行驱动，保留 bridge fallback）。
  - 代码：
    - `crates/oasis7/src/bin/oasis7_viewer_live/execution_bridge.rs`
    - `crates/oasis7/src/bin/oasis7_viewer_live.rs`
  - 变更：
    - 新增 `NodeRuntimeExecutionDriver`（实现 `NodeExecutionHook`），将执行步进/CAS 持久化/record 落盘下沉到节点提交回调。
    - `start_live_node` 启动时注入 execution driver，默认开启节点内生执行。
    - reward runtime 在检测到节点已提供 execution 结果时跳过旧桥接执行，避免双执行；否则自动 fallback 到历史 bridge。
    - 报表 `node_snapshot.consensus` 新增 execution 字段输出。
    - 新增 `execution_bridge` 单测：`node_runtime_execution_driver_persists_chain_records`。
- 测试：
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 --bin oasis7_viewer_live execution_bridge::tests::node_runtime_execution_driver_persists_chain_records --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 --bin oasis7_viewer_live --features test_tier_required -- --nocapture`
- 遗留事项：
  - 继续执行 PRG-B4：阶段回归汇总与项目文档状态收口。

- 日期：2026-02-17
- 时刻：2026-02-17 19:21:25 CST
- 完成内容：完成 PRG-B4（Phase B 回归与文档收口）。
  - 文档：`doc/p2p/production-grade-blockchain-p2pfs-phaseb-consensus-execution.project.md` 已回写为 PRG-B1 ~ PRG-B4 全完成。
  - 复核文件行数约束：
    - `crates/oasis7_node/src/lib.rs = 1079`（<1200）
    - `crates/oasis7/src/bin/oasis7_viewer_live.rs = 1112`（<1200）
- 测试：
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_node -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 --bin oasis7_viewer_live --features test_tier_required -- --nocapture`
- 遗留事项：
  - 路线图后续里程碑待进入下一轮：PRG-M5（DistFS 跨节点挑战证明网络化）与 PRG-M6（交易化需求侧支付市场）。

- 日期：2026-02-17
- 时刻：2026-02-17 19:34:18 CST
- 完成内容：完成 PRG-C1（PRG-M5 阶段文档）并删除 PRG-M6 设计方向。
  - 新增：
    - `doc/p2p/production-grade-blockchain-p2pfs-phasec-distfs-proof-network.md`
    - `doc/p2p/production-grade-blockchain-p2pfs-phasec-distfs-proof-network.project.md`
  - 更新：
    - `doc/p2p/production-grade-blockchain-p2pfs-roadmap.md`：移除 PRG-M6（交易化支付市场），明确系统预算池主导。
    - `doc/p2p/production-grade-blockchain-p2pfs-phaseb-consensus-execution.md`：同步口径为预算池主导。
- 测试：
  - 文档任务，无代码测试。
- 遗留事项：
  - 继续执行 PRG-C2 ~ PRG-C4（跨节点 DistFS challenge/proof 网络化实现与回归）。

- 日期：2026-02-17
- 时刻：2026-02-17 19:44:49 CST
- 完成内容：完成 PRG-C2（DistFS challenge/proof 消息协议与签名验签）。
  - 代码：
    - `crates/oasis7/src/bin/oasis7_viewer_live/distfs_challenge_network.rs`
  - 变更：
    - 新增 request/proof topic、消息结构、编解码、签名与验签。
    - 新增消息 ID 计算与失败原因归类。
    - 新增模块单测（topic/签名/跨节点 roundtrip）。
- 测试：
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 --bin oasis7_viewer_live distfs_challenge_network::tests::distfs_network_driver_roundtrip_challenge_and_proof --features test_tier_required -- --nocapture`
- 遗留事项：
  - 继续执行 PRG-C3：reward runtime 接线（网络挑战优先 + fallback）。

- 日期：2026-02-17
- 时刻：2026-02-17 19:45:16 CST
- 完成内容：完成 PRG-C3（reward runtime 接线跨节点 DistFS challenge/proof）。
  - 代码：
    - `crates/oasis7/src/bin/oasis7_viewer_live.rs`
    - `crates/oasis7/src/bin/oasis7_viewer_live/observation_trace_runtime.rs`
  - 变更：
    - reward runtime 新增 `DistfsChallengeNetworkDriver` 接线，storage 角色优先使用跨节点 challenge/proof。
    - 在无可用存储目标时保留本地 probe fallback。
    - 将网络证明语义映射到 `storage_challenge_proof_hint`，并新增报表字段 `distfs_network_challenge`。
    - 观察轨迹应用逻辑拆分到 `observation_trace_runtime.rs`，保持 `oasis7_viewer_live.rs` <1200 行。
- 测试：
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 --bin oasis7_viewer_live --features test_tier_required -- --nocapture`
- 遗留事项：
  - 继续执行 PRG-C4：项目状态回写与全量回归收口。

- 日期：2026-02-17
- 时刻：2026-02-17 19:46:01 CST
- 完成内容：完成 PRG-C4（Phase C 回归与文档收口）。
  - 文档：`doc/p2p/production-grade-blockchain-p2pfs-phasec-distfs-proof-network.project.md` 已回写为 PRG-C1 ~ PRG-C4 全完成。
  - 约束复核：
    - `crates/oasis7/src/bin/oasis7_viewer_live.rs = 1161`（<1200）
    - `crates/oasis7/src/bin/oasis7_viewer_live/distfs_challenge_network.rs = 1086`（<1200）
    - `crates/oasis7_node/src/lib.rs = 1079`（<1200）
- 测试：
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 --bin oasis7_viewer_live --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_node -- --nocapture`
- 遗留事项：
  - 无；下一阶段可按预算池主导口径继续做治理与运维生产化细化。

- 日期：2026-02-17
- 时刻：2026-02-17 22:09:15 CST
- 完成内容：完成 MIP-2 代码迁移（builtin wasm 模块独立工程化 + 构建链路适配）。
  - `crates/oasis7_builtin_wasm` 改为共享逻辑库（`rlib`），新增 `builtin_alloc`、`reduce_for_module`、`call_for_module` 供模块 wrapper 复用。
  - 新增 23 个独立模块 crate：`crates/oasis7_builtin_wasm_modules/*`（每个 `module_id` 对应一个独立 `Cargo.toml` + 导出入口）。
  - 新增映射清单：`crates/oasis7/src/runtime/world/artifacts/builtin_module_manifest_map.txt`。
  - 改造 `scripts/build-builtin-wasm-modules.sh`：按 `module_id -> manifest_path` 映射构建，不再复用单一 manifest。
- 测试：
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_builtin_wasm`
  - `env -u NO_COLOR ./scripts/build-builtin-wasm-modules.sh --module-id m1.rule.move --module-id m1.rule.visibility --out-dir .tmp/mip-modules-test --profile release`
- 遗留事项：
  - 执行 MIP-3：全量 sync/check（m1+m4）、更新 hash 清单并完成收口回归。

- 日期：2026-02-17
- 时刻：2026-02-17 22:21:54 CST
- 完成内容：完成 builtin wasm 独立工程迁移收口（MIP-3）。
  - 执行 `scripts/sync-m1-builtin-wasm-artifacts.sh --check` 与 `scripts/sync-m4-builtin-wasm-artifacts.sh --check`，确认 `m1/m4` 哈希清单与独立模块构建产物一致，并完成 DistFS blob 回填。
  - 更新产物清单：`crates/oasis7/src/runtime/world/artifacts/m1_builtin_modules.sha256`、`crates/oasis7/src/runtime/world/artifacts/m4_builtin_modules.sha256`。
  - 生成并纳入模块独立工程 `Cargo.lock`（按用户确认允许提交 lock 文件）。
- 完成内容：按用户要求下线旧 builtin wasm 流程 CI gate。
  - 代码：`scripts/ci-tests.sh`、`scripts/pre-commit.sh`
  - 变更：从默认 required gate 中移除 `sync-m1/sync-m4` 旧流程校验，保留手工同步脚本能力。
- 完成内容：回写项目管理文档并标记 MIP-3 完成。
  - 文档：`doc/world-runtime/builtin-wasm-independent-module-crates.project.md`
- 测试：
  - `env -u NO_COLOR ./scripts/sync-m1-builtin-wasm-artifacts.sh --check`
  - `env -u NO_COLOR ./scripts/sync-m4-builtin-wasm-artifacts.sh --check`
  - `env -u RUSTC_WRAPPER cargo check -p oasis7 --features test_tier_required`
  - `./scripts/ci-tests.sh required`（失败：`cargo fmt --check` 命中仓库内既有未格式化文件，与本任务改动无关）
- 遗留事项：
  - TODO：后续需要单独处理仓库既有 `cargo fmt --check` 漂移文件，恢复 required gate 全绿。

- 日期：2026-02-17
- 时刻：2026-02-17 22:25:18 CST
- 完成内容：完成 LIFESDK-1（生命周期 trait + wasm sdk）文档任务。
  - 新增设计文档：`doc/world-runtime/builtin-wasm-lifecycle-sdk.md`
  - 新增项目管理文档：`doc/world-runtime/builtin-wasm-lifecycle-sdk.project.md`
  - 设计对齐 `third_party/agent-os` wasm sdk 思路，明确在 oasis7 采用 `trait + dispatch + 导出宏` 的 wasm 生命周期约束。
- 测试：
  - 文档任务，无代码测试。
- 遗留事项：
  - 继续执行 LIFESDK-2：创建 `oasis7_wasm_sdk` 并迁移 builtin wasm 模块接入生命周期 trait。

- 日期：2026-02-17
- 时刻：2026-02-17 22:30:53 CST
- 完成内容：完成 LIFESDK-2（wasm 生命周期 trait + SDK 接入）代码迁移。
  - 新增 crate：`crates/oasis7_wasm_sdk`
    - 定义 `WasmModuleLifecycle` trait（`on_init/on_teardown/on_reduce/on_call`）
    - 定义 `LifecycleStage` 与 `dispatch_reduce/dispatch_call`
    - 提供统一导出宏 `export_wasm_module!`（导出 `alloc/reduce/call`）
  - workspace 接线：`Cargo.toml` 增加 `crates/oasis7_wasm_sdk`。
  - 批量迁移 23 个 builtin wasm 独立模块：
    - `crates/oasis7_builtin_wasm_modules/*/src/lib.rs`
    - `crates/oasis7_builtin_wasm_modules/*/Cargo.toml`
    - 从手写三导出函数改为实现生命周期 trait + 使用 SDK 导出宏。
  - 构建与产物同步：执行 m1/m4 sync + check，确认 hash manifest 与 DistFS 均一致。
  - 依赖锁更新：根 `Cargo.lock` 与模块级 `Cargo.lock` 同步引入 `oasis7_wasm_sdk`。
- 测试：
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_wasm_sdk`
  - `env -u RUSTC_WRAPPER cargo check -p oasis7_builtin_wasm`
  - `env -u NO_COLOR ./scripts/sync-m1-builtin-wasm-artifacts.sh`
  - `env -u NO_COLOR ./scripts/sync-m4-builtin-wasm-artifacts.sh`
  - `env -u NO_COLOR ./scripts/sync-m1-builtin-wasm-artifacts.sh --check`
  - `env -u NO_COLOR ./scripts/sync-m4-builtin-wasm-artifacts.sh --check`
  - `env -u RUSTC_WRAPPER cargo check -p oasis7 --features test_tier_required`
- 遗留事项：
  - 无新增遗留；后续如需要可在 runtime 侧继续扩展生命周期阶段（如 load/unload）并映射到现有 ABI。

- 日期：2026-02-17
- 时刻：2026-02-17 22:35:48 CST
- 完成内容：修复 CI `fmt` 失败并恢复格式门禁通过。
  - 根因：`env -u RUSTC_WRAPPER cargo fmt --all -- --check` 命中仓库内既有格式漂移（`oasis7_viewer_live*`、`oasis7_node*` 相关文件）。
  - 修复：执行 `env -u RUSTC_WRAPPER cargo fmt --all`，统一格式化以下文件：
    - `crates/oasis7/src/bin/oasis7_viewer_live.rs`
    - `crates/oasis7/src/bin/oasis7_viewer_live/distfs_challenge_network.rs`
    - `crates/oasis7/src/bin/oasis7_viewer_live/execution_bridge.rs`
    - `crates/oasis7_node/src/lib.rs`
    - `crates/oasis7_node/src/tests.rs`
  - 文档回写：`doc/scripts/pre-commit.project.md`。
- 测试：
  - `env -u RUSTC_WRAPPER cargo fmt --all -- --check`
- 遗留事项：
  - 无。
...
- 日期：2026-02-17
- 时刻：2026-02-17 22:41:46 CST
- 完成内容：清理 `required` 阶段编译 warning（oasis7_viewer_live / distfs_challenge_network / distfs_probe_runtime）。
  - 代码：
    - `crates/oasis7/src/bin/oasis7_viewer_live.rs`
    - `crates/oasis7/src/bin/oasis7_viewer_live/distfs_challenge_network.rs`
    - `crates/oasis7/src/bin/oasis7_viewer_live/distfs_probe_runtime.rs`
  - 变更：
    - 将仅测试使用的 `WorldScenario`、`PosValidator`、`HashMap`、`Mutex` 导入改为 `#[cfg(test)]`。
    - 将仅测试调用的 `collect_distfs_challenge_report` 改为 `#[cfg(test)]`，避免非测试构建 dead_code warning。
  - 文档回写：`doc/scripts/pre-commit.project.md`。
- 测试：
...
- 日期：2026-02-17
- 时刻：2026-02-17 22:44:12 CST
- 完成内容：完成 RCR-1 文档任务（删除旧 `oasis7_builtin_wasm` 并替换为新流程）。
  - 设计文档：`doc/world-runtime/builtin-wasm-runtime-core-replacement.md`
  - 项目管理文档：`doc/world-runtime/builtin-wasm-runtime-core-replacement.project.md`
  - 明确迁移策略：保留模块独立工程与 lifecycle SDK，移除旧 crate 名称/路径，由新 runtime core crate 承接共享逻辑。
- 测试：
  - 文档任务，无代码测试。
- 遗留事项：
  - 执行 RCR-2：代码迁移、sync/check 与 required-tier 回归。

- 日期：2026-02-17
- 时刻：2026-02-17 22:52:32 CST
- 完成内容：完成 RCR-2 代码迁移（删除旧 `oasis7_builtin_wasm`，切换到新流程）。
  - 目录迁移：`crates/oasis7_builtin_wasm` -> `crates/oasis7_builtin_wasm_runtime`。
  - workspace / 依赖切换：
    - 根 `Cargo.toml` 成员改为 `crates/oasis7_builtin_wasm_runtime`。
    - `crates/oasis7/Cargo.toml` 依赖改为 `oasis7_builtin_wasm_runtime`。
    - 23 个模块 crate (`crates/oasis7_builtin_wasm_modules/*/Cargo.toml`) 依赖改为 `oasis7_builtin_wasm_runtime`。
  - 生命周期模块调用切换：
    - 23 个模块 wrapper (`crates/oasis7_builtin_wasm_modules/*/src/lib.rs`) 从 `oasis7_builtin_wasm::` 全量替换到 `oasis7_builtin_wasm_runtime::`。
  - runtime 常量与回退构建链路切换：
    - `crates/oasis7/src/runtime/mod.rs` re-export 源改为新 crate。
    - `crates/oasis7/src/runtime/world/bootstrap_power.rs`
    - `crates/oasis7/src/runtime/world/bootstrap_economy.rs`
      - bootstrap build manifest 的 crate 标识改为 `oasis7_builtin_wasm_runtime`。
    - `crates/oasis7/src/runtime/builtin_wasm_materializer.rs`
      - fallback 编译从旧单 crate manifest 路径，改为调用 `scripts/build-builtin-wasm-modules.sh`（按 module_id 构建）。
  - 文档状态回写：
    - `doc/world-runtime/builtin-wasm-runtime-core-replacement.project.md`
    - `doc/world-runtime/builtin-wasm-lifecycle-sdk.project.md`
- 测试：
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_builtin_wasm_runtime`
  - `env -u NO_COLOR ./scripts/sync-m1-builtin-wasm-artifacts.sh`
  - `env -u NO_COLOR ./scripts/sync-m4-builtin-wasm-artifacts.sh`
  - `env -u NO_COLOR ./scripts/sync-m1-builtin-wasm-artifacts.sh --check`
  - `env -u NO_COLOR ./scripts/sync-m4-builtin-wasm-artifacts.sh --check`
...
- 日期：2026-02-17
- 时刻：2026-02-17 22:59:50 CST
- 完成内容：完成 NSDK-1（`oasis7_wasm_sdk` no_std 优先化）文档任务。
  - 新增设计文档：`doc/world-runtime/wasm-sdk-no-std.md`
  - 新增项目管理文档：`doc/world-runtime/wasm-sdk-no-std.project.md`
  - 明确迁移口径：保持生命周期 trait 与 ABI 不变，仅做 SDK no_std 化与构建回归。
- 测试：
  - 文档任务，无代码测试。
- 遗留事项：
  - 执行 NSDK-2：完成 `oasis7_wasm_sdk` no_std 代码迁移与回归。

- 日期：2026-02-17
- 时刻：2026-02-17 23:00:47 CST
- 完成内容：完成 NSDK-2（`oasis7_wasm_sdk` no_std 优先迁移）代码任务。
  - `crates/oasis7_wasm_sdk/src/lib.rs`：启用 `#![cfg_attr(not(any(test, feature = "std")), no_std)]`，引入 `alloc`/`core` 替代 `std`。
  - `crates/oasis7_wasm_sdk/Cargo.toml`：新增 feature 声明（`default = []`, `std = []`），避免 `cfg(feature = "std")` 校验告警。
  - `doc/world-runtime/wasm-sdk-no-std.project.md`：回写状态，标记 NSDK-2 完成。
- 测试：
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_wasm_sdk`
  - `env -u RUSTC_WRAPPER cargo check -p oasis7_wasm_sdk --target wasm32-unknown-unknown`
  - `env -u RUSTC_WRAPPER cargo check -p oasis7 --features test_tier_required`
- 遗留事项：
  - 无新增遗留；后续如需进一步对齐 `agent-os`，可在 no_std 基础上引入 typed context/effect API。

- 日期：2026-02-17
- 时刻：2026-02-17 23:06:42 CST
- 完成内容：完成 MBM-1（builtin wasm 业务逻辑下沉到模块工程）文档任务。
  - 新增设计文档：`doc/world-runtime/builtin-wasm-module-business-logic-migration.md`
  - 新增项目管理文档：`doc/world-runtime/builtin-wasm-module-business-logic-migration.project.md`
  - 明确迁移口径：runtime 仅保留常量/通用能力，23 模块 crate 承载自身业务逻辑。
- 测试：
  - 文档任务，无代码测试。
- 遗留事项：
  - 执行 MBM-2：代码迁移、m1/m4 同步校验、required-tier 回归。

- 日期：2026-02-17
- 时刻：2026-02-17 23:32:29 CST
- 完成内容：完成 MBM-2（builtin wasm 业务逻辑下沉到模块工程）代码迁移与构建链路适配。
  - 23 个 `crates/oasis7_builtin_wasm_modules/*` 模块 crate 已内聚自身业务逻辑，不再通过 runtime 分发函数执行。
  - 模块依赖面收敛：移除 `oasis7_builtin_wasm_runtime` 依赖，统一依赖 `oasis7_wasm_sdk` + 序列化库。
  - runtime 常量迁移：`oasis7` runtime 常量改为从 `crates/oasis7_wasm_store/src/lib.rs` 导出。
  - 工程收敛：workspace 与 `oasis7` crate 依赖图中移除 `oasis7_builtin_wasm_runtime`，并删除其源码文件。
  - 文档回填：更新 `doc/world-runtime/builtin-wasm-module-business-logic-migration.md` 与 `doc/world-runtime/builtin-wasm-module-business-logic-migration.project.md`，标记 MBM 任务完成。
- 测试：
  - `env -u NO_COLOR ./scripts/sync-m1-builtin-wasm-artifacts.sh`
  - `env -u NO_COLOR ./scripts/sync-m4-builtin-wasm-artifacts.sh`
  - `env -u NO_COLOR ./scripts/sync-m1-builtin-wasm-artifacts.sh --check`
  ...
  - `env -u RUSTC_WRAPPER cargo fmt --all`
- 遗留事项：
  - 无新增遗留；后续如需继续收敛可评估清理文档中的历史 `oasis7_builtin_wasm_runtime` 描述。

- 日期：2026-02-17
- 时刻：2026-02-17 23:44:45 CST
- 完成内容：完成 WIRESDK-1（WASM SDK wire 类型收敛）文档任务。
  - 新增设计文档：`doc/world-runtime/wasm-sdk-wire-types-dedup.md`
  - 新增项目管理文档：`doc/world-runtime/wasm-sdk-wire-types-dedup.project.md`
  - 明确迁移口径：将 `ModuleCallInput/ModuleContext/ModuleOutput` 等同构协议结构收敛到 SDK，模块只保留业务结构。
- 测试：
  - 文档任务，无代码测试。
- 遗留事项：
  - 执行 WIRESDK-2：SDK wire 抽象代码落地与 23 模块迁移。

- 日期：2026-02-17
- 时刻：2026-02-17 23:52:34 CST
- 完成内容：完成 WIRESDK-2（WASM SDK wire 类型收敛）代码迁移。
  - `crates/oasis7_wasm_sdk/Cargo.toml`：新增 `wire` feature（含 `serde/serde_cbor/serde_json` 可选依赖）。
  - `crates/oasis7_wasm_sdk/src/lib.rs`：新增 `wire` 模块，统一提供 `ModuleCallInput/ModuleContext/ModuleEffectIntent/ModuleEmit/ModuleOutput` 与 `decode_input/decode_action/empty_output/encode_output`。
  - `crates/oasis7_builtin_wasm_modules/*`：23 个模块 crate 删除本地重复 wire 结构与 helper，改为复用 SDK `wire`；模块 `Cargo.toml` 启用 `oasis7_wasm_sdk` 的 `wire` feature。
  - `doc/world-runtime/wasm-sdk-wire-types-dedup.project.md`：回填状态并标记 WIRESDK-2 完成。
- 测试：
  - `env -u RUSTC_WRAPPER cargo fmt --all`
  - `env -u RUSTC_WRAPPER cargo check -p oasis7_wasm_sdk`
  - `env -u RUSTC_WRAPPER cargo check -p oasis7_wasm_sdk --features wire`
  - `env -u RUSTC_WRAPPER cargo check -p oasis7 --features test_tier_required`
  - `env -u NO_COLOR ./scripts/sync-m1-builtin-wasm-artifacts.sh`
  - `env -u NO_COLOR ./scripts/sync-m4-builtin-wasm-artifacts.sh`
  - `env -u NO_COLOR ./scripts/sync-m1-builtin-wasm-artifacts.sh --check`
  ...
  - `env -u RUSTC_WRAPPER cargo check -p oasis7 --features test_tier_required`
- 遗留事项：
  - 无新增遗留；后续可按同样方式继续收敛 `decode_state/encode_state` 等可复用 helper。
