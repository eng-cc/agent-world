...
- 完成内容：完成 PRG-2（`agent_world_node` 链式 `block_hash` 改造）。
  - 代码：
...
- 测试：
  - `env -u RUSTC_WRAPPER cargo test -p agent_world_node pos_engine_generates_chain_hashed_block_ids -- --nocapture`
...
- 测试：
  - `env -u RUSTC_WRAPPER cargo test -p agent_world settlement_envelope_signature_verifies --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p agent_world settlement_envelope_signature_rejects_tampered_payload --features test_tier_required -- --nocapture`
- 遗留事项：
...
- 完成内容：完成 PRG-4（回归与文档收口）。
  ...
  - 回归结果：
    - `agent_world_node` 全量单测：21/21 通过。
    - `agent_world` 的 `reward_runtime_network` 模块测试：13/13 通过。
- 测试：
  - `env -u RUSTC_WRAPPER cargo test -p agent_world reward_runtime_network --features test_tier_required -- --nocapture`
...
- 完成内容：完成 PRG-B2（`agent_world_node` 共识内生执行 hook + 快照持久化扩展）。
  - 代码：
    - `crates/agent_world_node/src/lib.rs`
    - `crates/agent_world_node/src/pos_state_store.rs`
    - `crates/agent_world_node/src/tests.rs`
    - `crates/agent_world_node/src/error.rs`
...
- 测试：
  - `env -u RUSTC_WRAPPER cargo test -p agent_world --bin world_viewer_live execution_bridge::tests::node_runtime_execution_driver_persists_chain_records --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p agent_world --bin world_viewer_live --features test_tier_required -- --nocapture`
- 遗留事项：
...
- 完成内容：完成 PRG-B4（Phase B 回归与文档收口）。
  ...
  - 复核文件行数约束：
    - `crates/agent_world_node/src/lib.rs = 1079`（<1200）
    - `crates/agent_world/src/bin/world_viewer_live.rs = 1112`（<1200）
- 测试：
  - `env -u RUSTC_WRAPPER cargo test -p agent_world_node -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p agent_world --bin world_viewer_live --features test_tier_required -- --nocapture`
- 遗留事项：
...
- 测试：
  - `env -u RUSTC_WRAPPER cargo test -p agent_world --bin world_viewer_live distfs_challenge_network::tests::distfs_network_driver_roundtrip_challenge_and_proof --features test_tier_required -- --nocapture`
- 遗留事项：
...
- 完成内容：完成 PRG-C4（Phase C 回归与文档收口）。
  ...
  - 约束复核：
    - `crates/agent_world/src/bin/world_viewer_live.rs = 1161`（<1200）
    - `crates/agent_world/src/bin/world_viewer_live/distfs_challenge_network.rs = 1086`（<1200）
    - `crates/agent_world_node/src/lib.rs = 1079`（<1200）
- 测试：
  - `env -u RUSTC_WRAPPER cargo test -p agent_world --bin world_viewer_live --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p agent_world_node -- --nocapture`
...
- 完成内容：完成 MIP-2 代码迁移（builtin wasm 模块独立工程化 + 构建链路适配）。
  - `crates/agent_world_builtin_wasm` 改为共享逻辑库（`rlib`），新增 `builtin_alloc`、`reduce_for_module`、`call_for_module` 供模块 wrapper 复用。
  - 新增 23 个独立模块 crate：`crates/agent_world_builtin_wasm_modules/*`（每个 `module_id` 对应一个独立 `Cargo.toml` + 导出入口）。
  - 新增映射清单：`crates/agent_world/src/runtime/world/artifacts/builtin_module_manifest_map.txt`。
...
- 完成内容：完成 builtin wasm 独立工程迁移收口（MIP-3）。
  ...
  - 更新产物清单：`crates/agent_world/src/runtime/world/artifacts/m1_builtin_modules.sha256`、`crates/agent_world/src/runtime/world/artifacts/m4_builtin_modules.sha256`。
...
- 完成内容：完成 LIFESDK-1（生命周期 trait + wasm sdk）文档任务。
  ...
  - 设计对齐 `third_party/agent-os` wasm sdk 思路，明确在 Agent World 采用 `trait + dispatch + 导出宏` 的 wasm 生命周期约束。
- 测试：
  - 文档任务，无代码测试。
- 遗留事项：
  - 继续执行 LIFESDK-2：创建 `agent_world_wasm_sdk` 并迁移 builtin wasm 模块接入生命周期 trait。
...
- 完成内容：完成 LIFESDK-2（wasm 生命周期 trait + SDK 接入）代码迁移。
  ...
  - workspace 接线：`Cargo.toml` 增加 `crates/agent_world_wasm_sdk`。
  - 批量迁移 23 个 builtin wasm 独立模块：
    - `crates/agent_world_builtin_wasm_modules/*/src/lib.rs`
    - `crates/agent_world_builtin_wasm_modules/*/Cargo.toml`
    - 从手写三导出函数改为实现生命周期 trait + 使用 SDK 导出宏。
  - 构建与产物同步：执行 m1/m4 sync + check，确认 hash manifest 与 DistFS 均一致。
  - 依赖锁更新：根 `Cargo.lock` 与模块级 `Cargo.lock` 同步引入 `agent_world_wasm_sdk`。
- 测试：
  - `env -u RUSTC_WRAPPER cargo test -p agent_world_wasm_sdk`
...
- 完成内容：修复 CI `fmt` 失败并恢复格式门禁通过。
  - 根因：`env -u RUSTC_WRAPPER cargo fmt --all -- --check` 命中仓库内既有格式漂移（`world_viewer_live*`、`agent_world_node*` 相关文件）。
...
- 完成内容：完成 RCR-1 文档任务（删除旧 `agent_world_builtin_wasm` 并替换为新流程）。
...
- 完成内容：完成 RCR-2 代码迁移（删除旧 `agent_world_builtin_wasm`，切换到新流程）。
  - 目录迁移：`crates/agent_world_builtin_wasm` -> `crates/agent_world_builtin_wasm_runtime`。
  - workspace / 依赖切换：
    - 根 `Cargo.toml` 成员改为 `crates/agent_world_builtin_wasm_runtime`。
    - `crates/agent_world/Cargo.toml` 依赖改为 `agent_world_builtin_wasm_runtime`。
    - 23 个模块 crate (`crates/agent_world_builtin_wasm_modules/*/Cargo.toml`) 依赖改为 `agent_world_builtin_wasm_runtime`。
  - 生命周期模块调用切换：
    - 23 个模块 wrapper (`crates/agent_world_builtin_wasm_modules/*/src/lib.rs`) 从 `agent_world_builtin_wasm::` 全量替换到 `agent_world_builtin_wasm_runtime::`。
  - runtime 常量与回退构建链路切换：
    - `crates/agent_world/src/runtime/mod.rs` re-export 源改为新 crate。
    - `crates/agent_world/src/runtime/world/bootstrap_power.rs`
    - `crates/agent_world/src/runtime/world/bootstrap_economy.rs`
      - bootstrap build manifest 的 crate 标识改为 `agent_world_builtin_wasm_runtime`。
...
- 测试：
  - `env -u RUSTC_WRAPPER cargo test -p agent_world_builtin_wasm_runtime`
  - `env -u NO_COLOR ./scripts/sync-m1-builtin-wasm-artifacts.sh`
  - `env -u NO_COLOR ./scripts/sync-m4-builtin-wasm-artifacts.sh`
  - `env -u NO_COLOR ./scripts/sync-m1-builtin-wasm-artifacts.sh --check`
  - `env -u NO_COLOR ./scripts/sync-m4-builtin-wasm-artifacts.sh --check`
...
- 完成内容：完成 NSDK-1（`agent_world_wasm_sdk` no_std 优先化）文档任务。
...
- 测试：
  - 文档任务，无代码测试。
- 遗留事项：
  - 执行 NSDK-2：完成 `agent_world_wasm_sdk` no_std 代码迁移与回归。

- 日期：2026-02-17
- 时刻：2026-02-17 23:00:47 CST
- 完成内容：完成 NSDK-2（`agent_world_wasm_sdk` no_std 优先迁移）代码任务。
  - `crates/agent_world_wasm_sdk/src/lib.rs`：启用 `#![cfg_attr(not(any(test, feature = "std")), no_std)]`，引入 `alloc`/`core` 替代 `std`。
  - `crates/agent_world_wasm_sdk/Cargo.toml`：新增 feature 声明（`default = []`, `std = []`），避免 `cfg(feature = "std")` 校验告警。
  - `doc/world-runtime/wasm-sdk-no-std.project.md`：回写状态，标记 NSDK-2 完成。
- 测试：
  - `env -u RUSTC_WRAPPER cargo test -p agent_world_wasm_sdk`
  - `env -u RUSTC_WRAPPER cargo check -p agent_world_wasm_sdk --target wasm32-unknown-unknown`
  - `env -u RUSTC_WRAPPER cargo check -p agent_world --features test_tier_required`
- 遗留事项：
...
- 完成内容：完成 MBM-2（builtin wasm 业务逻辑下沉到模块工程）代码迁移与构建链路适配。
  - 23 个 `crates/agent_world_builtin_wasm_modules/*` 模块 crate 已内聚自身业务逻辑，不再通过 runtime 分发函数执行。
  - 模块依赖面收敛：移除 `agent_world_builtin_wasm_runtime` 依赖，统一依赖 `agent_world_wasm_sdk` + 序列化库。
  - runtime 常量迁移：`agent_world` runtime 常量改为从 `crates/agent_world_wasm_store/src/lib.rs` 导出。
  - 工程收敛：workspace 与 `agent_world` crate 依赖图中移除 `agent_world_builtin_wasm_runtime`，并删除其源码文件。
...
- 测试：
  ...
  - `env -u RUSTC_WRAPPER cargo fmt --all`
- 遗留事项：
  - 无新增遗留；后续如需继续收敛可评估清理文档中的历史 `agent_world_builtin_wasm_runtime` 描述。
...
- 完成内容：完成 WIRESDK-2（WASM SDK wire 类型收敛）代码迁移。
  - `crates/agent_world_wasm_sdk/Cargo.toml`：新增 `wire` feature（含 `serde/serde_cbor/serde_json` 可选依赖）。
  - `crates/agent_world_wasm_sdk/src/lib.rs`：新增 `wire` 模块，统一提供 `ModuleCallInput/ModuleContext/ModuleEffectIntent/ModuleEmit/ModuleOutput` 与 `decode_input/decode_action/empty_output/encode_output`。
  - `crates/agent_world_builtin_wasm_modules/*`：23 个模块 crate 删除本地重复 wire 结构与 helper，改为复用 SDK `wire`；模块 `Cargo.toml` 启用 `agent_world_wasm_sdk` 的 `wire` feature。
  - `doc/world-runtime/wasm-sdk-wire-types-dedup.project.md`：回填状态并标记 WIRESDK-2 完成。
- 测试：
  - `env -u RUSTC_WRAPPER cargo fmt --all`
  - `env -u RUSTC_WRAPPER cargo check -p agent_world_wasm_sdk`
