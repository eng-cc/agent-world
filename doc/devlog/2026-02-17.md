...
- 测试：
  ...
  - `env -u RUSTC_WRAPPER cargo run -p oasis7 --bin world_llm_agent_demo -- llm_bootstrap --ticks 30 --print-llm-io --llm-io-max-chars 1800 --report-json output/llm_bootstrap/user_factory_closedloop_lfo6_rerun_2026-02-16_235611/report.json`
- 遗留事项：
...
- 完成内容：执行 tool-only 协议在线闭环抽样（用户目标：建工厂 + 制成品），并输出 JSON 报告。
  - 命令：`env -u RUSTC_WRAPPER OASIS7_LLM_SHORT_TERM_GOAL='你必须先建造可用工厂，再安排可执行配方，确保在本次运行内至少产出1批制成品。' OASIS7_LLM_LONG_TERM_GOAL='形成稳定生产链并持续产出制成品，避免无效等待。' cargo run -p oasis7 --bin world_llm_agent_demo -- llm_bootstrap --ticks 30 --report-json output/llm_closed_loop/tool_only_llm_bootstrap_report.json`
...
- 完成内容：执行短程 `--print-llm-io` 抽样定位 parse_error 根因。
  - 命令：`env -u RUSTC_WRAPPER OASIS7_LLM_SHORT_TERM_GOAL='你必须先建造可用工厂，再安排可执行配方，确保在本次运行内至少产出1批制成品。' OASIS7_LLM_LONG_TERM_GOAL='形成稳定生产链并持续产出制成品，避免无效等待。' cargo run -p oasis7 --bin world_llm_agent_demo -- llm_bootstrap --ticks 10 --print-llm-io --llm-io-max-chars 500`
...
- 完成内容：按用户指令执行一次在线闭环测试（`llm_bootstrap --ticks 30`），使用强化目标 prompt 驱动 Agent 完成“建工厂 + 制成品 data”。
  - 命令：`OASIS7_LLM_SYSTEM_PROMPT='你是工业生产调度Agent。主目标：在30 tick内形成闭环，必须先建工厂，再排产制成品data。禁止空转和重复无效动作；遇到reject_reason要立即切换恢复动作。严格输出一个tool call，不要输出文本JSON。' OASIS7_LLM_SHORT_TERM_GOAL='30 tick 内必须完成：1) build_factory(factory.assembler.mk1) 至少一次成功；2) schedule_recipe(recipe.assembler.logistics_drone) 至少两次成功，推动 data 增长。恢复策略：insufficient hardware->refine_compound(compound_mass_g>=7000)；insufficient electricity->harvest_radiation(max_amount>=120)；factory_not_found->build_factory；agent_already_at_location->不要move，改为refine或schedule。' OASIS7_LLM_LONG_TERM_GOAL='保持可持续工业闭环：harvest_radiation -> refine_compound -> build_factory -> schedule_recipe，避免连续3轮以上同类无效采集。' env -u RUSTC_WRAPPER cargo run -p oasis7 --bin world_llm_agent_demo -- llm_bootstrap --ticks 30 --print-llm-io --llm-io-max-chars 1800 --report-json output/llm_bootstrap/user_factory_closedloop_codex_2026-02-17_115631/report.json | tee output/llm_bootstrap/user_factory_closedloop_codex_2026-02-17_115631/run.log`
...
- 测试：
  ...
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 llm_agent_reroutes_schedule_recipe_when_hardware_cannot_cover_one_batch --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 llm_agent --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER OASIS7_LLM_SHORT_TERM_GOAL='你必须先建造可用工厂，再安排可执行配方，确保在本次运行内至少产出1批制成品。' OASIS7_LLM_LONG_TERM_GOAL='形成稳定生产链并持续产出制成品，避免无效等待。' cargo run -p oasis7 --bin world_llm_agent_demo -- llm_bootstrap --ticks 10 --print-llm-io --llm-io-max-chars 700`
- 遗留事项：
...
- 完成内容：更新项目管理文档，新增 LFO12 任务与结果摘要，并记录本轮新增 TODO。
  - 文档：`doc/world-simulator/llm-factory-strategy-optimization.project.md`
- 测试：
  - `env -u RUSTC_WRAPPER cargo run -p oasis7 --bin world_llm_agent_demo -- llm_bootstrap --ticks 60 --print-llm-io --llm-io-max-chars 1800 --report-json output/llm_bootstrap/all_factory_all_products_codex_2026-02-17_123501/report.json`
- 遗留事项：
...
- 完成内容：完成 MMD3 闭环验证与收口。
  ...
  - 在线闭环抽样 #1（机制链路验证：先采矿再生产）：
    - 命令：`env -u RUSTC_WRAPPER cargo run -p oasis7 --bin world_llm_agent_demo -- llm_bootstrap --ticks 60 --print-llm-io --llm-io-max-chars 1800 --report-json output/llm_bootstrap/mmd3_industrial_closure_2026-02-17_132336/report.json`
    - 产物：
  ...
  - 在线闭环抽样 #2（全配方覆盖尝试）：
    - 命令：`env -u RUSTC_WRAPPER cargo run -p oasis7 --bin world_llm_agent_demo -- llm_bootstrap --ticks 80 --print-llm-io --llm-io-max-chars 1200 --report-json output/llm_bootstrap/mmd3_all_recipes_2026-02-17_132728/report.json`
    - 产物：
...
- 测试：
  - `env -u RUSTC_WRAPPER cargo check -p oasis7 --tests --features test_tier_required`
  - `env -u RUSTC_WRAPPER cargo check -p oasis7_viewer --target wasm32-unknown-unknown`
  - `env -u RUSTC_WRAPPER cargo run -p oasis7 --bin world_llm_agent_demo -- llm_bootstrap --ticks 60 --print-llm-io --llm-io-max-chars 1800 --report-json output/llm_bootstrap/mmd3_industrial_closure_2026-02-17_132336/report.json`
  - `env -u RUSTC_WRAPPER cargo run -p oasis7 --bin world_llm_agent_demo -- llm_bootstrap --ticks 80 --print-llm-io --llm-io-max-chars 1200 --report-json output/llm_bootstrap/mmd3_all_recipes_2026-02-17_132728/report.json`
- 遗留事项：
...
- 按单文件约束拆分 `world_viewer_live` 测试模块：
  - `crates/oasis7/src/bin/world_viewer_live.rs` 保持在 1200 行以内。
  - 测试迁移到 `crates/oasis7/src/bin/world_viewer_live/world_viewer_live_tests.rs`。
...
- `env -u RUSTC_WRAPPER cargo test -p oasis7 --bin world_viewer_live --features test_tier_required -- --nocapture`
...
## 测试
- `env -u RUSTC_WRAPPER cargo check -p oasis7 --bin world_viewer_live`
...
## 测试
- `env -u RUSTC_WRAPPER cargo check -p oasis7 --bin world_viewer_live`
- `env -u RUSTC_WRAPPER cargo test -p oasis7 --bin world_viewer_live --features test_tier_required`
...
- 完成 `oasis7` `world_viewer_live` 编译检查与 `test_tier_required` 回归，确认主 crate 链路稳定。
...
## 完成内容
- 新增 `world_viewer_live` 子模块：`crates/oasis7/src/bin/world_viewer_live/distfs_probe_runtime.rs`。
...
## 测试
- `env -u RUSTC_WRAPPER cargo test -p oasis7 --bin world_viewer_live build_reward_settlement_mint_records_uses_preview_world_without_mutation --features test_tier_required -- --nocapture`
- `env -u RUSTC_WRAPPER cargo test -p oasis7 --bin world_viewer_live --features test_tier_required -- --nocapture`
...
- 完成内容：修复仓库 Rust 格式校验失败（fmt），统一格式化受影响代码文件。
  ...
  - 变更文件：
    ...
    - `crates/oasis7/src/bin/world_viewer_live/distfs_probe_runtime.rs`
    - `crates/oasis7/src/bin/world_viewer_live/world_viewer_live_tests.rs`
...
## 完成内容
- 新增 `world_viewer_live` 子模块：`crates/oasis7/src/bin/world_viewer_live/execution_bridge.rs`。
...
## 测试
- `env -u RUSTC_WRAPPER cargo test -p oasis7 --bin world_viewer_live --features test_tier_required execution_bridge -- --nocapture`
- `env -u RUSTC_WRAPPER cargo check -p oasis7 --bin world_viewer_live`
...
- 完成内容：完成 ERCB-2（奖励结算网络触发）实现收口并接入 `world_viewer_live` 主循环。
  - 代码：
    - `crates/oasis7/src/bin/world_viewer_live.rs`
    - `crates/oasis7/src/bin/world_viewer_live/reward_runtime_network.rs`
    - `crates/oasis7/src/bin/world_viewer_live/world_viewer_live_tests.rs`
...
- 完成内容：按仓库约束拆分 `world_viewer_live`，确保单文件不超过 1200 行。
  - 新增模块：
    - `crates/oasis7/src/bin/world_viewer_live/cli.rs`
    - `crates/oasis7/src/bin/world_viewer_live/node_keypair_config.rs`
...
- 完成内容：完成 PRG-B1（Phase B 共识内生执行）文档任务。
  ...
  - 明确本轮范围：NodeRuntime 执行 hook、快照/持久化扩展、world_viewer_live 执行驱动接线与 fallback 策略。
- 测试：
  - 文档任务，无代码测试。
- 遗留事项：
...
- 测试：
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_node -- --nocapture`
- 遗留事项：
  - 继续执行 PRG-B3（world_viewer_live 接入节点内生执行驱动）。

- 日期：2026-02-17
- 时刻：2026-02-17 19:20:42 CST
- 完成内容：完成 PRG-B3（`world_viewer_live` 接入节点内生执行驱动，保留 bridge fallback）。
  - 代码：
    - `crates/oasis7/src/bin/world_viewer_live/execution_bridge.rs`
    - `crates/oasis7/src/bin/world_viewer_live.rs`
  - 变更：
...
- 测试：
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 --bin world_viewer_live execution_bridge::tests::node_runtime_execution_driver_persists_chain_records --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 --bin world_viewer_live --features test_tier_required -- --nocapture`
- 遗留事项：
...
- 完成内容：完成 PRG-B4（Phase B 回归与文档收口）。
  ...
  - 复核文件行数约束：
    ...
    - `crates/oasis7/src/bin/world_viewer_live.rs = 1112`（<1200）
- 测试：
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_node -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 --bin world_viewer_live --features test_tier_required -- --nocapture`
- 遗留事项：
...
- 完成内容：完成 PRG-C2（DistFS challenge/proof 消息协议与签名验签）。
  - 代码：
    - `crates/oasis7/src/bin/world_viewer_live/distfs_challenge_network.rs`
  - 变更：
    - 新增 request/proof topic、消息结构、编解码、签名与验签。
    - 新增消息 ID 计算与失败原因归类。
    - 新增模块单测（topic/签名/跨节点 roundtrip）。
- 测试：
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 --bin world_viewer_live distfs_challenge_network::tests::distfs_network_driver_roundtrip_challenge_and_proof --features test_tier_required -- --nocapture`
- 遗留事项：
...
- 完成内容：完成 PRG-C3（reward runtime 接线跨节点 DistFS challenge/proof）。
  - 代码：
    - `crates/oasis7/src/bin/world_viewer_live.rs`
    - `crates/oasis7/src/bin/world_viewer_live/observation_trace_runtime.rs`
  - 变更：
    ...
    - 观察轨迹应用逻辑拆分到 `observation_trace_runtime.rs`，保持 `world_viewer_live.rs` <1200 行。
- 测试：
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 --bin world_viewer_live --features test_tier_required -- --nocapture`
- 遗留事项：
...
- 完成内容：完成 PRG-C4（Phase C 回归与文档收口）。
  ...
  - 约束复核：
    - `crates/oasis7/src/bin/world_viewer_live.rs = 1161`（<1200）
    - `crates/oasis7/src/bin/world_viewer_live/distfs_challenge_network.rs = 1086`（<1200）
    - `crates/oasis7_node/src/lib.rs = 1079`（<1200）
- 测试：
  - `env -u RUSTC_WRAPPER cargo test -p oasis7 --bin world_viewer_live --features test_tier_required -- --nocapture`
  - `env -u RUSTC_WRAPPER cargo test -p oasis7_node -- --nocapture`
...
- 完成内容：修复 CI `fmt` 失败并恢复格式门禁通过。
  - 根因：`env -u RUSTC_WRAPPER cargo fmt --all -- --check` 命中仓库内既有格式漂移（`world_viewer_live*`、`oasis7_node*` 相关文件）。
...
- 完成内容：清理 `required` 阶段编译 warning（world_viewer_live / distfs_challenge_network / distfs_probe_runtime）。
  - 代码：
    - `crates/oasis7/src/bin/world_viewer_live.rs`
    - `crates/oasis7/src/bin/world_viewer_live/distfs_challenge_network.rs`
    - `crates/oasis7/src/bin/world_viewer_live/distfs_probe_runtime.rs`
