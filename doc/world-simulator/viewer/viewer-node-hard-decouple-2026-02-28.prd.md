...
## 1. Executive Summary
- 将 `oasis7_viewer_live` 从“可内嵌节点运行时”的混合进程，重构为纯 Viewer/游戏服务进程。
- 明确节点运行时唯一入口为 `oasis7_chain_runtime`，避免双路径并存导致的配置歧义。
- 对外发行统一链路保持不变：`oasis7_game_launcher` 负责编排 viewer + chain-runtime。

## 2. User Experience & Functionality
- 重写 `oasis7_viewer_live` 启动入口与 CLI，仅保留 Viewer 相关参数与 Web bridge。
- 移除 `oasis7_viewer_live` 中的节点启动、PoS/P2P、reward-runtime、execution-bridge 运行路径。
- 更新测试与文档，确保“禁止在 viewer 内启节点”的行为可验证。

## 非目标
- 本轮不删除历史节点实现源码文件（先停止编译接线，后续再做代码归档清理）。
- 本轮不改动 `oasis7_chain_runtime` 的节点能力边界。
...
### `oasis7_viewer_live` CLI（重构后）
- 保留：
  - `oasis7_viewer_live [scenario]`
  - `--bind <host:port>`
  - `--web-bind <host:port>`
  - `--llm` / `--no-llm`
  - `-h` / `--help`
...
## 5. Risks & Roadmap
- M1：文档建档（设计 + 项目管理）。
- M2：`oasis7_viewer_live` 重构为纯 viewer 入口。
- M3：测试回归与文档结项。

### Technical Risks
- 依赖旧 `oasis7_viewer_live --node-*` 的脚本将失效。
  - 缓解：CLI 显式报错并提示改用 `oasis7_chain_runtime`/`oasis7_game_launcher`。
