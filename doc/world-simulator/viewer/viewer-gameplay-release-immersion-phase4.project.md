# Viewer 发行体验改造（第四阶段：项目管理文档）

- 对应设计文档: `doc/world-simulator/viewer/viewer-gameplay-release-immersion-phase4.design.md`
- 对应需求文档: `doc/world-simulator/viewer/viewer-gameplay-release-immersion-phase4.prd.md`

审计轮次: 5
## 任务拆解（含 PRD-ID 映射）
- [x] VRI4P0：建立第四阶段设计文档与项目管理文档。
- [x] VRI4P1：实现 Player 场景氛围层（背景层次/呼吸光晕/边缘发光）。
- [x] VRI4P2：实现 Player 引导与目标卡片过渡动效（淡入/位移/脉冲）。
- [x] VRI4P3：执行回归与 Web 闭环验收并完成文档收口。

## 依赖
- doc/world-simulator/viewer/viewer-gameplay-release-immersion-phase4.prd.md
- `crates/oasis7_viewer/src/egui_right_panel_player_experience.rs`
- `crates/oasis7_viewer/src/egui_right_panel_player_card_motion.rs`
- `crates/oasis7_viewer/src/egui_right_panel_tests.rs`
- `testing-manual.md`

## 状态
- 当前阶段：VRI4P0~VRI4P3 全部完成（第四阶段收口）。
- 阻塞项：无。
- 最近更新：完成 S6 Web 闭环验收、回归与文档收口。
