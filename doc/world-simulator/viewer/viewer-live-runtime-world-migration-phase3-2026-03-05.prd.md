...
- Proposed Solution: 在 runtime live 补齐高频可映射 action 覆盖并增加等价回归；同时删除 `world_viewer_live` simulator 启动分支，统一为 runtime-only 链路。
- Success Criteria:
  ...
  - SC-4: `world_viewer_live` 删除 simulator 启动分支，运行入口统一 runtime-only。
...
- User Stories:
  - As a 玩法架构开发者, I want runtime live action mapping coverage and runtime-only launch path, so that world_viewer_live no longer has split simulator/runtime behavior.
- Critical User Flows:
  ...
  2. Flow-VIEWER-RUNTIME-LLM-006（分支收敛）:
     `启动 world_viewer_live -> 固定 runtime live server -> script/llm 按开关执行`
...
| world_viewer_live runtime-only | `--llm/--no-llm` | 启动后仅走 runtime live server | `runtime_script/runtime_llm` | 不再分叉到 simulator server | 本地受控启动链路 |
- Acceptance Criteria:
...
- Tool Requirements:
  ...
  - world_viewer_live runtime-only 启动链路
- Evaluation Strategy:
  - 行为一致性：映射等价回归通过。
  - 可诊断性：不可映射动作拒绝语义稳定。
