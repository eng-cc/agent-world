# Viewer 商业化发行缺口收敛 Phase 1（项目管理）

- 对应设计文档: `doc/world-simulator/viewer/viewer-commercial-release-phase1-asset-pipeline.design.md`
- 对应需求文档: `doc/world-simulator/viewer/viewer-commercial-release-phase1-asset-pipeline.prd.md`

审计轮次: 5

## 任务拆解（含 PRD-ID 映射）
- [x] VCR1-0 文档建档：设计文档 + 项目管理文档
- [x] VCR1-1 外部 mesh 配置结构与环境变量解析
- [x] VCR1-2 场景初始化接入外部 mesh 覆盖与内置兜底
- [x] VCR1-3 测试、手册、devlog 收口

## 依赖
- doc/world-simulator/viewer/viewer-commercial-release-phase1-asset-pipeline.prd.md
- `crates/oasis7_viewer/src/viewer_3d_config.rs`
- `crates/oasis7_viewer/src/viewer_3d_config_profile_tests.rs`
- `crates/oasis7_viewer/src/main.rs`
- `doc/world-simulator/viewer/viewer-manual.md`

## 状态
- 当前阶段：VCR1-0 ~ VCR1-3 全部完成。
- 下一步：进入 Phase 2（真实美术资源包接入与视觉验收基线）设计与排期。
- 最近更新：2026-02-21（完成外部 mesh 配置接入、测试与手册收口）。
