# oasis7 Simulator：Viewer 自动聚焦（人机共用 + 截图闭环）设计文档

- 对应设计文档: `doc/world-simulator/viewer/viewer-auto-focus-capture.design.md`
...
- 与现有渲染/协议兼容：不改 `oasis7` 协议，仅在 viewer 本地消费现有 snapshot/scene 数据。

## 2. User Experience & Functionality

### In Scope
- `oasis7_viewer` 新增自动聚焦配置（环境变量）与一次性启动聚焦流程。
- `oasis7_viewer` 新增人工快捷键（`F`）聚焦当前选中对象。
...
### 脚本参数
- `--auto-focus-target <target>`：透传到 `OASIS7_VIEWER_AUTO_FOCUS_TARGET`。
- `--auto-focus-radius <value>`：透传到 `OASIS7_VIEWER_AUTO_FOCUS_RADIUS`。
- `--auto-focus-keep-2d`：将 `OASIS7_VIEWER_AUTO_FOCUS_FORCE_3D=0`。
