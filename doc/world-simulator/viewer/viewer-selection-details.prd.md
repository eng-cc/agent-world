...
- 扩展覆盖对象：除 Agent/Location 外，新增 Asset/PowerPlant/Chunk 详情能力。
- 保持离线回放兼容：无 LLM trace 数据时仍可展示基础详情并明确提示“无 LLM trace”。
- 遵循可视化总原则：通过详情面板以最直接的方式获取对象相关的模拟信息。
- 修复右侧 UI 在高信息密度下的面板重叠/挤压问题，并统一基础视觉风格。
- 顶部控制区支持折叠开关，并对高密度文本进行字号微调，降低视觉拥挤。
- 右侧上半区（Top Controls）在内容超高时可独立滚动，避免与下半区争抢滚轮焦点。
- 按钮交互补充悬停/按下视觉反馈，并为 `Step` 控制提供等待态（loading）提示。
...
- **范围内**
  ...
  - 新增顶部控制区折叠按钮（Hide/Show Top），支持快速聚焦下半区详情面板。
  - 时间轴/诊断/覆盖层状态文本做字号微调，提升同屏可读性。
  - 右侧滚轮分流支持“上半区优先滚动（命中 Top band）/下半区兜底滚动”。
  - 控制按钮增加 hover/pressed 视觉反馈，提升可点击感知。
  - `Step` 在响应返回前显示 `Step ...`，并防止重复发送 step 请求。
- **范围外**
  - 不实现历史 trace 分页/检索系统。
...
- UI 状态扩展：`StepControlLoadingState { pending, baseline_tick }`
- 详情面板展示策略：
...
- **M4**：完成 Asset/PowerPlant 详情扩展与测试覆盖。
- **M5**：完成 Chunk 详情扩展与测试覆盖。
- **M6**：补齐模块可视实体事件联动与右侧面板滚动能力。
- **M7**：修复右侧面板重叠并完成基础视觉美化（层次、间距、按钮排版）。
- **M8**：新增顶部控制区折叠交互与文本密度优化（字号/按钮文案）。
- **M9**：补齐上半区滚动、按钮反馈与 Step loading 等待态。
