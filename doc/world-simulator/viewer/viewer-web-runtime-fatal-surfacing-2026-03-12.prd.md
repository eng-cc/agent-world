...
- 本专题目标不是一次性修复上游渲染兼容，而是先把 fatal 信号透出到 `window.__AW_TEST__`、让自动化脚本快失败，并把 Viewer 手册补到与测试总手册一致。
...
- 不承诺本轮内彻底消除 SwiftShader/WebGL2 的渲染崩溃。
...
- `web_test_api` 新增浏览器运行时 fatal 采集：
  ...
  - `webglcontextlost`
- `publish_web_test_api_state()` 在存在 runtime fatal 时：
  - 将 `connectionStatus` 对外透出为 `error`
  - 将 fatal 消息写入 `lastError`
  - 递增 `errorCount`
...
- 风险 2：仍未根治 SwiftShader 渲染崩溃。
  - 缓解：本轮先保证失败可诊断、脚本不假挂起、手册不误导。
