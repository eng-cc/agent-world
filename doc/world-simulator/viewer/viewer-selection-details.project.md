...
## 任务拆解
- [x] 输出设计文档（`doc/world-simulator/viewer/viewer-selection-details.md`）
- [x] 输出项目管理文档（本文件）
...
- [x] 新增/更新测试（顶部折叠交互回归）
- [x] M9：右侧上半区滚动（Top band 命中优先）与下半区兜底滚动分流
- [x] M9：控制按钮悬停/按下视觉反馈（hover/pressed）
- [x] M9：`Step` 等待态文案与防重复触发（`Step ...` + pending）
- [x] 新增/更新测试（滚动命中、Step loading 状态、控制按钮请求路径）
- [x] M9.1：滚动命中改为按 UI 实际边界（ComputedNode）判定，修复上下半区随机串滚
- [x] M9.1：高 DPI 光标坐标统一到 physical pixel，避免命中偏移
- [x] 更新可视化文档与总项目管理文档状态
- [x] 更新任务日志并提交
...
- 最近更新：完成 M9.1（按真实 UI 边界命中滚动，修复上下半区串滚，2026-02-07）
