...
审计轮次: 5
## 任务拆解（含 PRD-ID 映射）
- [x] 输出设计文档（`doc/world-simulator/viewer/viewer-visualization.prd.md`）
- [x] 输出项目管理文档（本文件）
...
- [x] UI：自动诊断结论面板（无需人工分析）
- [x] UI 测试覆盖：诊断优先级结论文案
- [x] UI：事件列表逐条点击定位对象（Click Event -> Select Object）
- [x] UI 测试覆盖：事件行点击触发对象定位与时间轴联动
- [x] UI：右侧顶部控制区/详情区布局防重叠（修复固定高与 100% 高度叠加）
- [x] UI：时间轴与事件联动按钮排版改为可换行（窄面板防覆盖）
- [x] UI：右侧卡片基础视觉样式优化（边框/层次/间距）
- [x] UI 测试覆盖：事件联动按钮区换行布局回归
...
- [x] 支持自动诊断结论直出（连接/LLM/ActionRejected）
