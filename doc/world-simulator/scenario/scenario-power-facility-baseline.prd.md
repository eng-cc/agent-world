...
- 避免 simulator 与 runtime 出现“双轨默认供能语义”：
  - runtime 已通过 `m1.power.radiation_harvest` 提供生命出厂基础供能；
  - simulator 不再在常用默认场景里额外预置发电设施。
- 保留电力设施能力，但改为“显式场景选择”：只有在场景文件中显式声明 `power_plants` 时才注入设施。
...
### Out of Scope
- 移除 simulator 的发电设施数据结构或 Action（`RegisterPowerPlant` 仍保留）。
...
- `build_world_model` 初始化逻辑不变，仍按 `init.power_plants` 注入；本次仅调整默认场景配置与测试口径。
