...
- [ ] FRB4.2 扩展 `AsteroidFragmentConfig`（`replenish_interval_ticks` / `replenish_percent_ppm` / `material_distribution_strategy`）
- [ ] FRB4.3 在 `WorldKernel::step` 接入运行期补种与 `FragmentsReplenished` 事件（含 replay）
- [ ] FRB4.4 在 `generate_fragments` 接入资源类型分区分布策略
- [ ] FRB4.5 补充 `test_tier_required` 测试（补种周期、补种比例、分布策略、replay）
