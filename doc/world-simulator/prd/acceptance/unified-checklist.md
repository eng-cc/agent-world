...
- In Scope:
  ...
  - Viewer Web-first 闭环（Playwright）验收命令与证据标准。
...
| G2 | Viewer Web 闭环 | 页面可加载、`console error = 0`、至少 1 张截图 | 按 `doc/testing/manual/web-ui-playwright-closure-manual.prd.md` 执行 S6（如 `./scripts/viewer-release-qa-loop.sh`） | `output/playwright/*.png` + `.playwright-cli/console-*.log` |
