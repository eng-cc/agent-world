...
- 最新完成: `TASK-WORLD_SIMULATOR-266`（已收口 `doc/world-simulator/viewer/{viewer-websocket-http-bridge,viewer-copyable-text,viewer-i18n,viewer-web-closure-testing-policy,viewer-control-predictability-tasklist-2026-02-28}.{prd,project}` 中仍把旧 `agent_world*` crate/path/package/command 写成当前真值的活跃 Web-first/基础交互专题口径，统一到 `oasis7*` 与当前 `cargo -p oasis7_viewer`。）
