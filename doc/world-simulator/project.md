...
- 最新完成: `TASK-WORLD_SIMULATOR-267`（已收口 `doc/world-simulator/viewer/{viewer-dual-view-2d-3d,viewer-wasd-camera-navigation,viewer-3d-polish-performance,viewer-visual-upgrade,viewer-visualization}.{prd,project}` 中仍把旧 `agent_world*` crate/path/package/command 与旧 `AGENT_WORLD_VIEWER_*` visual env 写成当前真值的活跃 visualization/navigation 专题口径，统一到 `oasis7*` 与当前 `OASIS7_VIEWER_*`。）
