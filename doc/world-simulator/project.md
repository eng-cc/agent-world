...
- 最新完成: `TASK-WORLD_SIMULATOR-281`（已收口 `runtime_live`、`self_guided`、`egui_right_panel_chat` 与 `world_game_launcher_tests` 的环境变量尾批负向测试命名和断言辅助，统一到 `removed_old_brand` 语义，并集中旧 viewer auth env 的失效断言。）
