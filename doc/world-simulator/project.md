...
- 最新完成: `TASK-WORLD_SIMULATOR-209~218`（已将 `agent_world_wasm_{sdk,abi,router,executor,store}`、`agent_world_proto`、`agent_world_{distfs,consensus,net,node}`、`agent_world_launcher_ui`、`agent_world_client_launcher`、`agent_world_viewer` 与主 crate `agent_world` 的 crate name、workspace member 与目录名切到 `oasis7*`，并同步更新 builtin wasm 模块、runtime、chain runtime、viewer live、launcher bundle/Trunk/theme/dist 入口、tests、脚本与协议层下游入口。）
