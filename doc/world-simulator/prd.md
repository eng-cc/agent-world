...
- Acceptance Criteria:
  ...
  - AC-58: `scripts/setup-openclaw-oasis7-runtime.sh`、`.agents/skills/oasis7/scripts/oasis7-run.sh`、`.agents/skills/oasis7/**/*.md` 与 `world_openclaw_local_bridge` 相关测试样例必须默认使用 `oasis7_runtime` 与 `OPENCLAW_OASIS7_{AGENT_ID,WORKSPACE,MODEL}`；repo-owned runtime workspace 默认路径必须切到 `tools/openclaw/oasis7_runtime_workspace`，旧 `scripts/setup-openclaw-agent-world-runtime.sh` / `agent_world_runtime_workspace` 仅可作为兼容包装层或历史路径上下文保留，不得继续作为 operator-facing 真值。
