...
- 备注: 本专题只定义“双轨模式”的产品目标与执行边界；它不替代 `PRD-WORLD_SIMULATOR-038` 的 parity 门禁，而是为 parity/回归/观战拆出各自清晰口径。2026-03-18 已将该分层直接回写到 `oasis7` operator 入口，明确“OpenClaw 能跑不等于必须开 UI”。
