...
- T5 主链路补充: `oasis7_client_launcher` 已把 `agent_provider_mode/openclaw_base_url/openclaw_auth_token/openclaw_connect_timeout_ms/openclaw_agent_profile` 正式透传到 `oasis7_game_launcher`；后者再通过环境变量把 OpenClaw 设置注入 `world_viewer_live` 的 runtime live sidecar，OpenClaw 现在可以走产品默认启动链路进入真实运行时。
...
- T5 停止收敛补充: `oasis7-run.sh play` 现已在 wrapper 层记录 launcher pid / process group，并在 `INT`/`TERM`/`HUP`/`EXIT` 时做 best-effort 子树清理，避免 bundle/source 启动后残留 `oasis7_game_launcher` / `world_chain_runtime` / `world_viewer_live` 常驻。
