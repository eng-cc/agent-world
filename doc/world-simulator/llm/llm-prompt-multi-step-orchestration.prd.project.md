...
## 任务拆解（含 PRD-ID 映射）
- [x] LMSO1 输出设计文档（`doc/world-simulator/llm/llm-prompt-multi-step-orchestration.prd.md`）
...
## 状态（LMSO28）
- 当前子阶段：LMSO29（进行中，详见 `doc/world-simulator/llm/llm-lmso29-stability.prd.md`）
...
- [x] LMSO30A 多场景评测设计文档（`doc/world-simulator/llm/llm-multi-scenario-evaluation.prd.md`）。
...
## 状态（LMSO30）
- 当前子阶段：LMSO30（已完成，详见 `doc/world-simulator/llm/llm-multi-scenario-evaluation.prd.project.md`）
