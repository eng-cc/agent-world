...
- [ ] T1 (PRD-WORLD_SIMULATOR-012) [test_tier_required]: `agent_world_client_launcher` 落地 wasm 入口，复用同一套 egui UI 渲染与 schema 字段映射。
- [ ] T2 (PRD-WORLD_SIMULATOR-012) [test_tier_required]: `world_web_launcher` 改为静态资源服务（launcher wasm）+ API 路由并存。
- [ ] T3 (PRD-WORLD_SIMULATOR-012) [test_tier_required]: 更新 bundle 构建脚本生成 `web-launcher/`，补齐测试与文档回写。
