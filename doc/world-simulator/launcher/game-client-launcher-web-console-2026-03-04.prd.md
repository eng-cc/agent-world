...
- Critical User Flows:
  1. Flow-LAUNCHER-WEB-001（远程启动）:
     `SSH 启动 world_web_launcher -> 浏览器访问控制台 -> 填写/确认配置 -> 点击启动 -> 观察 running 状态与游戏 URL`
...
- Acceptance Criteria:
  ...
  - AC-2: `POST /api/start` 可拉起 `world_game_launcher`，并透传链路参数。
...
- Phased Rollout:
  - M1: PRD 建模与任务拆解。
  - M2: `world_web_launcher` 与 Web API 落地。
  - M3: 打包入口与文档收口。
- Technical Risks:
