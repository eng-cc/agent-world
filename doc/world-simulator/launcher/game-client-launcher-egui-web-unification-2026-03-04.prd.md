...
- User Stories:
  ...
  - As an 运维人员, I want world_web_launcher to serve launcher static assets directly, so that I can manage game startup remotely from browser on headless servers.
- Critical User Flows:
...
- Non-Goals:
  ...
  - 不在本轮改造 `world_game_launcher` 与 `world_chain_runtime` 的底层进程协议。
...
- Non-Functional Requirements:
  ...
  - NFR-3: Rust 单文件行数约束持续满足（`world_web_launcher.rs`、`main.rs` 均 <=1200）。
...
- Decision Log:
  - DEC-LAUNCHER-EGUI-001: 采用“同一 launcher egui crate 双目标编译 + world_web_launcher 静态托管”方案，而非继续维护独立 HTML 控制台。理由：可从代码结构层面消除 UI 分叉并降低后续维护成本。
