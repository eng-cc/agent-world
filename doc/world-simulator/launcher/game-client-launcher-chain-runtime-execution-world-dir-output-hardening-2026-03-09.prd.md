...
审计轮次: 1
- 对应项目管理文档: `doc/world-simulator/launcher/game-client-launcher-chain-runtime-execution-world-dir-output-hardening-2026-03-09.prd.project.md`
...
- Proposed Solution: 在 `world_game_launcher` 与 `world_web_launcher` 构造 runtime 启动参数时，显式传递 `--execution-world-dir`，统一收敛到 `output/chain-runtime/<node_id>/reward-runtime-execution-world`。
- Success Criteria:
  ...
  - SC-4: `world_game_launcher` 与 `world_web_launcher` 的参数构建单测均锁住该行为，防止回归。
...
- User Scenarios & Frequency:
  - 每次通过 `world_game_launcher` 或 `world_web_launcher` 拉起链运行时都会触发；日常开发和回归高频发生。
- User Stories:
...
- Critical User Flows:
  1. Flow-LAUNCHER-033-001（CLI 启动链路）:
     `world_game_launcher 解析链参数 -> 构建 world_chain_runtime args -> 附带 --execution-world-dir -> runtime 写 explorer-index.json 到 output/...`
  2. Flow-LAUNCHER-033-002（Web 控制面链路）:
     `world_web_launcher 接收 /api/chain/start -> 构建 world_chain_runtime args -> 附带 --execution-world-dir -> runtime 写 explorer-index.json 到 output/...`
...
- Acceptance Criteria:
  - AC-1: `world_game_launcher::build_world_chain_runtime_args` 返回参数必须包含 `--execution-world-dir` 及 `output/chain-runtime/<node_id>/reward-runtime-execution-world`。
  - AC-2: `world_web_launcher::build_chain_runtime_args` 返回参数必须包含 `--execution-world-dir` 及同规则路径。
...
| DEC-LAUNCHER-033-002 | 双入口（`world_game_launcher` + `world_web_launcher`）同时收敛 | 仅修复单入口 | 两条入口都可托管 runtime，必须保持一致避免隐式分叉。 |
