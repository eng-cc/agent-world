...
- [ ] T1 (PRD-WORLD_SIMULATOR-011) [test_tier_required]: 新增共享 schema crate，沉淀字段定义与可见性策略。
- [ ] T2 (PRD-WORLD_SIMULATOR-011) [test_tier_required]: native 启动器改为 schema 驱动渲染核心配置区。
- [ ] T3 (PRD-WORLD_SIMULATOR-011) [test_tier_required]: web 控制台新增 `/api/ui/schema` 并改为动态表单渲染。
- [ ] T4 (PRD-WORLD_SIMULATOR-011) [test_tier_required]: 拆分 `world_web_launcher` 大文件（<=1200 行），补齐测试与文档回写。
