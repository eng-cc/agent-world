...
- [x] T5 (PRD-WORLD_SIMULATOR-034) [test_tier_required]: 将 `world_web_launcher` / `oasis7_client_launcher` / `world_game_launcher` 的默认链配置改为 fresh `chain_node_id`，并完成“默认 `start_chain -> query_explorer_overview`”实机复验。
...
- 联审: `runtime_engineer`
- 备注: `T0~T5` 已完成；`world_web_launcher` 已新增 stale execution world 结构化状态、GUI Agent `recover_chain` 动作与 fresh node id 恢复建议；现在 `world_web_launcher` / `oasis7_client_launcher` / `world_game_launcher` 默认链配置与 `scripts/run-game-test.sh` 一样，都会优先使用 fresh `chain_node_id`，减少默认入口复用脏 execution world 的概率。
