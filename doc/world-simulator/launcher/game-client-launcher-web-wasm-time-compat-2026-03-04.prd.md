...
审计轮次: 5
- 对应项目管理文档: doc/world-simulator/launcher/game-client-launcher-web-wasm-time-compat-2026-03-04.prd.project.md
...
- Proposed Solution: 为 launcher wasm 路径替换为 Web 兼容的计时实现，并将 Playwright 闭环（open/snapshot/console/state）纳入验收，阻断同类回归。
- Success Criteria:
...
- User Personas:
  ...
  - Web 闭环执行者：需要 Playwright 验收可复现且可审计。
...
- User Stories:
  ...
  - As a 测试执行者, I want Playwright evidence to include console/state/screenshot, so that regressions are quickly diagnosable.
- Critical User Flows:
  ...
  3. Flow-LAUNCHER-WASM-TIME-003（闭环采证）:
     `Playwright open --headed -> snapshot -> console error -> screenshot -> 产物归档`
...
| Playwright 证据 | `snapshot/console/screenshot/state` | 执行固定命令序列采集证据 | `running -> evidence_ready` | 证据目录固定 `output/playwright/` | 执行者可写 |
- Acceptance Criteria:
...
- Architecture Overview:
  ...
  - Playwright CLI 作为 Web 闭环执行器，输出标准证据。
- Integration Points:
