...
- [ ] T1 (PRD-WORLD_SIMULATOR-030) [test_tier_required]: 落地首次 3 步引导向导（打开/跳过/完成/重置）与任务状态联动。
...
- [ ] T9 (PRD-WORLD_SIMULATOR-030) [test_tier_required]: 为 nonce/slot/mempool/action_id 补齐术语内联解释。
- [ ] T10 (PRD-WORLD_SIMULATOR-030) [test_tier_required]: 新增“最近成功配置画像”保存/恢复/清空闭环。
- [ ] T11 (PRD-WORLD_SIMULATOR-030) [test_tier_required]: 落地演示模式一键闭环（安全默认配置 + 串行动作 + 日志提示）。
...
- 当前阶段: in_progress
- 当前任务: T1（首次 3 步引导向导）
- 备注: T0 已完成，T1~T12 待逐步交付并按任务单独提交。
