...
- Proposed Solution: 基于现有 PRD 与测试手册执行启动器全功能审查，覆盖 CLI/脚本行为、生命周期与就绪、Web 闭环（Playwright）以及迁移阻断路径，并沉淀审计结论与证据路径。
- Success Criteria:
  - SC-1: 启动器核心能力清单（脚本迁移/生命周期/鉴权注入/静态目录覆盖/阻断策略）均有逐项结论。
  - SC-2: 至少完成 1 次真实 Web 闭环（`world_game_launcher + Playwright`）并产出截图/日志/state 证据。
...
- User Stories:
  ...
  - PRD-TESTING-LAUNCHER-REVIEW-002: As a 测试维护者, I want real Web loop evidence from Playwright, so that UI availability is not inferred from unit tests only.
...
- Critical User Flows:
  ...
  2. Flow-REVIEW-002: `执行 Playwright 语义动作 -> 采集 snapshot/console/screenshot/state -> 形成闭环证据`
...
| Web 闭环验收 | Playwright `snapshot/eval/console/screenshot` | 以语义动作操作 Viewer 并采样 | `sampling -> evidenced` | 至少包含 screenshot + state + console | 执行者产出，发布者审阅 |
...
- Tool Requirements: Playwright CLI（`$CODEX_HOME/skills/playwright/scripts/playwright_cli.sh`）用于真实浏览器操作；仓库脚本 `run-game-test.sh`、`viewer-release-qa-loop.sh`、`s10-five-node-game-soak.sh`、`p2p-longrun-soak.sh`。
- Evaluation Strategy: 通过闭环动作成功率、脚本返回码、启动器就绪判定、故障签名可定位性评估审查质量。

## 4. Technical Specifications
- Architecture Overview: 本审查以 `world_game_launcher` 为核心执行器，结合启动脚本与 Playwright 形成“进程编排 -> Web 可达 -> 语义交互 -> 证据采集 -> 分级结论”的验证流水线。
- Integration Points:
...
- Phased Rollout:
  ...
  - v2.0 (LAUNCHREV-3): 完成真实 Web Playwright 闭环与证据采样。
...
| PRD-TESTING-LAUNCHER-REVIEW-002 | LAUNCHREV-2/3 | `test_tier_required` | `world_game_launcher` 定向测试 + Playwright 真实闭环采样 | Web 可用性与 UI 真实性 |
| PRD-TESTING-LAUNCHER-REVIEW-003 | LAUNCHREV-2/4 | `test_tier_required` | 迁移脚本行为验证 + 阻断文案验证 + 风险分级 | 脚本兼容与误用防护 |
- Decision Log:
| 决策ID | 选定方案 | 备选方案（否决） | 依据 |
| --- | --- | --- | --- |
| DEC-REVIEW-001 | 审查默认采用 `world_game_launcher + Playwright` 真闭环 | 仅依赖单元测试/日志静态审查 | 真闭环可验证真实用户路径，降低假通过。 |
...
- 实际闭环要求（Playwright 真浏览器） -> 第 2 章 Flow-REVIEW-002 + 第 3/4 章工具与架构。
