...
- 制作人试玩 / 发布前人工验收默认先执行 `scripts/build-game-launcher-bundle.sh`，再通过 `<bundle>/run-game.sh` 或 `./scripts/run-game-test.sh --bundle-dir <bundle>` 启动；未传 `--bundle-dir` 的源码模式仅用于开发回归和问题复现。
