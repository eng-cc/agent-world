...
- [x] LAUNCHMIG-6 (PRD-TESTING-LAUNCHER-SCRIPT-004): 修复 Viewer 静态资源目录兼容（`viewer-release-qa-loop` 透传 `--viewer-static-dir` + `world_game_launcher` 支持 `OASIS7_GAME_STATIC_DIR` 覆盖默认目录）。
