...
- [ ] `RPOF-2`：实现 runtime perf 模块（采集 + 统计 + 分析）
  - [ ] 新增 simulator runtime perf 数据结构与统计逻辑
  - [ ] 增加 health/bottleneck 规则与单测
- [ ] `RPOF-3`：接入 `AgentRunner` 与 `RunnerMetrics`
  - [ ] `tick/tick_decide_only/notify_action_result` 采集
  - [ ] 外部 action 执行补录接口
  - [ ] `RunnerMetrics` 扩展与兼容默认值
- [ ] `RPOF-4`：输出链路打通
  - [ ] `world_llm_agent_demo` 报告与 stdout 接线
  - [ ] `world_viewer_live` metrics 链路接线（LLM driver）
- [ ] `RPOF-5`：长跑脚本汇总扩展
  - [ ] `scripts/llm-longrun-stress.sh` 读取 runtime perf 字段
  - [ ] 单场景 summary 与多场景聚合输出补充
- [ ] `RPOF-6`：测试与回归
...
## 状态
- 当前阶段：`RPOF-2` 进行中（开始实现 runtime perf 模块与 runner 接线）
- 阻塞：无
- 风险跟踪：
