...
After the separately authorized split is reviewed, update references and remove superseded duplicate authority. Then deliver the separate policy/code change that recognizes the resulting paths and exact dependency/completion semantics. Activation requires those changes plus real acceptance evidence; approval of this plan alone satisfies none of those implementation steps.
