...
The old-base inline bootstrap resolves a unique canonical Task UID from the live PR before finding its exact Issue and reciprocal PR binding. Absorbed explanatory Refs do not create competing task identities. Multiple UIDs, ambiguous lookup or mismatched/repeated reverse binding block. A PR without a UID may use only the existing unique-Refs reciprocal legacy path; candidate helpers remain forbidden on an old effective base.

Request ordering uses the execution-attempt start (or the queued rerun observation timestamp), with run ID/attempt as deterministic ties; a pending newer attempt cannot consume a prior attempt's green artifact. After artifact validation, selection is re-read and must still identify the same request/attempt. `pr-lifecycle-gate.py --integration-run-id <RUN-ID>` accepts a locator only and applies the same discovery/validation as receipt issuance, recording the live selected run in readiness evidence. Ordinary PR green is considered only after complete discovery proves no matching manual request; an old default workflow that demonstrably cannot dispatch integration may retain its ordinary compatibility path. A missing title on a workflow that could dispatch integration remains uncertain, not unrelated.
