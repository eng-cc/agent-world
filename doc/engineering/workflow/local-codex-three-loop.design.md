...
CI has a separate credential prerequisite. `rust.yml` gives its required-gate job only repository `contents`, `issues` and `pull-requests` read permissions. Its isolated trusted admission step accepts repository secret `OASIS7_LOOP_READ_TOKEN`; the candidate scope planner and tests never receive it. A bound loop must pass a live `gh project view 1` probe using that credential before the effective helper runs. Missing or denied access blocks activation explicitly; local `gh` login and fixture tokens do not establish runner access. Legacy admission continues using `GITHUB_TOKEN` and repository Issue/PR/comment readbacks.

Provision the credential through repository administration with read access to the selected Project and repository metadata, Issues, pull requests and contract-author permission readbacks. For a user-owned Project, a suitably scoped PAT needs `read:project`; for an organization Project, use an appropriately authorized PAT or GitHub App credential with Projects read access. Avoid write permissions and unrelated repository access. GitHub documents why Projects need [separate authentication](https://docs.github.com/en/issues/planning-and-tracking-with-projects/automating-your-project/automating-projects-using-actions) and why the [repository-scoped default token cannot access Projects](https://github.github.com/gh-aw/reference/auth-projects/). Repository secret-name inspection on 2026-09-09 returned no configured repository secrets; this change provisions none. A real activated CI run with the credential, including terminal dependency/obligation reads, remains required before enablement. Fork PRs without access to the credential remain blocked rather than silently skipping loop admission.
