...
Manual loop CI activation additionally requires an explicitly provisioned read credential for the canonical GitHub Project and repository metadata/Issues/PRs. `GITHUB_TOKEN` alone is not Project authority. The trusted admission step alone receives `OASIS7_LOOP_READ_TOKEN`; it must probe selected Project access and fail closed when absent or denied, without passing the credential to candidate planning or tests. Legacy repository readbacks retain the job's minimal read-only `GITHUB_TOKEN`. Provisioning and actual activated CI verification remain explicit enablement prerequisites, not consequences of merging these helpers.
