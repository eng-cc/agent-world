...
- [x] TASK-P2P-040 (PRD-P2P-022) [test_tier_required]: 新增“shared network / release train 最小执行形态”专题 PRD / design / project / runbook，并把 `shared_devnet/staging/canary`、`release_candidate_bundle`、promotion/freeze/rollback、liveops run window 与 claims gate 纳入模块追踪。
  ...
  - 产物文件:
    ...
    - `doc/p2p/blockchain/p2p-mainnet-public-claims-policy-2026-03-23.project.md`
...
- 下一任务: 继续执行 `TASK-P2P-040` 的 shared-network 实操，先用 `shared-devnet-rehearsal.sh` 收口 same-candidate 多入口与 lane scaffold，把 first `shared_devnet` dry run 的 `partial` 提升到 `pass`，再进入 `staging/canary` rehearsal；在此之前继续执行当前 preview claims policy。
- 最新完成: `TASK-P2P-040`（已完成 `RTMIN-4A` 前置收口：shared-devnet orchestration 入口已落地，可围绕同一 `candidate_id` 复跑 multi-entry、生成 lane scaffold 与 shared-network gate；但当前 shared-network 总结论仍为 `partial`，尚未达到 promotion-ready `pass`。）
