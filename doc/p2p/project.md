...
- 最新进展: `TASK-P2P-041-D` 已把 hosted access hint 接到 game URL：`oasis7_web_launcher` 生成的 viewer URL 现会附带精简 `hosted_access` query contract，`software_safe.js` 与 `__AW_TEST__.getState()` 会优先消费这份真值做 capability 判定与 QA 采样，不再只靠 hostname 启发式。
