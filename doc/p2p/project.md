...
- 最新进展: `TASK-P2P-041-B/C` 已修正独立 runtime presence monitor：`oasis7_game_launcher` 现在会起后台线程按固定间隔短连 `live_bind` 拉取 snapshot，对账后立即释放连接，不再用常驻 monitor 独占 runtime-live 的单连接入口；这样 hosted browser websocket 可以重新拿到 `snapshot/metrics`，而 admission snapshot 与 viewer summary 也继续回出 `runtimeProbe` 状态和错误，便于直接判断 public player plane 当前是否仍在健康对账。
- 最新进展: `TASK-P2P-041-B/C` 已补 runtime-live 多客户端并发：`oasis7_viewer_live` 现允许 browser/web-bridge 长连与 hosted runtime probe 并发访问同一 `live_bind`，不再出现“浏览器一打开，admission.runtime_probe_status 就掉成 hello_ack timeout”的新型抢占；真实本地 hosted 栈下，页面保持打开时 `runtime_probe_status` 仍可维持 `ok`。
- 最新进展: `TASK-P2P-041-B/C` 已补 web-bridge 多浏览器并发接入：`ViewerWebBridge::run()` 不再串行 `accept -> serve_stream`，而是为每个 websocket 派生独立线程；定向回归已覆盖“首个 websocket 保持打开时，第二个 websocket 仍能连到 upstream”，真实本地 hosted 栈上的双 `agent-browser` session 也已验证第二个页面不再永久卡在 `debug_viewer:detached`，最多只会经历一次自动重连窗口后收敛到双 `subscribed`。
...
- 最新进展: `TASK-P2P-041-B/C` 已补 revoke-path unbind 事件契约：runtime revoke 现在会发出 `AgentPlayerUnbound` 虚拟事件；当前 hosted presence reconcile 仍主要靠下一轮短连 snapshot 收敛，但这条事件真值已为后续多客户端 presence bus / 更快回流预留好协议基础。
- 最新进展: `TASK-P2P-041-C` 已补 same-agent rebind 事件契约：runtime-live 现在会在同一 agent 从旧 player 改绑到新 player 时显式发出 `AgentPlayerUnbound(old) -> AgentPlayerBound(new)` 事件序列；当前 hosted presence monitor 仍以周期性 snapshot 为准，但后续若补多客户端 presence bus，不必再倒推旧玩家解绑语义。
...
- 最新进展: `TASK-P2P-041-B/D` 已补资产阻断态的独立玩家可见面：交互区现有单独的 `Asset / Governance Lane` 面板，会把 `main_token_transfer` 的 `required_auth/availability`、阻断原因和禁用 CTA 直接摊给玩家与 QA。
- 最新进展: `TASK-P2P-041-B/C` 已补 hosted 会话恢复提示面：viewer summary 现会在 `session_revoked/session_not_found/本地 release` 后显示 `Hosted Recovery` 面板，并把派生结果同步暴露到 `__AW_TEST__.getState().hostedRecoveryHint`，避免玩家只看到重新掉回 guest 却不知道下一步动作。
...
- 最新进展: `TASK-P2P-041-D` 已完成真实 hosted grant 联调定位：transport / runtime probe / web-bridge 多页并发都已不再构成主 blocker；伪造 env signer 现会被显式拒绝为 keypair mismatch，换成真实匹配 signer 后，不带 `--with-llm` 会停在 `llm_mode_required`，带 `--with-llm` 则进一步命中 `release_token does not map to an active player slot`。当前真缺口已经收敛为 hosted session issuer 在 runtime 仍绑定玩家时提前释放 active slot 的生命周期竞争。
- 最新进展: `TASK-P2P-041-C/D` 已补 hosted session issuer 生命周期修复：`observe_runtime_active_players()` 现会先用当前 runtime probe snapshot 给仍在 runtime 里绑定的 active slot 续租，再执行过期清理，不再在同一轮 probe 明明已看到该 player 仍活着时，先把 release token 按旧 `last_seen` 剪掉。`hosted_player_session_runtime_probe_refreshes_runtime_bound_slot_before_expiry_prune` 与整套 `oasis7_game_launcher` bin 单测均已通过；剩余待办是把这条修复拿到真实 hosted `prompt_control` 成功链路里复验。
- 最新进展: `TASK-P2P-041-C/D` 已补 public player plane 路由前的同步 runtime reconcile：`admission/issue/refresh/release/strong-auth grant` 现在会在处理前短连 `live_bind` 拉最新 snapshot，把“刚完成 register、后台 presence monitor 还没来得及看到”的 runtime binding 先回写到 issuer，再执行 hosted session / grant 逻辑。这关闭了 `pending_registration_ttl_ms` 与首个 strong-auth grant 之间的时序竞争。
- 最新进展: `TASK-P2P-041-D` 已拿到真实 hosted success evidence：在真实 signer + `--with-llm` + browser-local ephemeral key + approval code 的 hosted 栈上，选用未绑定的 `agent-1` 时，`prompt_control preview/apply` 均已拿到 `preview_ack/apply_ack`，`strongAuthLastGrantError` 为空，runtime snapshot 也确认 `agent-1` 绑定到当前 player；`release_token does not map to an active player slot` 不再复现。
- 最新进展: `TASK-P2P-041-B/C/F` 已补 remote revoke 结构化回流：runtime-live 的 `authoritative_recovery_ack/error` 现会在 `session_revoked` 时携带 `revoke_reason/revoked_by`，`software_safe.js` 会把这两项真值保留到 guest fallback，并通过 `Hosted Recovery` 面板与 `__AW_TEST__.getState().authRevokeReason/authRevokedBy` 直接暴露给玩家、QA 和 operator runbook。
...
- 最新进展: `TASK-P2P-041-E` 已补 capability bypass 定向回归：`oasis7_hosted_access` 现要求即便 prompt-control preview reauth 环境已配置完成，`main_token_transfer` 仍必须保持 `blocked_until_strong_auth`，不能被 prompt lane 的 backend grant 环境顺带开放。
- 最新进展: `TASK-P2P-041-E` 已补 public snapshot 组合态回归：`oasis7_web_launcher::server` 现要求 env-ready 时同一份 hosted action matrix 必须同时体现“`prompt_control_apply` 升到 preview reauth”与“`main_token_transfer` 继续 blocked”，避免对外 contract 导出互相冲突的阶段信号。
- 最新进展: `TASK-P2P-041-E` 已补浏览器侧 hosted evidence：新增 `doc/testing/evidence/hosted-world-browser-auth-surface-2026-03-26.md`，通过真实 `agent-browser` 会话确认 `Hosted Action Matrix`、`Asset / Governance Lane`、`Hosted Recovery`、`pending_registration_ttl_ms` 与 `release_token` 绑定都能在页面上复现；同时确认 detached/agentless 页面下 `prompt_control_*` 不会误签发 grant、`main_token_transfer` 仍返回 `strong_auth_action_not_enabled`。
- 最新进展: `TASK-P2P-041-E/F` 已补 remote revoke 浏览器证据：新增 `doc/testing/evidence/hosted-world-browser-revoke-recovery-2026-03-27.md`，通过真实 hosted 页面 + 外部 `revoke-session` drill 确认下一轮 heartbeat 会把玩家面收敛到 `guest_session`，并同步暴露 `authRevokeReason/authRevokedBy` 与带操作者/原因的 `Hosted Recovery` 文案。
