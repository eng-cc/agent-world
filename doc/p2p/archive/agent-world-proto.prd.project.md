...
## 任务拆解（含 PRD-ID 映射）
- [x] T1 (PRD-P2P-MIG-001)：新增设计文档与项目管理文档（本文件）。
- [x] T2 (PRD-P2P-MIG-001)：新建 `crates/agent_world_proto`，迁移协议类型与 trait 定义。
- [x] T3 (PRD-P2P-MIG-001)：`agent_world` 侧适配（wrapper + 实现对接）并完成编译/测试回归。
- [x] T4 (PRD-P2P-MIG-001)：扩展设计/项目文档，纳入共识成员变更协议类型抽离范围。
- [x] T5 (PRD-P2P-MIG-001)：迁移共识成员变更协议类型到 `agent_world_proto` 并完成 `agent_world` 侧适配回归。
