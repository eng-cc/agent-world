...
- 当前 blocker:
  ...
  - 先前“hosted browser websocket 打开但无消息”的 runtime starvation 已修复：fresh `oasis7_game_launcher --deployment-mode hosted_public_join --chain-disable` 栈上的 ws 探针现在能稳定收到 `snapshot/authoritative_recovery_ack/metrics`。当前剩余缺口已回到产品/证据层面，即还没有带真实 agent state 的 runtime-attached `prompt_control` success evidence。
