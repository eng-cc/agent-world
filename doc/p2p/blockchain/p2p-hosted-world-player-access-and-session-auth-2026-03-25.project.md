...
- 当前 blocker:
  ...
  - 先前两类 runtime starvation 都已修复：fresh `oasis7_game_launcher --deployment-mode hosted_public_join --chain-disable` 栈上的 ws 探针现在能稳定收到 `snapshot/authoritative_recovery_ack/metrics`，且浏览器保持打开时 `/api/public/player-session/admission.runtime_probe_status` 仍可维持 `ok`。当前剩余缺口已回到产品/证据层面，即还没有带真实 hosted auth 升级链路的 runtime-attached `prompt_control` success evidence。
