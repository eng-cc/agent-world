...
- Proposed Solution: 建立 producer-owned 的“生产级 signer custody / keystore”专题 PRD，明确哪些 signer 仍是 local bootstrap、哪些生产路径必须迁移到 managed keystore 或 external signer，并冻结 rotation、revocation、audit trail 与环境分层门禁。
- Success Criteria:
  ...
  - SC-3: 给出生产级 signer custody 的最小完成定义：`managed keystore or external signer`、`rotation`、`revocation`、`audit trail`、`environment policy`。
...
- Critical User Flows:
  ...
  2. Flow-P2P-CUSTODY-002: `runtime 定义 managed signer or keystore 边界 -> 配置层禁止明文私钥继续作为生产入口 -> QA 复核 gate`
...
| Managed custody target | `signer_scope/target_backend/sign_api/key_visibility/verdict` | 为每类 signer 定义生产目标态 | `classified -> target_defined` | 生产目标态必须满足私钥不落仓、不直注入页面、不长期留明文配置 | `runtime_engineer` 牵头 |
...
| Environment policy | `environment/allowed_signer_sources/reject_reason` | 冻结 local/dev/preview/production 允许的 signer 注入方式 | `draft -> enforced` | production 一律禁止 `config.toml` 明文私钥、HTML 私钥注入、env 直传私钥 | `liveops_community` 执行，producer 审批 |
- Acceptance Criteria:
  ...
  - AC-3: 必须为 `node runtime signer`、`viewer/player signer`、`governance/controller signer` 分别写出目标后端与边界。
  - AC-4: 必须定义 `rotation`、`revocation`、`audit trail`、`environment policy` 四类 gate；任一缺失则 `MAINNET-1` 不通过。
  - AC-5: 必须明确生产环境禁止把私钥明文写进仓库、`config.toml`、HTML bootstrap、长期环境变量。
...
- Non-Functional Requirements:
  ...
  - NFR-P2P-CUSTODY-2: 生产路径的 signer 私钥不得以明文形式存在于仓库、HTML 页面、长期配置文件或长期环境变量。
...
| DEC-P2P-CUSTODY-003 | `MAINNET-1` 只定义生产级 custody gate，不直接实现 keystore 服务 | 在同一专题里同时完成所有 keystore 工程改造 | 先冻结边界和 gate，才能避免后续实现反复返工。 |
