...
- Proposed Solution: 建立 `MAINNET-2` 专题 PRD，把治理 finality signer 与 controller signer 的外部化目标、source-of-truth 边界、failover、rotation、revocation 和 operator ownership 一次性冻结下来，并明确在完成前 oasis7 仍不得升级为 `mainnet-grade`。
- Success Criteria:
  ...
  - SC-3: 给出治理 signer 外部化的最小完成定义：`externalized source of truth`、`rotation`、`revocation`、`failover`、`operator ownership`。
...
- Critical User Flows:
  ...
  2. Flow-P2P-GOVSIGN-002: `runtime 定义 externalized signer source -> 把 local seed/config 退出 production path -> QA 复核 failover/rotation/revocation`
...
| Externalized truth target | `governance_scope/target_truth_store/signer_registry/update_authority` | 冻结生产级长期真值落点 | `classified -> target_defined` | 若 production truth 仍依赖 local seed/config，则不通过 | `runtime_engineer` 牵头 |
...
- Acceptance Criteria:
  ...
  - AC-3: 必须分别为 `finality signer` 与 `controller signer` 写出长期 source-of-truth 目标与更新 authority。
...
- Architecture Overview: 当前治理 signer 风险分成两层。finality 路径在 `world/governance` 内仍会从 deterministic local seed 派生 signer；controller 路径虽然已经要求 threshold proof，但 policy 真值仍由 `NodeConfig.main_token_controller_binding.controller_signer_policies` 提供。`MAINNET-2` 的目标不是一次写完工程实现，而是先冻结长期治理真值、更新 authority 与失效恢复门禁。
- Integration Points:
...
- Non-Functional Requirements:
  - NFR-P2P-GOVSIGN-1: production governance signer truth 不得由 deterministic local seed 或单机本地配置单独承担。
...
| DEC-P2P-GOVSIGN-003 | `MAINNET-2` 先冻结 externalization gate，不在本轮直接写完工程实现 | 同一专题里同时完成全部 runtime 改造 | 先冻结长期治理真值和失效恢复边界，后续实现才不会返工。 |
