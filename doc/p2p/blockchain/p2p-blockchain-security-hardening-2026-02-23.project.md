...
- [x] T3：实现 replication 远端 writer allowlist 授权校验。
- [x] T4：实现 fetch-commit/fetch-blob 请求签名鉴权（客户端签名 + 服务端验签）。
- [x] T5：实现网络订阅有界缓存与 membership DHT 恢复默认策略收紧。
- [x] T6：补齐定向/回归测试，回写文档状态与 devlog。
