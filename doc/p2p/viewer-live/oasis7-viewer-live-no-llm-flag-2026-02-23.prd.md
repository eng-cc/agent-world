# oasis7_viewer_live `--no-llm` 关闭开关设计文档（2026-02-23）
...
## 1. Executive Summary
- Problem Statement: 在 `oasis7_viewer_live` 默认启用 LLM 决策的前提下，提供显式关闭开关 `--no-llm`，用于本地调试或脚本回退到 Script 决策。
...
- User Stories: As a 维护者, I want oasis7_viewer_live `--no-llm` 关闭开关设计文档（2026-02-23） 的需求结构化, so that implementation is auditable.
