# oasis7_viewer_live 发行锁定启动（P2P）设计文档（2026-02-23）
...
审计轮次: 5
> 状态更新（2026-03-08）:
> - `oasis7_viewer_live` 已移除 `--release-config` 与 `--node-*` 控制面参数。
> - 链路控制面统一由 `oasis7_chain_runtime`（通常经 `oasis7_game_launcher/oasis7_web_launcher/oasis7_client_launcher`）承载。
> - 本文仅保留历史归档，不再作为当前实现验收门禁。

## 1. Executive Summary
- Problem Statement: 将 `oasis7_viewer_live` 从“开发期高自由度 CLI”收口为“发行期锁定参数启动”，避免 P2P 上线后因单节点临时调参导致网络语义漂移。
...
- User Stories: As a 维护者, I want oasis7_viewer_live 发行锁定启动（P2P）设计文档（2026-02-23） 的需求结构化, so that implementation is auditable.
...
- Acceptance Criteria:
  ...
  - AC-5: `crates/oasis7/src/bin/oasis7_viewer_live.rs`（`#[cfg(test)]`）
