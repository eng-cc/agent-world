...
## 1. Executive Summary
- Problem Statement: `agent_world_node` 内部已实现 wall-clock slot 与 logical tick 语义，但 `world_chain_runtime/world_viewer_live/launcher/scripts` 仍以 `node_tick_ms` 为主入口，导致“轮询间隔”和“出块时间参数”混淆。
...
- Success Criteria:
  - SC-1: `world_chain_runtime` 与 `world_viewer_live` 可配置 `slot_duration_ms/ticks_per_slot/proposal_tick_phase` 等核心参数。
...
- User Stories:
  - PRD-P2P-NODE-SURFACE-001: As a 协议工程师, I want runtime/viewer to accept slot-clock parameters, so that consensus timing is configured explicitly.
...
- Critical User Flows:
  ...
  2. Flow-NODE-SURFACE-002: `用户启动 world_viewer_live -> 内嵌节点读取参数 -> 共识运行 -> UI/日志不误导 node_tick_ms 语义`
...
| viewer live 内嵌节点参数映射 | 与 runtime 同口径参数 + `--node-tick-ms` | 构造 `NodeConfig`/`NodePosConfig` | `configured -> started` | `node_tick_ms` 仅用于轮询与回退间隔 | 运行节点操作者可配置 |
| launcher 配置透传 | `chain_node_tick_ms` + PoS 参数字段 | UI 录入、校验、构建 args | `editing -> validated -> launched` | 参数缺失使用默认值，非法值拒绝 | 本地用户可配置 |
| chain status 观测扩展 | `ticks_per_slot/tick_phase/proposal_tick_phase/last_observed_tick/missed_tick_count/adaptive_tick_scheduler_enabled` | `/v1/chain/status` 返回并供脚本消费 | `collecting -> reported` | 与 `NodeConsensusSnapshot` 一致映射 | 只读公开 |
...
- Acceptance Criteria:
  ...
  - AC-2: `world_viewer_live` 支持并校验全部 PoS 时间锚定参数。
...
- Non-Functional Requirements:
  ...
  - NFR-2: status 字段投影与快照保持 1:1，不引入推导字段漂移。
