...
- User Stories: As a 维护者, I want README P2 缺口收口：Node Replication 统一到 agent_world_net 网络栈 的需求结构化, so that implementation is auditable.
...
- Acceptance Criteria:
  ...
  - AC-3: 改为对 `agent_world_net::Libp2pNetwork` 的封装。
  - AC-4: 在封装层补齐 node 语义（轮换/重试/回退）。
  - AC-5: `crates/agent_world_node/Cargo.toml`
  - AC-6: 增加 `agent_world_net`（仅 native 目标依赖）。
- Non-Goals:
  - 不扩展超出原文边界的新需求。
...
- Integration Points:
  ...
  - `doc/p2p/node/readme-p2-node-net-stack-unification.prd.project.md`
  - `testing-manual.md`
