...
- [ ] STRAUTH-2 (PRD-P2P-TXAUTH-001/003) [test_tier_required]: 由 `runtime_engineer` 继续把 `ClaimMainTokenVesting / InitializeMainTokenGenesis / DistributeMainTokenTreasury` 纳入统一 signed transaction envelope。
  ...
  - [ ] STRAUTH-2B [test_tier_required]: 将 genesis/treasury 的治理控制从“signed metadata”推进到正式 controller slot binding，并继续保留 signer allowlist / ceremony 后续任务。
    ...
    - [ ] STRAUTH-2B2 [test_tier_required]: 将 controller slot 继续收口到真实 signer allowlist / threshold / ceremony，移除仅 slot binding 的临时阶段。
...
- 仍待完成:
  - genesis/treasury 仍缺真实 signer allowlist / threshold / ceremony 收口，当前只完成 controller slot binding。
...
## 状态
- 当前阶段: active
- 下一步: 进入 `STRAUTH-2B2`，把 controller slot 继续收口到真实 signer allowlist / threshold / ceremony；随后再由 `viewer_engineer` 补 `STRAUTH-3` 的签名 UI/交互闭环。
- 最近更新: 2026-03-23
