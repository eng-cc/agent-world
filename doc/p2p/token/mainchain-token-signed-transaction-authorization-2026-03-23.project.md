...
- 仍待完成:
  - claim/genesis/treasury 三类资产动作的统一签名模型。
  - Web/native UI 真正产出签名材料的交互闭环。
...
## 状态
- 当前阶段: active
- 下一步: 执行 `STRAUTH-2`，把 claim/genesis/treasury 动作纳入同一 signed transaction 模型；并由 `viewer_engineer` 补 `STRAUTH-3` 的签名 UI/交互闭环。
- 最近更新: 2026-03-23
