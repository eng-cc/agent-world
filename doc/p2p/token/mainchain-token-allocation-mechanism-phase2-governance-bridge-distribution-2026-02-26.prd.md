# oasis7 主链 Token 分配机制二期：地址绑定 + 治理绑定 + 分发闭环（2026-02-26）
...
- User Stories: As a 维护者, I want oasis7 主链 Token 分配机制二期：地址绑定 + 治理绑定 + 分发闭环（2026-02-26） 的需求结构化, so that implementation is auditable.
