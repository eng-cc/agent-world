...
## 任务拆解
- [x] T0：完成设计文档与项目管理文档。
- [ ] T1：实现 `agent_world_node` 执行校验强化（配置、入站校验、gap-sync 重放校验、快照可观测字段）。
- [ ] T2：实现 `world_viewer_live` reward runtime leader/failover 策略与生产默认接线。
- [ ] T3：补齐测试并执行定向回归（node + world_viewer_live），回写文档与 devlog。
