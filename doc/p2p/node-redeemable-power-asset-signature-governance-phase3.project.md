...
- [ ] SGC-2：落地 `mintsig:v2`（ed25519）签名/验签与治理策略结构。
- [ ] SGC-3：实现签名版兑换动作与治理门禁（策略要求时拒绝无签名兑换）。
- [ ] SGC-4：接线 `world_viewer_live` reward runtime（真实私钥结算 + 签名兑换）。
- [ ] SGC-5：补齐 `test_tier_required` 回归、回写文档状态与 devlog 收口。
...
## 状态
- 当前阶段：SGC-1 完成，进入 SGC-2（`mintsig:v2` + 治理策略实现）。
- 阻塞项：无。
- 最近更新：2026-02-17。
