...
审计轮次: 5
## 1. Executive Summary
- Problem Statement: 完成分布式能力到基础 crate 的彻底拆分：`oasis7_net` / `oasis7_consensus` / `oasis7_distfs` / `oasis7_proto`。
- Proposed Solution: 删除 `oasis7` 内分布式实现文件，`oasis7` 仅保留世界内核与模拟层，不再承载分布式实现逻辑。
- Success Criteria:
  - SC-1: 将 Viewer 协议并入 `oasis7_proto`，消除 `oasis7` 对 Viewer 协议层的承载。
...
- User Stories: As a 维护者, I want oasis7 Runtime：分布式能力彻底拆分（Phase 7） 的需求结构化, so that implementation is auditable.
...
- Acceptance Criteria:
  - AC-1: 新增 `oasis7_distfs` crate，并迁移分布式文件能力：CAS、分片、组装校验。
  - AC-2: 分布式彻底拆分（历史里程碑）：删除 `crates/oasis7/src/runtime/distributed*`、`libp2p_net.rs` 及相关分布式路径代码。
  - AC-2b: 当前实现锚点：分布式主线由 `oasis7_net/oasis7_consensus/oasis7_distfs/oasis7_proto` 四个基础 crate 承载，`oasis7` 不再承载分布式核心实现。
  - AC-3: `oasis7` facade 收敛：移除分布式导出，按领域划分导出边界。
  - AC-4: Viewer 协议迁移到 `oasis7_proto` 并完成 server/viewer 双端适配。
  - AC-5: WASM ABI 边界调整：消除 `oasis7_net` 内重复 `ModuleManifest`，统一到 ABI/proto。
  - AC-6: 超长文件拆分（>1200 行）。
- Non-Goals:
...
- Integration Points:
  ...
  - `crates/oasis7_proto/src/lib.rs`
  - `testing-manual.md`
...
## 5. Risks & Roadmap
- Phased Rollout:
  - M13-1：`oasis7_distfs` 落地并接入现有路径
  - M13-2：`oasis7` 分布式文件删除完成，调用方切换到基础 crate
  - M13-3：`oasis7` facade 收敛完成
...
- 验收命令:
  - `test -f crates/oasis7_net/src/lib.rs`
  - `test -f crates/oasis7_consensus/src/lib.rs`
  - `test -f crates/oasis7_distfs/src/lib.rs`
  - `test -f crates/oasis7_proto/src/lib.rs`
