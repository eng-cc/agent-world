...
- [x] CCG-1 (PRD-P2P-MIG-058)：在 `oasis7_consensus` 实现可复用 node_pos 核心状态机，并在 `oasis7_node` 完成接线替换。
- [x] CCG-2 (PRD-P2P-MIG-058)：补齐/回归测试并回写文档与 devlog。
- [x] CCG-3 (PRD-P2P-MIG-058)：扩展设计/项目文档，定义第二阶段全收口任务。
- [x] CCG-4 (PRD-P2P-MIG-058)：将 `oasis7_node` 残留共识纯逻辑（action/signature/message）迁移到 `oasis7_consensus` 并接线。
...
- `crates/oasis7_consensus/src/network.rs`
