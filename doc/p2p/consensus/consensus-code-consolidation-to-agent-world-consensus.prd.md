# oasis7 Runtime：共识代码统一收敛到 agent_world_consensus
...
- User Stories: As a 维护者, I want Agent World Runtime：共识代码统一收敛到 agent_world_consensus 的需求结构化, so that implementation is auditable.
...
- Acceptance Criteria:
  - AC-1: 在 `crates/agent_world_consensus` 新增 node 可复用的 PoS 核心模块：
  - AC-2: attestation 结构
  - AC-3: pending proposal 结构
  - AC-4: decision 结构
  - AC-5: proposal 构造、attestation 插入、状态推进函数
  - AC-6: `crates/agent_world_node` 改为调用上述核心模块，不再本地维护同构结构与核心推进算法。
- Non-Goals:
