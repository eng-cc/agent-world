# Agent World Runtime：节点存储系统奖励池（项目管理文档）

## 任务拆解
- [x] SBR-1：完成设计文档与项目管理文档。
- [x] SBR-2：实现 `node_points` 双池结算（主池+存储池）与存储挑战门槛逻辑，补齐单元测试。
- [x] SBR-3：实现 `node_points_runtime` 存储挑战采样透传与运行时测试。
- [ ] SBR-4：执行 `test_tier_required` 回归，回写文档与 devlog 收口。
...
## 状态
- 当前阶段：SBR-1~SBR-3 已完成，SBR-4 待完成。
- 最近更新：2026-02-16。
