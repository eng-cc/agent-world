# oasis7 Runtime：`oasis7_net` runtime_bridge 可编译闭环
...
审计轮次: 5
## 1. Executive Summary
- Problem Statement: 让 `oasis7_net --features runtime_bridge` 在当前拆分架构下可独立编译通过。
...
- User Stories: As a 维护者, I want oasis7 Runtime：`oasis7_net` runtime_bridge 可编译闭环 的需求结构化, so that implementation is auditable.
...
- Acceptance Criteria:
  - AC-1: `oasis7_net` runtime_bridge 相关模块导入收敛：
  - AC-2: `bootstrap.rs`
...
- 依赖来源调整：
  - BlobStore/分片组装：`oasis7_distfs`
  - World/Snapshot/Journal/事件类型：`oasis7::runtime`
  - 协议类型：`oasis7_proto`
...
- Technical Risks:
  - `oasis7_net -> oasis7` 依赖可能增加层级耦合，需要后续继续下沉抽象。
