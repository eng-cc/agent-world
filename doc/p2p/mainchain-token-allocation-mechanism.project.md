...
- [ ] TAM-1：实现主链 Token 状态模型、快照字段与基础查询接口。
- [ ] TAM-2：实现创世分配初始化与 vesting 领取动作闭环。
- [ ] TAM-3：实现 epoch 增发公式与分配执行（含余数确定性策略）。
- [ ] TAM-4：实现 Gas/罚没/模块费用销毁与协议金库记账。
...
## 状态
- 当前阶段：TAM-0 已完成（文档草案阶段）。
- 下一步：TAM-1（主链 Token 状态模型与快照落地）。
- 最近更新：2026-02-26。
