...
- [ ] BWI-3 扩展 `scripts/sync-m1-builtin-wasm-artifacts.sh`：生成/校验 identity manifest
- [ ] BWI-4 扩展 `scripts/sync-m4-builtin-wasm-artifacts.sh` 与 `scripts/sync-m5-builtin-wasm-artifacts.sh` 复用 identity 生成/校验
- [ ] BWI-5 runtime 接入 identity manifest（m1/m4/m5 builtin artifact loader）
- [ ] BWI-6 bootstrap 流程改造：`bootstrap_power/economy/gameplay` 使用 identity manifest 生成 `artifact_identity`
- [ ] BWI-7 测试补齐：identity 解析/校验单测与 runtime 闭环测试
...
## 状态
- 当前阶段：执行中（BWI-3）
- 最近更新：BWI-1/BWI-2 完成（2026-02-20）
