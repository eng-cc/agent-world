...
审计轮次: 4
## 任务拆解（含 PRD-ID 映射）
- [x] BWD-1 输出设计文档（`doc/p2p/distfs/distfs-builtin-wasm-storage.prd.md`）与项目管理文档（本文件）。 (PRD-P2P-MIG-060)
- [x] BWD-2 改造 wasm 同步脚本 (PRD-P2P-MIG-060)：以 hash 清单为 git 基线，产物落盘到 DistFS 本地存储。
- [x] BWD-3 改造 pre-commit (PRD-P2P-MIG-060)：提交前执行 builtin wasm 校验。
- [x] BWD-4 改造 runtime builtin wasm 加载路径 (PRD-P2P-MIG-060)：DistFS 读取 + hash 校验。
- [x] BWD-5 移除 wasm 二进制 git 追踪，更新 ignore 规则。 (PRD-P2P-MIG-060)
- [x] BWD-6 执行 `test_tier_required` 回归并完成文档/devlog 收口。 (PRD-P2P-MIG-060)
