...
- ROUND-035 进展: `runtime_engineer` 已定位并修复 `scripts/s10-five-node-game-soak.sh` 的 cleanup 漏洞：此前 run 结束后会残留默认端口 listener，导致下一轮 sequencer startup 直接报 `bind 127.0.0.1:5811 failed: Address already in use`；修复后，`--base-port 6410` 的 back-to-back 10 秒诊断样本已证明不再残留 listener。与此同时，`20260322-115646` 的 60 秒样本也确认 reward runtime / settlement 链路本身可跑通。当前剩余阻断是：修复后的默认端口 120 秒/600 秒候选样本与 replay/rollback drill 仍未补齐，因此阶段继续保持 `internal_playable_alpha_late`。
